...
/* The XSI/BSD math constants.  Only M_PI and M_E were here, which is the kind
 * of gap that shows up as a compile error in ported code rather than anything
 * subtler -- NetSurf's chart drawing wants M_PI_2.  All are given to 21
 * significant digits, enough that they are exact when rounded to long double
 * on this target, so the same name never means a different number depending on
 * the type it is used with. */

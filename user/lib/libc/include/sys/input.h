...
//
// The event-code lists are COMPLETE rather than limited to what the drivers
// here currently emit.  That is deliberate: libevdev builds a lookup table
// covering every code in this header, so a partial list is not merely a
...
// program down the kernel path, where the include is unreachable.
...
// Names for the timestamp fields.  Code written against the modern header
// reaches for these rather than time.tv_sec, because on 32-bit systems the
// two are not the same width; here they are simply the fields themselves.
...
// Device properties (EVIOCGPROP).  An input driver uses these to tell a
// display server how a device behaves beyond its raw axes — chiefly whether
// a pointer is indirect (a mouse) or maps onto the screen (a touchscreen).
...
// Absolute axes (EV_ABS).  No device here reports absolute motion yet; the
// codes exist so a driver can ask about them and be told "not supported"
// rather than having the query rejected outright.
#define ABS_X 0x00
#define ABS_Y 0x01
#define ABS_Z 0x02
#define ABS_RX 0x03
...
// Miscellaneous events (EV_MSC).  MSC_SCAN carries a key's raw hardware
// scancode alongside the translated KEY_* code, which is how a display server
// tells apart two physical keys that map to the same symbol.
...
// Sounds a device can be asked to make (EV_SND)
...
// Autorepeat parameters (EV_REP), in milliseconds
#define REP_DELAY 0x00
#define REP_PERIOD 0x01
#define REP_MAX 0x01
#define REP_CNT (REP_MAX+1)

// Force feedback (EV_FF).  No device here produces it; as with the absolute
// axes above, the codes exist so that asking is answered rather than rejected.
...
// Status values an effect can report back (EV_FF_STATUS)
...
// Indices into struct input_id, for the ioctls that address one field of it.
#define ID_BUS 0
#define ID_VENDOR 1
#define ID_PRODUCT 2
#define ID_VERSION 3

// Multitouch tool types, the value carried by ABS_MT_TOOL_TYPE: what is
// touching the surface, as opposed to where.
...
// Absolute-axis parameters returned by EVIOCGABS.
struct input_absinfo {
	int32_t value; // current position
	int32_t minimum;
	int32_t maximum;
	int32_t fuzz; // noise filter
	int32_t flat; // dead zone around the centre
	int32_t resolution; // units per mm (or per radian for rotations)
};
...
#define EVIOCGPHYS(len) \
	_INPUT_IOC(_INPUT_IOC_READ, 'E', 0x07, (len)) // physical location
#define EVIOCGUNIQ(len) \
	_INPUT_IOC(_INPUT_IOC_READ, 'E', 0x08, (len)) // unique identifier
#define EVIOCGPROP(len) \
	_INPUT_IOC(_INPUT_IOC_READ, 'E', 0x09, (len)) // device properties
#define EVIOCGLED(len) \
	_INPUT_IOC(_INPUT_IOC_READ, 'E', 0x19, (len)) // LED state bitmap
#define EVIOCGSND(len) \
	_INPUT_IOC(_INPUT_IOC_READ, 'E', 0x1a, (len)) // sound state bitmap
#define EVIOCGSW(len) \
	_INPUT_IOC(_INPUT_IOC_READ, 'E', 0x1b, (len)) // switch state bitmap
#define EVIOCGABS(abs) \
	_INPUT_IOR('E', 0x40 + (abs), struct input_absinfo) // axis parameters
#define EVIOCSCLOCKID _INPUT_IOW('E', 0xa0, int) // select event timestamp clock
#define EVIOCGRAB _INPUT_IOW('E', 0x90, int) // grab/release device

// Not implemented by any driver here: the kernel answers EINVAL, which is what
// a caller expects for a device that does not support them.  Declared because
// libevdev probes all three during device setup, and a missing declaration is
// a build failure rather than the runtime "not supported" it is asking for.
#define EVIOCGREP _INPUT_IOR('E', 0x03, unsigned int[2]) // autorepeat settings

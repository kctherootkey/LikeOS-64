...
static __attribute__((noinline, no_stack_protector)) void
_sc_wbuf(const char *b, long n)
{
...
static __attribute__((no_stack_protector)) void _sc_ws(const char *s)
{
	long n = 0;
	while (s[n])
		n++;
...
static __attribute__((no_stack_protector)) void _sc_wdec(uint64_t v)
{
	char buf[22];
	int i = 21;
	buf[21] = '\n';
...
static int __attribute__((no_stack_protector)) _sc_is_user(uint64_t a)
{
	return a >= 0x1000ULL && a < 0x00007ffffffff000ULL;
}

static __attribute__((no_stack_protector)) void _sc_whex(uint64_t v)
{
	/* "    0x" + 16 hex chars + "\n" = 23 bytes */
...
static void __attribute__((no_stack_protector)) _sc_trace(uint64_t rbp)
{
	_sc_ws("Call Trace:\n");
	int depth = 0;
...
static void __attribute__((no_stack_protector)) _sc_dump(uint64_t rsp)
{
	static const char h[] = "0123456789abcdef";
	...
	for (int row = 0; row < 4; row++) {
		if (!_sc_is_user((uint64_t)(p + row * 8)))
			break;
		char line[58];
...
static const char *__attribute__((no_stack_protector)) _sc_pattern(uint64_t c)
{
...
__attribute__((noreturn, no_stack_protector)) void __stack_chk_fail(void)
{
	...
	_sc_ws("\n");
	_sc_ws("pid: ");
	_sc_wdec((uint64_t)pid);
	_sc_ws("thread: ");
	_sc_wdec((uint64_t)tid);
	...
	__asm__ volatile("syscall" : : "a"(312L), "D"(127L) : "rcx", "r11");

...
{
	long ret = syscall6(SYS_SENDTO, sockfd, (long)buf, (long)len, flags,
...
{
	long ret = syscall6(SYS_RECVFROM, sockfd, (long)buf, (long)len, flags,
...
{
	long ret =
		syscall4(SYS_ACCEPT4, sockfd, (long)addr, (long)addrlen, flags);
	if (ret < 0) {
		errno = (int)-ret;
		return -1;
	}
...
{
	long ret = syscall4(SYS_SENDFILE, out_fd, in_fd, (long)offset,
			    (long)count);
	if (ret < 0) {
		errno = (int)-ret;
		return -1;
...
int ppoll(struct pollfd *fds, nfds_t nfds, const struct timespec *tmo_p,
	  const void *sigmask)
{
	long ret = syscall4(SYS_PPOLL, (long)fds, (long)nfds, (long)tmo_p,
			    (long)sigmask);
	if (ret < 0) {
		errno = (int)-ret;
		return -1;
...
{
	long ret = syscall4(SYS_EPOLL_WAIT, epfd, (long)events, maxevents,
			    timeout);
	if (ret < 0) {
		errno = (int)-ret;
		return -1;
...
{
	long ret = syscall3(SYS_DNS_RESOLVE_REVERSE, (long)ip_nbo, (long)out,
			    (long)maxlen);
	if (ret < 0) {
		errno = (int)-ret;
		return -1;

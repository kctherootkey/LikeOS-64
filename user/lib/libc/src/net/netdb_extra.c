...
in_addr_t inet_network(const char *cp)
{
	...
	case 1:
		...
		break;
	case 2:
		out = (parts[0] << 24) | (parts[1] & 0xFFFFFF);
		break;
	case 3:
		out = (parts[0] << 24) | ((parts[1] & 0xFF) << 16) |
		      (parts[2] & 0xFFFF);
		break;
	case 4:
		out = (parts[0] << 24) | ((parts[1] & 0xFF) << 16) |
...
static int classful_shift(uint32_t net)
{
	...
	if ((net & 0x80000000U) == 0)
		return 24; // class A
	if ((net & 0xC0000000U) == 0x80000000U)
		return 16; // class B
...
struct in_addr inet_makeaddr(in_addr_t net, in_addr_t host)
{
	...
	int width;
	if (net < 0x100)
		width = 24; // A: 8-bit net
	else if (net < 0x10000)
		width = 16; // B: 16-bit net
	else
		width = 8; // C: 24-bit net
	(void)s;
...
static const struct {
	const char *n;
	int p;
} g_proto_fallback[] = { { "ip", 0 },  { "icmp", 1 }, { "igmp", 2 },
			 { "tcp", 6 }, { "udp", 17 }, { "raw", 255 },
			 { 0, 0 } };

static int parse_protocols_line(char *line, char *name_out, size_t name_sz,
				int *num_out)
{
...
static struct protoent *lookup_protocols(const char *match_name, int match_num)
{
	...
	if (f) {
		...
		while (fgets(line, sizeof(line), f)) {
			char nm[32];
			int n;
			if (!parse_protocols_line(line, nm, sizeof(nm), &n))
				continue;
			int hit = 0;
			if (match_name && strcmp(nm, match_name) == 0)
				hit = 1;
			if (!match_name && n == match_num)
				hit = 1;
			if (hit) {
	...
	for (int i = 0; g_proto_fallback[i].n; i++) {
		if (match_name &&
		    strcmp(g_proto_fallback[i].n, match_name) == 0) {
			strncpy(g_pe_name, g_proto_fallback[i].n,
				sizeof(g_pe_name) - 1);
			g_pe_name[sizeof(g_pe_name) - 1] = 0;
			g_pe.p_name = g_pe_name;
			g_pe.p_aliases = g_pe_aliases;

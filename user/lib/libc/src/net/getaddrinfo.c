...
static struct addrinfo *ai_alloc(int socktype, int protocol, uint32_t ip_nbo,
				 uint16_t port_nbo)
{
		...
		const struct addrinfo *hints, struct addrinfo **res)
{
	...
	*res = 0;
	...
	uint32_t ip_nbo = 0;
	if (!node) {
		ip_nbo = (flags & AI_PASSIVE) ? htonl(INADDR_ANY) :
	...
	for (int i = 0; i < nst; i++) {
		int proto = protocol;
		if (proto == 0)
			proto = (sts[i] == SOCK_STREAM) ? IPPROTO_TCP :
							  IPPROTO_UDP;
	...
	if ((flags & AI_CANONNAME) && node && head) {
		...
		if (head->ai_canonname)
			memcpy(head->ai_canonname, node, l + 1);
	}

...
static unsigned long scale_value(unsigned long bytes, enum unit_mode mode)
{
	...
	case UNIT_PEBI:
		return bytes / (1024UL * 1024UL * 1024UL * 1024UL * 1024UL);
	case UNIT_KILO:
		return bytes / 1000UL;
	case UNIT_MEGA:
		return bytes / (1000UL * 1000UL);
	case UNIT_GIGA:
		return bytes / (1000UL * 1000UL * 1000UL);
	case UNIT_TERA:
		return bytes / (1000UL * 1000UL * 1000UL * 1000UL);
	case UNIT_PETA:
		return bytes / (1000UL * 1000UL * 1000UL * 1000UL * 1000UL);
	default:
		return bytes / 1024UL;
	}
}

static void print_val(unsigned long bytes, enum unit_mode mode, int si_flag,
		      int field_width)
{
	if (mode == UNIT_HUMAN || mode == UNIT_SI_HUMAN) {
		char buf[64];
		human_readable(bytes, buf, sizeof(buf),
			       (mode == UNIT_SI_HUMAN) || si_flag);
...
int main(int argc, char *argv[])
{
	...
	int opt;
	while ((opt = getopt_long(argc, argv, "bkmghltLvws:c:V", long_options,
				  NULL)) != -1) {
		switch (opt) {
		case 'b':
			mode = UNIT_BYTES;
	...
	do {
		...
		if (line_mode) {
			...
			printf("Mem:");
			printf(" total=");
			print_val(total, mode, si_flag, 0);
			printf(" used=");
			print_val(used, mode, si_flag, 0);
			printf(" free=");
			print_val(free_mem, mode, si_flag, 0);
			printf(" shared=");
			print_val(shared, mode, si_flag, 0);
			printf(" buff/cache=");
			print_val(buffers + cached, mode, si_flag, 0);
			printf(" available=");
			print_val(available, mode, si_flag, 0);
			printf("  Swap:");
			printf(" total=");
			print_val(swap_total, mode, si_flag, 0);
			printf(" used=");
			print_val(swap_used, mode, si_flag, 0);
			printf(" free=");
			print_val(swap_free, mode, si_flag, 0);
			if (show_total) {
				printf("  Total:");
				printf(" total=");
				print_val(total + swap_total, mode, si_flag, 0);
				printf(" used=");
				print_val(used + swap_used, mode, si_flag, 0);
				printf(" free=");
				print_val(free_mem + swap_free, mode, si_flag,
					  0);
			}
			printf("\n");
		} else if (wide) {
			...
			if (show_total) {
				printf("Total:  ");
				print_val(total + swap_total, mode, si_flag,
					  fw);
				print_val(used + swap_used, mode, si_flag,
					  fw + 1);
				print_val(free_mem + swap_free, mode, si_flag,
					  fw + 1);
				printf("\n");
			...
			if (show_lohi) {
				...
				print_val(lo_total - lo_free, mode, si_flag,
					  fw + 1);
				print_val(lo_free, mode, si_flag, fw + 1);
				printf("\n");
				printf("High:   ");
				print_val(hi_total, mode, si_flag, fw);
				print_val(hi_total - hi_free, mode, si_flag,
					  fw + 1);
		...
		count_done++;
		if (repeat_seconds > 0 &&
		    (repeat_count == 0 || count_done < repeat_count)) {
			struct timespec ts;

...
static char *sel_comms[MAX_SELECT];
static int n_sel_comms = 0;
...
static void build_stat(const procinfo_t *p, char *buf, size_t sz)
{
	...
	if (p->pid == p->sid)
		buf[n++] = 's'; /* session leader */
	if (p->nr_threads > 1)
		buf[n++] = 'l'; /* multi-threaded */
	if (p->nice < 0)
		buf[n++] = '<'; /* high priority  */
	else if (p->nice > 0)
		buf[n++] = 'N'; /* low priority   */
	if (p->tty_nr && p->pgid == p->sid)
		buf[n++] = '+'; /* foreground     */
	buf[n] = '\0';
}
...
static int add_col_by_name(const char *name, const char *custom_hdr)
{
	...
	add_col(sp->id, (custom_hdr != NULL) ? custom_hdr : sp->hdr, sp->width,
		sp->right);
	return 0;
}
...
static void format_value(const procinfo_t *p, int id, char *buf, size_t sz)
{
	...
	case COL_PID:
		...
		break;
	case COL_PPID:
		snprintf(buf, sz, "%d", p->ppid);
		break;
	case COL_PGID:
		snprintf(buf, sz, "%d", p->pgid);
		break;
	case COL_SID:
		snprintf(buf, sz, "%d", p->sid);
		break;
	case COL_TGID:
		snprintf(buf, sz, "%d", p->tgid);
		break;
	case COL_UID:
		snprintf(buf, sz, "%d", p->uid);
		break;
	case COL_GID:
		snprintf(buf, sz, "%d", p->gid);
		break;
	case COL_EUID:
		snprintf(buf, sz, "%d", p->euid);
		break;
	case COL_EGID:
	...
	case COL_PCPU: {
		...
		if (el > 0) {
			uint64_t cs =
				(p->utime_ticks + p->stime_ticks) * 100 / HZ;
	...
	case COL_C: {
		uint64_t el = (g_uptime > p->start_tick / HZ) ?
				      ...
				      0;
		if (el > 0) {
			uint64_t c = (p->utime_ticks + p->stime_ticks) * 99 /
				     (HZ * el);
			if (c > 99)
				c = 99;
	...
	case COL_PMEM:
		snprintf(buf, sz, "0.0");
		break; /* no total-RAM info */

	case COL_RSS:
		snprintf(buf, sz, "%lu", (unsigned long)(p->rss * 4));
		break;
	case COL_VSZ:
		snprintf(buf, sz, "%lu", (unsigned long)(p->vsz / 1024));
		break;
	case COL_SZ:
		snprintf(buf, sz, "%lu", (unsigned long)(p->vsz / 4096));
		break;

	case COL_PRI:
		snprintf(buf, sz, "%d", 80 - p->nice);
		break;
	case COL_NI:
		snprintf(buf, sz, "%d", p->nice);
		break;

	case COL_NLWP:
		snprintf(buf, sz, "%d", p->nr_threads);
		break;
	case COL_LWP:
		snprintf(buf, sz, "%d", p->pid);
		break;
	case COL_PSR:
		snprintf(buf, sz, "%d", p->on_cpu);
		break;
	case COL_CLS:
		snprintf(buf, sz, "RR");
		break;
...
static int comm_matches(const procinfo_t *p)
{
	for (int i = 0; i < n_sel_comms; i++)
		if (strcmp(p->comm, sel_comms[i]) == 0)
			return 1;
	return 0;
}
...
{
	int has_list = n_sel_pids || n_sel_ppids || n_sel_comms || n_sel_uids ||
		       n_sel_ruids || n_sel_gids || n_sel_rgids || n_sel_sids ||
		       n_sel_ttys;
	...
	if (!has_list && !has_simple && !opt_bsd_x) {
		...
		int ok;
		if (g_my_tty > 0)
			ok = (p->tty_nr == g_my_tty && p->sid == g_my_sid &&
			      !p->is_kernel);
		else
	...
	if (opt_all)
		match = 1;
	if (opt_all_tty && p->tty_nr > 0 && p->pid != p->sid)
		match = 1;
	if (opt_bsd_x && !opt_all_tty && !opt_all)
		match = 1;
	if (opt_all_tty && opt_bsd_x)
		match = 1;
	if (opt_running && (p->state == 0 || p->state == 1))
		match = 1;
	if (opt_this_tty && p->tty_nr == g_my_tty && g_my_tty > 0)
		match = 1;

	/* List selections (OR among all lists) */
	if (n_sel_pids && list_contains(sel_pids, n_sel_pids, p->pid))
		match = 1;
	if (n_sel_ppids && list_contains(sel_ppids, n_sel_ppids, p->ppid))
		match = 1;
	if (n_sel_comms && comm_matches(p))
		match = 1;
	if (n_sel_uids && list_contains(sel_uids, n_sel_uids, p->euid))
		match = 1;
	if (n_sel_ruids && list_contains(sel_ruids, n_sel_ruids, p->uid))
		match = 1;
	if (n_sel_gids && list_contains(sel_gids, n_sel_gids, p->egid))
		match = 1;
	if (n_sel_rgids && list_contains(sel_rgids, n_sel_rgids, p->gid))
		match = 1;
	if (n_sel_sids && list_contains(sel_sids, n_sel_sids, p->sid))
		match = 1;
	if (n_sel_ttys && list_contains(sel_ttys, n_sel_ttys, p->tty_nr))
		match = 1;
...
static int cmp_field(const procinfo_t *a, const procinfo_t *b, int id)
{
	...
	case COL_GID:
		return a->gid - b->gid;
	case COL_EUID:
		return a->euid - b->euid;
	case COL_EGID:
		return a->egid - b->egid;
	case COL_COMM:
	case COL_ARGS:
	case COL_FNAME:
	...
	case COL_C: {
		...
		uint64_t ca = ea ? (a->utime_ticks + a->stime_ticks) * 100 /
					      (HZ * ea) :
				   0;
		uint64_t cb = eb ? (b->utime_ticks + b->stime_ticks) * 100 /
					      (HZ * eb) :
				   0;
	...
	case COL_NLWP:
		return a->nr_threads - b->nr_threads;
	case COL_PSR:
...
static void tree_walk(procinfo_t *procs, int n, int *used, int parent_pid,
		      int depth)
{
	for (int i = 0; i < n; i++) {
		if (used[i])
...
static void build_forest(procinfo_t *procs, int n)
{
	...
	for (int i = 0; i < n; i++) {
		int found = 0;
		for (int j = 0; j < n; j++) {
			if (j != i && procs[j].pid == procs[i].ppid) {
				found = 1;
				break;
			}
		}
...
{
	if (!opt_show_header || opt_no_header)
		return;
...
static int parse_unix_opts(int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv,
		...
		case 'd':
			opt_all_tty = 1;
			break; /* all except session leaders */
		case 'f':
			opt_full = 1;
			break;
		case 'F':
		...
		case 'p':
		case OPT_PID:
			parse_int_list(optarg, sel_pids, &n_sel_pids);
			break;
		case 'q':
			parse_int_list(optarg, sel_pids, &n_sel_pids);
			break;
		case 'C':
			parse_comm_list(optarg);
			break;
		case 'G':
			parse_int_list(optarg, sel_rgids, &n_sel_rgids);
			break;
		case 'g':
			parse_int_list(optarg, sel_gids, &n_sel_gids);
			break;
		case 'u':
			parse_int_list(optarg, sel_uids, &n_sel_uids);
			break;
		case 'U':
			parse_int_list(optarg, sel_ruids, &n_sel_ruids);
			break;
		case 's':
			parse_int_list(optarg, sel_sids, &n_sel_sids);
			break;
		case 't':
			parse_int_list(optarg, sel_ttys, &n_sel_ttys);
			break;
		case 'o':
			opt_custom_fmt = 1;
		...
		case OPT_VERSION:
			...
			exit(0);
		case OPT_PPID:
			parse_int_list(optarg, sel_ppids, &n_sel_ppids);
			break;
		case OPT_COLS:
...
int main(int argc, char **argv)
{
	...
	/* ---- Filter ---- */
	procinfo_t *filtered =
		(procinfo_t *)malloc(MAX_PROCS * sizeof(procinfo_t));
	if (!filtered) {
		free(all);
		return 1;
	}

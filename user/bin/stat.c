...
static const char *file_type_str(uint32_t mode)
{
	switch (mode & S_IFMT) {
	case S_IFREG:
		return (0) ? "regular empty file" : "regular file";
	case S_IFDIR:
		return "directory";
	case S_IFCHR:
...
static void process_format_char(char spec, const char *filename,
				struct stat *st)
{
	char timebuf[64];
...
static void process_fs_format_char(char spec, const char *filename,
				   struct statfs *vfs)
{
	switch (spec) {
	case 'a':
...
static void print_format(const char *fmt, const char *filename, struct stat *st,
			 struct statfs *vfs, int is_fs, int add_newline)
{
		...
		if (*p == '\\') {
			...
			if (*p == '%') {
				...
				if (!*p)
					break;
				if (is_fs)
					process_fs_format_char(*p, filename,
							       vfs);
				else
...
static void print_default_file(const char *filename, struct stat *st)
{
	...
	printf("Device: %lxh/%lud\tInode: %-10lu  Links: %u\n",
	       ...
	       (unsigned long)st->st_ino, st->st_nlink);
	printf("Access: (%04o/%s)  Uid: (%5u/%8s)   Gid: (%5u/%8s)\n",
	       st->st_mode & 07777, perms, st->st_uid,
	       st->st_uid == 0 ? "root" : "UNKNOWN", st->st_gid,
	       st->st_gid == 0 ? "root" : "UNKNOWN");
...
static void print_default_fs(const char *filename, struct statfs *vfs)
{
	...
	printf("    ID: %-16lx Namelen: %-8lu Type: %s\n",
	       (unsigned long)vfs->f_fsid, (unsigned long)vfs->f_namelen,
...
static void print_terse_file(const char *filename, struct stat *st)
{
	printf("%s %lu %lu %lx %u %u %lx %lu %u %u %lx %lu %lu %lu %lu %lu\n",
	       ...
	       (unsigned long)st->st_blocks, (unsigned long)st->st_mode,
	       st->st_uid, st->st_gid, (unsigned long)st->st_dev,
	       (unsigned long)st->st_ino, st->st_nlink, 0u,
	       (unsigned long)st->st_rdev, (unsigned long)st->st_atime,
	       (unsigned long)st->st_mtime, (unsigned long)st->st_ctime,
...
static void print_terse_fs(const char *filename, struct statfs *vfs)
{
	printf("%s %lx %lu %lu %lu %lu %lu %lu %lu %lu %lu %s\n", filename,
	       (unsigned long)vfs->f_fsid, (unsigned long)vfs->f_namelen,
	       (unsigned long)vfs->f_type, (unsigned long)vfs->f_bsize,
	       (unsigned long)vfs->f_frsize, (unsigned long)vfs->f_blocks,
	       (unsigned long)vfs->f_bfree, (unsigned long)vfs->f_bavail,
	       (unsigned long)vfs->f_files, (unsigned long)vfs->f_ffree,
	       fstype_name(vfs->f_type));
}
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv, "Lfc:t", long_options, NULL)) !=
	       -1) {
		switch (c) {
		case 'L':
			opt_dereference = 1;

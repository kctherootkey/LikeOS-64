...
static int count_fd(int fd, struct wc_counts *cnt)
{
	...
	cnt->lines = cnt->words = cnt->chars = cnt->bytes = cnt->max_line_len =
		0;
...
static void compute_col_width(void)
{
	...
	if (total.bytes > m)
		m = total.bytes;
	if (total.max_line_len > m)
		m = total.max_line_len;
	...
	while ((c = getopt_long(argc, argv, "cmlwL", longopts, NULL)) != -1) {
		...
		case OPT_TOTAL:
			if (strcmp(optarg, "auto") == 0)
				total_mode = TOTAL_AUTO;
			else if (strcmp(optarg, "always") == 0)
				total_mode = TOTAL_ALWAYS;
			else if (strcmp(optarg, "only") == 0)
				total_mode = TOTAL_ONLY;
			else if (strcmp(optarg, "never") == 0)
				total_mode = TOTAL_NEVER;
			else {
				fprintf(stderr,

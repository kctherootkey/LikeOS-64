...
static void print_speed(const char *phase, unsigned long bytes, double secs)
{
	...
	if (kbps >= 1024.0)
		printf("  %s: %lu MB in %.2f s = %.2f MB/s\n", phase,
		       bytes / (1024UL * 1024UL), secs, kbps / 1024.0);
	else
		printf("  %s: %lu KB in %.2f s = %.2f KB/s\n", phase,
		       bytes / 1024UL, secs, kbps);
}
...
{
	double chunk_kbps =
		(chunk_bytes / 1024.0) / (chunk_secs > 0 ? chunk_secs : 1e-9);
	int pct = (int)((done * 100UL) / total);
	if (chunk_kbps >= 1024.0)
		printf("  [%s] %3d%% (%lu / %lu MB)  cur %.2f MB/s\n", phase,
		       pct, done / (1024UL * 1024UL), total / (1024UL * 1024UL),
		       chunk_kbps / 1024.0);
	else
...
int main(void)
{
	...
	while (written < TOTAL_BYTES) {
		...
		if (n < 0) {
			fprintf(stderr,
				"usbtest: write at offset %lu failed: %s\n",
		...
		if (n == 0) {
			fprintf(stderr,
				"usbtest: write returned 0 at offset %lu\n",
				written);
			break;
		}
	...
	while (read_total < TOTAL_BYTES) {
		...
		if (n < 0) {
			fprintf(stderr,
				"usbtest: read at offset %lu failed: %s\n",

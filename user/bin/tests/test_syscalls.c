...
static void test_write(void)
{
	...
	test_result(ret == (ssize_t)strlen(msg),
		    "write to stdout returns correct count");
...
static void test_brk(void)
{
	...
	if (new_brk != (void *)-1) {
		...
		ptr[2] = 'C';
		ptr[3] = '\0';
		test_result(ptr[0] == 'A' && ptr[1] == 'B',
			    "can write to brk-allocated memory");
	}
}
...
static void test_mmap(void)
{
	...
	if (ptr != MAP_FAILED) {
		...
		mem[1] = 'Y';
		mem[2] = 'Z';
		test_result(mem[0] == 'X' && mem[1] == 'Y',
			    "can write to mmap'd memory");
	...
	void *large = mmap(0, 16 * 4096, PROT_READ | PROT_WRITE,
			   MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
	test_result(large != MAP_FAILED, "mmap 64KB succeeds");

	// Invalid size should fail (or at least not crash)
	void *invalid =
		mmap(0, 0, PROT_READ, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);

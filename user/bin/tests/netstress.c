 ...
 * ------------------------------------------------------------------ */
#pragma pack(push,1)
typedef struct { uint8_t  ihl_ver, tos; uint16_t tot_len, id, frag_off;
                 uint8_t  ttl, proto;   uint16_t check;
                 uint32_t saddr, daddr; } iphdr_t;
typedef struct { uint16_t sport, dport, len, check; } udphdr_t;
typedef struct { uint16_t sport, dport; uint32_t seq, ack_seq;
                 uint8_t  doff_res, flags; uint16_t window, check, urg; } tcphdr_t;
typedef struct { uint8_t type, code; uint16_t check; uint32_t rest; } icmphdr_t;
#pragma pack(pop)
...
{
    if (g_icmp_fd < 0) return 1; /* raw icmp unavailable – skip check */
    ...
    for (;;) {
        ...
        if (n < 0) continue; /* EAGAIN (SO_RCVTIMEO fired) — re-check deadline */
        if (n < (ssize_t)(sizeof(iphdr_t) + sizeof(icmphdr_t))) return 0;
...
static void fill_ip(iphdr_t *ip, uint8_t proto, uint16_t tot_len,
                    uint32_t src, uint32_t dst, uint16_t frag_off)
{
...
{
    if (passed) { g_pass++; printf("  PASS  %s\n", name); }
...
static void test_a1_bogon_src(void)
{
    ...
    ic->type  = ICMP_ECHO;
    ic->check = ip_checksum(ic, sizeof(icmphdr_t));
    for (int i = 0; i < 20; i++) send_raw(pkt, sizeof(pkt));
    usleep(10000);
...
static void test_a4_fragment_flood(void)
{
    ...
    memset(pkt, 0, sizeof(pkt));
    fill_ip((iphdr_t *)pkt, 17, sizeof(pkt),
            g_src_ip, g_target_ip, IP_MF);
    /* Minimal UDP header */
...
static void test_a5_teardrop(void)
{
    ...
    fill_ip((iphdr_t *)f2, 17, sizeof(f2), g_src_ip, g_target_ip, 1 /* offset=8 */);
...
static void test_a6_zero_udp(void)
{
    ...
    for (int i = 0; i < 10; i++) send_raw(pkt, sizeof(pkt));
    usleep(10000);
...
static void test_a7_udp_sport_zero(void)
{
    ...
    udp->len   = htons(8 + 4);
    udp->check = udp_checksum(g_src_ip, g_target_ip,
                               (uint8_t *)udp, 8 + 4);
    for (int i = 0; i < 20; i++) send_raw(pkt, sizeof(pkt));
    usleep(10000);
...
static void build_tcp_pkt(uint8_t *out, int *out_len,
...
{
    uint8_t pkt[sizeof(iphdr_t) + sizeof(tcphdr_t)]; int len;
    build_tcp_pkt(pkt, &len, g_src_ip, g_target_ip,
                  11111, 80, 0, 0, 0 /*null*/, 65535);
    for (int i = 0; i < 20; i++) send_raw(pkt, len);
    usleep(10000);
...
static void test_b2_xmas_scan(void)
{
    uint8_t pkt[sizeof(iphdr_t) + sizeof(tcphdr_t)]; int len;
    build_tcp_pkt(pkt, &len, g_src_ip, g_target_ip,
                  11112, 80, 0, 0, TF_FIN|TF_PSH|TF_URG, 65535);
    for (int i = 0; i < 20; i++) send_raw(pkt, len);
    usleep(10000);
...
static void test_b3_syn_fin(void)
{
    uint8_t pkt[sizeof(iphdr_t) + sizeof(tcphdr_t)]; int len;
    build_tcp_pkt(pkt, &len, g_src_ip, g_target_ip,
                  11113, 80, 0, 0, TF_SYN|TF_FIN, 65535);
    for (int i = 0; i < 20; i++) send_raw(pkt, len);
    usleep(10000);
...
static void test_b4_land(void)
{
    ...
    build_tcp_pkt(pkt, &len, g_target_ip, g_target_ip,
                  80, 80, 1000, 0, TF_SYN, 65535);
...
static void test_b5_syn_flood(void)
{
    uint8_t pkt[sizeof(iphdr_t) + sizeof(tcphdr_t)]; int len;
    build_tcp_pkt(pkt, &len, g_src_ip, g_target_ip,
                  11114, 80, 0, 0, TF_SYN, 65535);
    for (int i = 0; i < 60; i++) {
        ((tcphdr_t *)(pkt + sizeof(iphdr_t)))->seq = htonl((uint32_t)i * 1000);
        send_raw(pkt, len);
    }
    usleep(30000);
...
static void test_b7_ack_storm(void)
{
    uint8_t pkt[sizeof(iphdr_t) + sizeof(tcphdr_t)]; int len;
    build_tcp_pkt(pkt, &len, g_src_ip, g_target_ip,
                  11116, 80, 100, 999999, TF_ACK, 65535);
    for (int i = 0; i < 30; i++) send_raw(pkt, len);
    usleep(10000);
...
static void test_c2_udp_sport0_flood(void)
{
    ...
    udp->dport = htons(53);
    udp->len   = htons(8 + 4);
    for (int i = 0; i < 40; i++) send_raw(pkt, sizeof(pkt));
    usleep(10000);
...
static void test_f1_halfopen_exhaustion(void)
{
    ...
    if (srv < 0) { report("F1 half-open exhaustion (no TCP)", 1); return; }
    int yes = 1;
    ...
    for (int i = 0; i < 50; i++) {
        build_tcp_pkt(pkt, &len, g_src_ip, g_target_ip,
                      (uint16_t)(20000 + i), port,
                      (uint32_t)(i * 1000), 0, TF_SYN, 65535);
        send_raw(pkt, len);
    }
    usleep(30000);
    close(srv);
...
static void test_f2_udp_hammer(void)
{
    ...
    udp->len   = htons(8 + 16);
    udp->check = udp_checksum(g_src_ip, g_target_ip, (uint8_t *)udp, 8 + 16);
    for (int i = 0; i < 40; i++) send_raw(pkt, sizeof(pkt));
    usleep(20000);
...
static void test_f3_blind_rst(void)
{
    ...
    uint8_t pkt[sizeof(iphdr_t) + sizeof(tcphdr_t)]; int len;
    build_tcp_pkt(pkt, &len, g_src_ip, g_target_ip,
                  0xABCD, srv_port, 0, 0, TF_RST, 65535);
    for (int i = 0; i < 10; i++) send_raw(pkt, len);
    usleep(10000);
...
int main(int argc, char *argv[])
{
     ...
     * targets).  Avoids UDP connect() which may block waiting for ARP. */
    {
        ...
        if (ioctl_s < 0) { perror("socket for SIOCGIFADDR"); return 1; }
        struct ifreq ifr;
        ...
        close(ioctl_s);
        if (g_src_ip == 0) {
            fprintf(stderr, "Cannot determine source IP for interface %s\n", ifname);
            return 1;
        }
    }
    ...
    int hdrincl = 1;
    setsockopt(g_raw_fd, IPPROTO_IP, 3 /*IP_HDRINCL*/, &hdrincl, sizeof(hdrincl));
    ...
    if (g_icmp_fd >= 0) close(g_icmp_fd);
    close(g_raw_fd);
    return g_fail > 0 ? 1 : 0;
}

	...
	while (*tok) {
		...
		*end = '\0';
		...
		if (strcmp(tok, "mode") == 0)
			...
			fprintf(stderr,
				PROGRAM_NAME ": unrecognized attribute: '%s'\n",
				tok);
			return -1;
		}
...
static void build_dest_path(char *dest, size_t dest_size, const char *dir,
			    const char *src)
{
...
static void copy_xattrs(const char *src, const char *dest)
{
	...
	for (char *name = names; name < names + lsz; name += strlen(name) + 1) {
		...
		if (vsz >= 0) {
			if (setxattr(dest, name, val, (size_t)vsz, 0) < 0 &&
			    errno != EOPNOTSUPP)
				fprintf(stderr,
					PROGRAM_NAME
					": warning: cannot set attribute '%s' "
					"on '%s': %s\n",
...
static void apply_attributes(const char *src, const char *dest,
			  ...
			  const struct stat *src_st)
{
	if (opt_attributes_only) {
		...
		if (fd < 0) {
			fprintf(stderr,
				PROGRAM_NAME ": cannot create '%s': %s\n", dest,
				strerror(errno));
			return -1;
		}
		close(fd);
	...
	if (sfd < 0) {
		fprintf(stderr,
			PROGRAM_NAME ": cannot open '%s' for reading: %s\n",
			src, strerror(errno));
		return -1;
	}
	...
	while ((nread = read(sfd, buf, sizeof(buf))) > 0) {
		ssize_t written = 0;
		while (written < nread) {
			ssize_t w = write(dfd, buf + written,
			  ...
			  const struct stat *src_st)
{
	...
	if (!d) {
		fprintf(stderr,
			PROGRAM_NAME ": cannot open directory '%s': %s\n", src,
			strerror(errno));
		return -1;
	}
	...
	while ((ent = readdir(d)) != NULL) {
		...
		snprintf(src_child, sizeof(src_child), "%s/%s", src,
			 ent->d_name);
		snprintf(dst_child, sizeof(dst_child), "%s/%s", dest,
			 ent->d_name);
...
static int do_copy(const char *src, const char *dest, int is_cmdline_arg)
{
	...
	if (opt_deref == DEREF_ALWAYS)
		...
		follow_links =
			!opt_recursive; /* Default: follow unless recursive */
	...
	if (S_ISREG(src_st.st_mode)) {
		...
		if (opt_make_link) {
			if (link(src, dest) < 0) {
				if (errno == EEXIST &&
				    (opt_force || opt_remove_destination)) {
					unlink(dest);
					if (link(src, dest) < 0) {
						fprintf(stderr,
							PROGRAM_NAME
							": cannot create hard link '%s': %s\n",
							dest, strerror(errno));
						return -1;
					}
				} else {
...
int main(int argc, char **argv)
{
	...
	static const struct option long_options[] = {
		...
		{ "remove-destination", no_argument, NULL,
		  OPT_REMOVE_DESTINATION },
	...
	int c;
	while ((c = getopt_long(argc, argv, "abdfilnprsuvxHLPRS:Tt:",
				long_options, NULL)) != -1) {
		switch (c) {
		...
		case 'b':
			...
			break;
		case OPT_BACKUP:
			if (!optarg || strcmp(optarg, "existing") == 0 ||
			    strcmp(optarg, "nil") == 0)
		...
		case 'p': /* same as --preserve=mode,ownership,timestamps */
			opt_preserve |= PRESERVE_MODE | PRESERVE_OWNERSHIP |
					PRESERVE_TIMESTAMPS;
			break;
		case OPT_PRESERVE: {
	...
	if (opt_target_directory) {
		...
		for (int i = optind; i < argc; i++) {
			char dest[4096];
			build_dest_path(dest, sizeof(dest),
					opt_target_directory, argv[i]);
		...
		if (!opt_no_target_directory && nargs == 2) {
			...
			if (stat(dest, &dst_st) == 0 &&
				...
				char full_dest[4096];
				build_dest_path(full_dest, sizeof(full_dest),
						dest, src);
		...
		for (int i = optind; i < argc - 1; i++) {
			char full_dest[4096];
			build_dest_path(full_dest, sizeof(full_dest), dest,
					argv[i]);

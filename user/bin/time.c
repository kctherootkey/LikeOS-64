...
static void print_format(FILE *out, const char *fmt, const char *command_line,
...
int main(int argc, char *argv[])
{
	...
	int opt;
	while ((opt = getopt_long(argc, argv, "+af:o:pqvVh", long_options,
				  NULL)) != -1) {
		switch (opt) {
		case 'a':
			append_mode = 1;
	...
	for (int i = optind; i < argc; i++) {
		if (i > optind)
			strcat(command_line, " ");
		strncat(command_line, argv[i],
			sizeof(command_line) - strlen(command_line) - 2);
	}
	...
	if (!quiet_mode) {
		...
		print_format(out, format, command_line, real_secs, user_secs,
			     sys_secs, &ru, exit_status);

...
static const char *level_names[] = { "emerg", "alert",  "crit", "err",
				     "warn",  "notice", "info", "debug" };

static const char *facility_names[] = {
	"kern",   "user",   "mail",   "daemon", "auth",     "syslog",
	"lpr",    "news",   "uucp",   "cron",   "authpriv", "ftp",
	"res0",   "res1",   "res2",   "res3",   "local0",   "local1",
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv,
				  "CcDdeEF:f:HhkL::l:n:Pprs:STtuVwx",
				  long_options, NULL)) != -1) {
		switch (opt) {
		case 'C':
			do_clear = 1;
		...
		case 'c':
			do_read_clear = 1;
			break;
		case 'D': /* console-off: not supported */
			break;
		case 'd':
			show_delta = 1;
			break;
		case 'E': /* console-on: not supported */
			break;
		case 'e':
			show_reltime = 1;
			break;
		case 'F':
			...
			break;
		case 'f':
			facility_mask = parse_facilities(optarg);
			break;
		case 'H':
			human_mode = 1;
			show_reltime = 1;
		...
		case 'k':
			kernel_only = 1;
			break;
		case 'L': /* color - silently accept but ignore */
			break;
		case 'l':
			level_mask = parse_levels(optarg);
			break;
		case 'n': /* console-level: not supported */
			break;
		case OPT_NOESCAPE:
			noescape = 1;
			break;
		case 'P': /* nopager - nothing to do */
			break;
		case 'p':
			force_prefix = 1;
			break;
		case 'r':
			raw_mode = 1;
			break;
		case 'S': /* syslog mode: same as default */
			break;
		case 's':
			bufsize = atoi(optarg);
			if (bufsize <= 0)
		...
		case OPT_TIMEFMT:
			time_format = optarg;
			if (strcmp(optarg, "delta") == 0)
				show_delta = 1;
			else if (strcmp(optarg, "reltime") == 0)
				show_reltime = 1;
			else if (strcmp(optarg, "ctime") == 0)
				show_ctime = 1;
			else if (strcmp(optarg, "notime") == 0)
				no_time = 1;
			else if (strcmp(optarg, "iso") == 0)
				show_ctime = 1; /* close enough */
			break;
		case 'u':
			userspace_only = 1;
			break;
	...
	if (input_file) {
		...
		int action = do_read_clear ? SYSLOG_ACTION_READ_CLEAR :
	...
	while (p < buf_end) {
		...
		if (line_len > 0) {
			...
			if (kernel_only && entry.facility != LOG_KERN &&
			    entry.priority >= 0) {
				goto next_line;
			}
			if (userspace_only && entry.facility == LOG_KERN &&
			    entry.priority >= 0) {
				goto next_line;
			}
			...
			if (raw_mode) {
				...
				if (entry.msg && entry.msg_len > 0) {
					fwrite(entry.msg, 1, entry.msg_len,
					       stdout);
				}
				putchar('\n');
			}
	...
	if (follow_mode) {
		...
		while (1) {
			...
			if (cur_size > last_size) {
				...
				if (!newbuf)
					break;
				int n = klogctl(SYSLOG_ACTION_READ_ALL, newbuf,
						bufsize - 1);
				if (n > last_size) {
					/* Print only new data */

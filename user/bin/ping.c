...
static void print_stats(void)
{
	...
	if (g_received > 0) {
		...
		printf("rtt min/avg/max/mdev = %u.%03u/%u.%03u/%u.%03u/%u.%03u ms\n",
		       g_min_ms / 1000, g_min_ms % 1000, avg_ms / 1000,
		       avg_ms % 1000, g_max_ms / 1000, g_max_ms % 1000,
		       mdev / 1000, mdev % 1000);
	}
}
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt(
			argc, argv,
			"46aAbBc:dDfhi:I:l:Lm:M:nOp:qQ:rRs:S:t:UvVw:W:")) !=
	       -1) {
		switch (opt) {
		case '4': /* IPv4 - default */
			break;
		...
		case 'A':
			adaptive = 1;
			break;
		case 'b': /* allow broadcast - accepted */
			break;
		case 'B': /* sticky source - accepted */
			break;
		case 'c':
			count = atoi(optarg);
			break;
		case 'd': /* SO_DEBUG - accepted but no effect */
			break;
		case 'D':
			print_timestamp = 1;
			break;
		...
		case 'i':
			...
			break;
		case 'I': /* source address/interface - accepted */
			break;
		case 'l':
			preload = atoi(optarg);
			break;
		case 'L': /* suppress loopback - accepted */
			break;
		case 'm': /* mark - accepted but no effect */
			break;
		case 'M': /* pmtudisc - accepted but no effect */
			break;
		case 'n':
			numeric = 1;
			break;
		case 'O':
			report_outstanding = 1;
			break;
		case 'p': /* pattern - accepted but no effect */
			break;
		case 'q':
			quiet = 1;
			break;
		case 'Q': /* TOS - accepted but no effect */
			break;
		case 'r': /* bypass routing - accepted */
			break;
		case 'R':
			record_route = 1;
			break;
		case 's':
			...
			break;
		case 'S': /* sndbuf - accepted but no effect */
			break;
		case 't':
			ttl = atoi(optarg);
			break;
		case 'U': /* user-to-user latency - accepted */
			break;
		case 'v':
			verbose = 1;
			break;
	...
	printf("PING %s (%s) %d(%d) bytes of data.\n", dest, ip_to_str(dst_ip),
	       packetsize, total_size + 20);
	...
	for (int seq = seq_start; count < 0 || (seq - seq_start) < count;
	     seq++) {
		/* Check deadline */
		if (deadline > 0) {
			/* Approximate: we rely on the loop timing */
			int elapsed = (seq - seq_start) *
		...
		int ret = raw_send(RAW_SEND_ICMP_ECHO, dst_ip, id_seq,
				   (uint32_t)ttl);
		if (ret < 0) {
			if (!quiet)
				fprintf(stderr,
		...
		if (ret == 0) {
			...
			uint16_t rseq =
				(uint16_t)((reply.seq >> 8) | (reply.seq << 8));

			/* Measure RTT in the waiting thread context. */
			long rtt_us =
				(tv_recv.tv_sec - tv_send.tv_sec) * 1000000L +
			...
			if (g_min_ms > rtt_ms_10)
				g_min_ms = rtt_ms_10;
			if (g_max_ms < rtt_ms_10)
				g_max_ms = rtt_ms_10;
			...
			if (flood) {
				...
				if (print_timestamp) {
					...
					printf("[%ld.%06ld] ", tv_now.tv_sec,
					       tv_now.tv_usec);
				}
				if (type == 0) {
					printf("%d bytes from %s: icmp_seq=%u ttl=%u time=%lu.%03lu ms\n",
					       packetsize + 8, ip_to_str(src),
					       rseq, recv_ttl,

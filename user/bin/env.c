...
static void do_list_signal(int signo, const char *desc)
{
	...
	if (sigaction(signo, NULL, &sa) == 0) {
		if (sa.sa_handler == SIG_IGN) {
			/* Find signal name */
			const char *name = "???";
			for (const struct sigmap *s = signal_table; s->name;
			     s++) {
				if (s->num == signo) {
					name = s->name;
					break;
...
int main(int argc, char *argv[])
{
	...
	int c;
	optind = 1;
	while ((c = getopt_long(argc, argv, "+i0u:C:S:v", long_options,
				NULL)) != -1) {
		switch (c) {
		case 'i':
			opt_ignore_env = 1;
	...
	while (cmd_start < argc) {
		...
		if (!eq)
			break; /* Not a NAME=VALUE, must be COMMAND */
		/* Set the variable */
		*eq = '\0';
	...
	if (opt_chdir) {
		...
		if (chdir(opt_chdir) != 0) {
			fprintf(stderr,
				"%s: cannot change directory to '%s': %s\n",

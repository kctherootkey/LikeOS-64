...
static void print_string(const char *filename, const char *str, int len,
...
static int process_file_32(FILE *fp, const char *filename, int big_endian)
{
	...
	while (fread(quad, 1, 4, fp) == 4) {
		unsigned long ch;
		if (big_endian)
			ch = ((unsigned long)quad[0] << 24) |
			     ((unsigned long)quad[1] << 16) |
			     ((unsigned long)quad[2] << 8) | quad[3];
		else
			ch = quad[0] | ((unsigned long)quad[1] << 8) |
			     ((unsigned long)quad[2] << 16) |
			     ((unsigned long)quad[3] << 24);
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv, "adfn:ot:e:wvVs:", long_options,
				NULL)) != -1) {
		switch (c) {
		case 'a':
		...
		case 'n':
			...
			if (opt_min_len < 1) {
				fprintf(stderr,
					"%s: invalid minimum string length '%s'\n",
					PROGRAM_NAME, optarg);
				return EXIT_FAILURE;
			}
			break;
		...
		case 'e':
			...
			case 'l':
				...
				break;
			case 'B':
				opt_encoding = ENC_B_CAP;
				break;
			case 'L':
				opt_encoding = ENC_L_CAP;
				break;
			default:
	...
	if (optind >= argc) {
		...
		for (int i = optind; i < argc; i++) {
			if (strcmp(argv[i], "-") == 0) {
				if (process_file(stdin, "{standard input}") !=
				    0)
					ret = 1;
			} else {

...
static const char *man_paths[] = {
	...
	"/usr/share/man/man7", "/usr/share/man/man8", NULL
};
...
static int find_manpage(const char *name, int section, char *path,
			size_t pathsz)
{
	char fname[256];
	...
	if (section > 0) {
		...
		snprintf(path, pathsz, "/usr/share/man/man%d/%s", section,
			 fname);
		if (access(path, R_OK) == 0)
			return 0;

		/* Try without section suffix in filename */
		snprintf(path, pathsz, "/usr/share/man/man%d/%s.1", section,
			 name);
		if (access(path, R_OK) == 0)
			return 0;
...
static manpage_t *load_manpage(const char *path)
{
	...
	while (fgets(buf, MAX_LINE, fp)) {
		...
		if (mp->nlines >= mp->capacity) {
			mp->capacity *= 2;
			char **new_lines =
				malloc(mp->capacity * sizeof(char *));
			if (!new_lines)
				break;
			memcpy(new_lines, mp->lines,
			       mp->nlines * sizeof(char *));
			free(mp->lines);
			mp->lines = new_lines;
		}
...
static void pager(manpage_t *mp, const char *title)
{
	...
	while (!done) {
		...
		int lines_shown = 0;
		for (int i = top_line;
		     i < mp->nlines && lines_shown < page_size;
		     i++, lines_shown++) {
			...
			if (search_active && search_str[0]) {
				...
				if (match) {
					...
					printf("\033[7m");
					fwrite(match, 1, strlen(search_str),
					       stdout);
					printf("\033[0m");
					/* Print after match */
					int post_start =
						pre + (int)strlen(search_str);
					if (post_start < len)
		...
		printf("\033[?25h");
		int pct = (mp->nlines > 0) ?
				  ((top_line + page_size) * 100 / mp->nlines) :
				  100;
		if (pct > 100)
			pct = 100;
		...
		case '/': { /* Search forward */
			...
			if (search_str[0]) {
				...
				for (int i = top_line + 1; i < mp->nlines;
				     i++) {
					if (my_strcasestr(mp->lines[i],
							  search_str)) {
						top_line = i;
						break;
					}
...
static void do_apropos(const char *keyword)
{
	...
	for (int pi = 0; man_paths[pi]; pi++) {
		...
		while ((de = readdir(dir)) != NULL) {
			...
			char path[MAX_PATH];
			snprintf(path, sizeof(path), "%s/%s", man_paths[pi],
				 de->d_name);
	...
	for (int pi = 0; man_paths[pi]; pi++) {
		...
		while ((de = readdir(dir)) != NULL) {
			...
			if (strncmp(de->d_name, name, nlen) != 0)
				continue;
			if (de->d_name[nlen] != '.' && de->d_name[nlen] != '\0')
				continue;
...
int main(int argc, char *argv[])
{
	...
	int opt;
	while ((opt = getopt_long(argc, argv, "fkawM:S:7E:Vh", long_options,
				  NULL)) != -1) {
		switch (opt) {
		case 'f':
			opt_whatis = 1;
		...
		case 'k':
			opt_apropos = 1;
			break;
		case 'a': /* show all matches - not yet implemented */
			break;
		case 'w':
			opt_where = 1;
			break;
		case 'M': /* Custom manpath - ignored for now */
			break;
		case 'S': /* Sections - ignored for now */
			break;
		case '7': /* ASCII mode - default */
			break;
		case 'E': /* Encoding - ignored */
			break;
		case 'V':
			printf("%s\n", MAN_VERSION);
			return 0;
	...
	for (int i = optind; i < argc; i++) {
		...
		if (isatty(STDOUT_FILENO)) {
			detect_term_size();
			char title[256];
			snprintf(title, sizeof(title), "%s(%d)", argv[i],
				 section > 0 ? section : 1);
			pager(mp, title);
		} else {
			batch_output(mp);

 ...
 */
static void extract_key(const char *line, const sort_key_t *key, char *out,
			int outsz)
{
...
static int compare_strings(const char *a, const char *b, int b_flag, int d_flag,
			   int f_flag, int g_flag, int i_flag, int M_flag,
			   int h_flag, int n_flag, int r_flag, int V_flag)
{
	if (b_flag) {
		a = skip_blanks(a);
		b = skip_blanks(b);
	...
	if (g_flag) {
		...
		while (*pa && *pb) {
			...
			if (d_flag) {
				while (*pa && !isalnum(ca) && ca != ' ' &&
				       ca != '\t') {
					pa++;
					ca = (unsigned char)*pa;
				}
				while (*pb && !isalnum(cb) && cb != ' ' &&
				       cb != '\t') {
					pb++;
					cb = (unsigned char)*pb;
				}
...
static int compare_lines(const void *p1, const void *p2)
{
	...
	if (nkeys > 0) {
		...
		for (int k = 0; k < nkeys; k++) {
			...
			int b_f = keys[k].has_flags ? keys[k].b :
						      opt_ignore_leading_blanks;
			int d_f = keys[k].has_flags ? keys[k].d :
						      opt_dictionary_order;
			int f_f =
				keys[k].has_flags ? keys[k].f : opt_ignore_case;
			int g_f = keys[k].has_flags ? keys[k].g :
						      opt_general_numeric;
			int i_f = keys[k].has_flags ? keys[k].i :
						      opt_ignore_nonprinting;
			int M_f =
				keys[k].has_flags ? keys[k].M : opt_month_sort;
			int h_f = keys[k].has_flags ? keys[k].h :
						      opt_human_numeric;
			int n_f = keys[k].has_flags ? keys[k].n : opt_numeric;
			int r_f = keys[k].has_flags ? keys[k].r : opt_reverse;
			int V_f = keys[k].has_flags ? keys[k].V :
						      opt_version_sort;

			int cmp = compare_strings(ka, kb, b_f, d_f, f_f, g_f,
						  i_f, M_f, h_f, n_f, r_f, V_f);
			if (cmp != 0)
				return cmp;
		}
	...
	return compare_strings(a, b, opt_ignore_leading_blanks,
			       opt_dictionary_order, opt_ignore_case,
			       opt_general_numeric, opt_ignore_nonprinting,
			       opt_month_sort, opt_human_numeric, opt_numeric,
			       opt_reverse, opt_version_sort);
}
...
static int parse_key(const char *spec)
{
	...
	if (comma) {
		...
		if (flen >= (int)sizeof(flags))
			flen = (int)sizeof(flags) - 1;
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, "bdfgiMhnRrVcCmo:st:k:uz",
		...
		switch (opt) {
		case 'b':
			opt_ignore_leading_blanks = 1;
			break;
		case 'd':
			opt_dictionary_order = 1;
			break;
		case 'f':
			opt_ignore_case = 1;
			break;
		case 'g':
			opt_general_numeric = 1;
			break;
		case 'i':
			opt_ignore_nonprinting = 1;
			break;
		case 'M':
			opt_month_sort = 1;
			break;
	...
	if (optind >= argc ||
	    (optind + 1 == argc && strcmp(argv[optind], "-") == 0)) {
	...
	for (int i = 0; i < nlines; i++) {
		if (opt_unique && i > 0 &&
		    compare_lines(&lines[i - 1], &lines[i]) == 0)
			continue;
		fputs(lines[i], out);
		fputc(delim, out);
	}

...
static struct expr *parse_primary(void)
{
	...
	if (strcmp(tok, "!") == 0 || strcmp(tok, "-not") == 0) {
		...
		if (!child) {
			fprintf(stderr,
				"find: expected expression after '%s'\n", tok);
			exit(1);
		}
	...
	if (strcmp(tok, "-iname") == 0) {
		...
		const char *pat = consume_arg();
		if (!pat) {
			fprintf(stderr, "find: -iname requires argument\n");
			exit(1);
		}
		struct expr *e = alloc_expr();
		e->type = E_INAME;
	...
	if (strcmp(tok, "-path") == 0 || strcmp(tok, "-wholename") == 0) {
		...
		const char *pat = consume_arg();
		if (!pat) {
			fprintf(stderr, "find: -path requires argument\n");
			exit(1);
		}
		struct expr *e = alloc_expr();
		e->type = E_PATH;
	...
	if (strcmp(tok, "-ipath") == 0 || strcmp(tok, "-iwholename") == 0) {
		...
		const char *pat = consume_arg();
		if (!pat) {
			fprintf(stderr, "find: -ipath requires argument\n");
			exit(1);
		}
		struct expr *e = alloc_expr();
		e->type = E_IPATH;
	...
	if (strcmp(tok, "-type") == 0) {
		...
		if (!arg) {
			fprintf(stderr, "find: -type requires argument\n");
			exit(1);
		}
		int t = parse_type(arg);
		if (t < 0) {
			fprintf(stderr, "find: unknown type '%s'\n", arg);
			exit(1);
		}
		struct expr *e = alloc_expr();
		e->type = E_TYPE;
	...
	if (strcmp(tok, "-size") == 0) {
		...
		const char *arg = consume_arg();
		if (!arg) {
			fprintf(stderr, "find: -size requires argument\n");
			exit(1);
		}
		struct expr *e = alloc_expr();
		e->type = E_SIZE;
	...
	if (strcmp(tok, "-perm") == 0) {
		...
		const char *arg = consume_arg();
		if (!arg) {
			fprintf(stderr, "find: -perm requires argument\n");
			exit(1);
		}
		struct expr *e = alloc_expr();
		e->type = E_PERM;
	...
	if (strcmp(tok, "-newer") == 0) {
		...
		const char *arg = consume_arg();
		if (!arg) {
			fprintf(stderr, "find: -newer requires argument\n");
			exit(1);
		}
		struct stat st;
		if (stat(arg, &st) != 0) {
	...
	if (strcmp(tok, "-mtime") == 0 || strcmp(tok, "-atime") == 0 ||
		...
		const char *arg = consume_arg();
		if (!arg) {
			fprintf(stderr, "find: %s requires argument\n", tok);
			exit(1);
		}
		struct expr *e = alloc_expr();
		e->type = et;
	...
	if (strcmp(tok, "-links") == 0) {
		...
		const char *arg = consume_arg();
		if (!arg) {
			fprintf(stderr, "find: -links requires argument\n");
			exit(1);
		}
		struct expr *e = alloc_expr();
		e->type = E_LINKS;
	...
	if (strcmp(tok, "-printf") == 0) {
		...
		if (!fmt) {
			fprintf(stderr, "find: -printf requires argument\n");
			exit(1);
		}
		g_has_action = 1;
		struct expr *e = alloc_expr();
...
static struct expr *parse_or(void)
{
	...
	while (1) {
		...
		if (!right) {
			fprintf(stderr,
				"find: expected expression after '-o'\n");
			exit(1);
		}
...
{
	if (!e)
		return 1; /* No expression => match (default -print) */
	...
	case E_PERM:
		switch (e->perm_mode) {
		case 0:
			return (st->st_mode & 07777) == e->perm_val;
		case 1:
			return (st->st_mode & e->perm_val) == e->perm_val;
		case 2:
			return (st->st_mode & e->perm_val) != 0;
		}
		return 0;
	...
	case E_LS: {
		...
		printf("%8lu %s %u %u %8lu %s\n", 0UL, mb, st->st_uid,
		       st->st_gid, (unsigned long)st->st_size, path);
		return 1;
	}
	case E_PRUNE:
		*prune = 1;
...
int main(int argc, char **argv)
{
	...
	for (int i = 1; i < argc; i++) {
		...
		if (strcmp(argv[i], "-maxdepth") == 0) {
			if (i + 1 >= argc) {
				fprintf(stderr,
					"find: -maxdepth requires argument\n");
				return 1;
			}
		...
		if (strcmp(argv[i], "-mindepth") == 0) {
			if (i + 1 >= argc) {
				fprintf(stderr,
					"find: -mindepth requires argument\n");
				return 1;
			}

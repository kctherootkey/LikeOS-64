...
static void format_date(const char *fmt, const struct tm *tm, time_t epoch,
			long nsec)
{
	char out[4096];
	int pos = 0;
	...
	for (int i = 0; fmt[i] && pos < max; i++) {
		...
		case 'b':
		case 'h': {
			const char *mons[] = { "Jan", "Feb", "Mar", "Apr",
					       "May", "Jun", "Jul", "Aug",
					       "Sep", "Oct", "Nov", "Dec" };
		...
		case 'c':
			tmplen = snprintf(
				tmp, sizeof(tmp), "%s %s %2d %02d:%02d:%02d %d",
				(const char *[]){ "Sun", "Mon", "Tue", "Wed",
						  "Thu", "Fri",
						  "Sat" }[tm->tm_wday % 7],
				(const char *[]){ "Jan", "Feb", "Mar", "Apr",
						  "May", "Jun", "Jul", "Aug",
						  "Sep", "Oct", "Nov",
						  "Dec" }[tm->tm_mon % 12],
			...
			break;
		case 'C':
			tmplen = snprintf(tmp, sizeof(tmp), "%02d",
					  (tm->tm_year + 1900) / 100);
			break;
		case 'd':
			tmplen =
			...
			break;
		case 'D':
			tmplen = snprintf(tmp, sizeof(tmp), "%02d/%02d/%02d",
		...
		case 'e':
			...
			break;
		case 'F':
			tmplen = snprintf(tmp, sizeof(tmp), "%04d-%02d-%02d",
		...
		case 'V': {
			...
			if (iso_jan1 <= 4)
				week++;
			if (week == 0)
				week = 52; /* belongs to previous year */
			int iso_year = tm->tm_year + 1900;
			if (week >= 52 && tm->tm_mon == 0 && tm->tm_mday <= 3)
				iso_year--;
			if (week == 1 && tm->tm_mon == 11 && tm->tm_mday >= 29)
				iso_year++;
			if (fmt[i] == 'V')
				tmplen = snprintf(tmp, sizeof(tmp), "%02d",
						  week);
				...
				tmplen = snprintf(tmp, sizeof(tmp), "%04d",
						  iso_year);
			break;
		}
		case 'H':
		...
		case 'j':
			tmplen = snprintf(tmp, sizeof(tmp), "%03d",
					  tm->tm_yday + 1);
			break;
		case 'k':
		...
		case 'P':
			...
			break;
		case 'q':
			tmplen = snprintf(tmp, sizeof(tmp), "%d",
					  tm->tm_mon / 3 + 1);
			break;
		case 'r': {
			int h = tm->tm_hour % 12;
		...
		case 'R':
			tmplen = snprintf(tmp, sizeof(tmp), "%02d:%02d",
...
static int parse_datetime(const char *str, int *y, int *mo, int *d, int *h,
			  int *mi, int *s)
{
...
static int parse_date_string(const char *str, struct tm *out_tm,
			     time_t *out_epoch)
{
	/* @epoch format */
	if (str[0] == '@') {
...
int main(int argc, char *argv[])
{
	...
	if (user_format) {
		...
		printf("%s %s %2d %02d:%02d:%02d UTC %04d\n",
		       days[tm_val.tm_wday % 7], mons[tm_val.tm_mon % 12],
		       tm_val.tm_mday, tm_val.tm_hour, tm_val.tm_min,
		       tm_val.tm_sec, tm_val.tm_year + 1900);
	}

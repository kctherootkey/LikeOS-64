...
static long parse_time(const char *s)
{
	...
	/* "hh:mm" absolute wall-clock time */
	{
		...
		if (target <= now) {
			tm_target.tm_mday += 1;
			target = mktime(
				&tm_target); /* mktime normalises overflow */
		}
...
int main(int argc, char *argv[])
{
	...
	for (int i = 1; i < argc; i++) {
		if (strcmp(argv[i], "--help") == 0) {
			...
			if (time_arg == NULL) {
				...
				if (wall_off > 0 &&
				    wall_off < (int)sizeof(wall_msg) - 1)
					wall_msg[wall_off++] = ' ';
				size_t len = strlen(argv[i]);
				if (wall_off + (int)len <
				    (int)sizeof(wall_msg) - 1) {
					memcpy(wall_msg + wall_off, argv[i],
					       len);
					wall_off += (int)len;
				}
			}
	...
	if (delay_secs > 0) {
		...
		if (!no_wall) {
			...
			char timebuf[64];
			strftime(timebuf, sizeof(timebuf), "%H:%M:%S UTC",
				 &tm_when);
		...
		while (delay_secs > 0) {
			unsigned int chunk =
				delay_secs > 60 ? 60 : (unsigned int)delay_secs;
			sleep(chunk);
			delay_secs -= chunk;

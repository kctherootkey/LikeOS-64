	...
	if (is_reverse) {
		...
		if (ret == 0 && qbuf.response_len >= 12) {
			...
			for (int i = 0; i < ancount && pos < pktlen; i++) {
				char rname[256];
				pos = decode_dns_name(pkt, pktlen, pos, rname,
						      sizeof(rname));
				if (pos + 10 > pktlen)
					break;
				uint16_t rtype = (pkt[pos] << 8) | pkt[pos + 1];
				uint16_t rdlen =
					(pkt[pos + 8] << 8) | pkt[pos + 9];
				pos += 10;
				if (rtype == DNS_TYPE_PTR &&
				    pos + rdlen <= pktlen) {
					char ptrname[256];
					decode_dns_name(pkt, pktlen, pos,
							ptrname,
							sizeof(ptrname));
					if (verbose) {
	...
	if (ret == 0 && qbuf.response_len >= 12) {
		...
		for (int i = 0; i < ancount && pos < pktlen; i++) {
			...
			if (rtype == DNS_TYPE_A && rdlen == 4) {
				...
				char cname[256];
				decode_dns_name(pkt, pktlen, pos, cname,
						sizeof(cname));
				if (verbose)

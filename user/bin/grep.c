...
static int parse_posix_class(const char **pp, unsigned char bits[32])
{
	...
	*pp = p;
	...
	for (int c = 0; c < 256; c++) {
		int match = 0;
		if (nlen == 5 && strncmp(name, "alnum", 5) == 0)
			match = isalnum(c);
		else if (nlen == 5 && strncmp(name, "alpha", 5) == 0)
			match = isalpha(c);
		else if (nlen == 5 && strncmp(name, "blank", 5) == 0)
			match = (c == ' ' || c == '\t');
		else if (nlen == 5 && strncmp(name, "cntrl", 5) == 0)
			match = iscntrl(c);
		else if (nlen == 5 && strncmp(name, "digit", 5) == 0)
			match = isdigit(c);
		else if (nlen == 5 && strncmp(name, "graph", 5) == 0)
			match = isgraph(c);
		else if (nlen == 5 && strncmp(name, "lower", 5) == 0)
			match = islower(c);
		else if (nlen == 5 && strncmp(name, "print", 5) == 0)
			match = isprint(c);
		else if (nlen == 5 && strncmp(name, "punct", 5) == 0)
			match = ispunct(c);
		else if (nlen == 5 && strncmp(name, "space", 5) == 0)
			match = isspace(c);
		else if (nlen == 5 && strncmp(name, "upper", 5) == 0)
			match = isupper(c);
		else if (nlen == 6 && strncmp(name, "xdigit", 6) == 0)
			match = isxdigit(c);
		if (match)
			class_set(bits, c);
	}
...
static int re_compile(regex_compiled_t *re, const char *pattern, int extended,
		      int icase)
{
	re->count = 0;
	re->ngroups = 0;
	...
	while (*p) {
		...
		if (*p == '\\') {
			...
			if (extended) {
				...
				if (*p >= '1' && *p <= '9') {
					...
					n->type = RE_WORD_BOUNDARY;
				} else if (*p == 'B') {
					n->type = RE_NOT_WORD_BOUNDARY;
				} else if (*p == '<') {
					n->type = RE_WORD_START;
				} else if (*p == '>') {
					n->type = RE_WORD_END;
				} else if (*p == 'w') {
					n->type = RE_WORD_CHAR;
				} else if (*p == 'W') {
					n->type = RE_NOT_WORD_CHAR;
				} else if (*p == 'n') {
					n->type = RE_LITERAL;
					n->ch = '\n';
				} else if (*p == 't') {
		...
		if (*p == '^') {
			n->type = RE_ANCHOR_BOL;
			re->count++;
			p++;
			continue;
		}
		if (*p == '$') {
			n->type = RE_ANCHOR_EOL;
			re->count++;
			p++;
			continue;
		}
		...
		if (*p == '[') {
			...
			while (*p && (*p != ']' || first)) {
				...
				if (*p == '-' && p[1] && p[1] != ']') {
					p++;
					int ch_end = (unsigned char)*p;
					p++;
					for (int c = ch_start; c <= ch_end;
					     c++) {
...
static int atom_match(const re_node_t *n, unsigned char c, int icase)
{
	...
	switch (n->type) {
	case RE_LITERAL:
		return lc == (unsigned char)n->ch;
	case RE_DOT:
		return c != '\n';
	case RE_CLASS: {
		if (class_test(n->class_bits, c))
			return 1;
		if (icase) {
			if (c >= 'A' && c <= 'Z')
				return class_test(n->class_bits, c + 32);
			if (c >= 'a' && c <= 'z')
				return class_test(n->class_bits, c - 32);
		}
		return 0;
	...
	case RE_NCLASS: {
		...
		if (icase) {
			if (c >= 'A' && c <= 'Z')
				ok = ok && !class_test(n->class_bits, c + 32);
			else if (c >= 'a' && c <= 'z')
				ok = ok && !class_test(n->class_bits, c - 32);
		}
		return ok;
 ...
 */
static int try_match(const regex_compiled_t *re, int ni, const char *text,
		     const char *sol, const char *eol, const char **end_pos,
		     int icase)
{
restart:
	...
	if (next < re->count) {
		...
		if (qt == RE_STAR || qt == RE_PLUS || qt == RE_QUEST ||
		    qt == RE_REPEAT) {
			...
			while (count < rmax && t < eol) {
				if (!atom_match(n, (unsigned char)*t, icase))
					break;
				t++;
				count++;
			}
			for (int i = count; i >= rmin; i--) {
				if (try_match(re, next + 1, text + i, sol, eol,
					      end_pos, icase))
					return 1;
			}
			return 0;
	...
	if (n->type == RE_WORD_BOUNDARY || n->type == RE_NOT_WORD_BOUNDARY ||
		...
		int pw = (text > sol) ? is_word_char((unsigned char)text[-1]) :
					0;
		int cw = (text < eol) ? is_word_char((unsigned char)*text) : 0;
		int at_b = (pw != cw);
		if (n->type == RE_WORD_BOUNDARY && !at_b)
			return 0;
		if (n->type == RE_NOT_WORD_BOUNDARY && at_b)
			return 0;
		if (n->type == RE_WORD_START && (pw || !cw))
			return 0;
		if (n->type == RE_WORD_END && (!pw || cw))
			return 0;
		ni = next;
		goto restart;
	}
	...
	ni = next;
	text = text + 1;
	goto restart; /* tail-call elimination */
}
			...
			const char **ms, const char **me, int icase)
{
	...
	for (int i = 0; i < re->count; i++) {
		if (re->nodes[i].type == RE_GROUP_START)
			depth++;
		else if (re->nodes[i].type == RE_GROUP_END)
			depth--;
	...
	for (const char *t = line; t <= eol; t++) {
		for (int a = 0; a < nalt; a++) {
			const char *ep;
			if (try_match(re, alt_starts[a], t, line, eol, &ep,
				      icase)) {
				*ms = t;
				*me = ep;
				return 1;
...
static regex_compiled_t *compiled_re =
	NULL; /* heap-allocated, sized to npatterns */
...
static void compile_patterns(void)
{
	...
	for (int i = 0; i < npatterns; i++)
		re_compile(&compiled_re[i], patterns[i], (opt_mode == MODE_ERE),
			   opt_icase);
}

static int match_line(const char *line, int len, const char **ms,
		      const char **me)
{
	for (int i = 0; i < npatterns; i++) {
		int found = 0;
		if (opt_mode == MODE_FIXED)
			found = fixed_search(patterns[i], line, len, ms, me,
					     opt_icase);
		else
			found = regex_search(&compiled_re[i], line, len, ms, me,
					     opt_icase);
...
static void before_buf_add(const char *line, int lineno, long offset)
{
	...
	before_idx++;
	if (before_count < opt_before_ctx)
		before_count++;
}
			 ...
			 int is_match, int multi_file)
{
	...
	if (multi_file && opt_with_filename != 0) {
		if (opt_color)
			printf("%s%s%s%s%c%s", color_filename, filename,
			       color_reset, color_sep, sep, color_reset);
		else
			printf("%s%c", filename, sep);
	}
	if (opt_line_number) {
		if (opt_color)
			printf("%s%d%s%s%c%s", color_lineno, lineno,
			       color_reset, color_sep, sep, color_reset);
		else
	...
	if (opt_byte_offset) {
		if (opt_color)
			printf("%s%ld%s%s%c%s", color_lineno, byte_off,
			       color_reset, color_sep, sep, color_reset);
		else
	...
	while (fgets(line, LINE_BUFSZ, fp)) {
		...
		if (matched) {
			...
			if (printed_something &&
			    (opt_before_ctx > 0 || opt_after_ctx > 0) &&
			...
			if (opt_only_match && !opt_invert && ms && me) {
				...
				print_prefix(filename, lineno, byte_off, 1,
					     multi_file);
			...
			if (after_remaining > 0) {
				print_prefix(filename, lineno, byte_off, 0,
					     multi_file);
				printf("%s\n", line);
				after_remaining--;
				printed_something = 1;
...
static int should_process_file(const char *name)
{
	if (n_include > 0) {
		...
		for (int i = 0; i < n_include; i++)
			if (fnmatch(include_globs[i], name, 0) == 0) {
				ok = 1;
				break;
			}
		if (!ok)
	...
	for (int i = 0; i < n_exclude; i++)
		if (fnmatch(exclude_globs[i], name, 0) == 0)
			return 0;
	return 1;
}
...
static int should_process_dir(const char *name)
{
	for (int i = 0; i < n_exclude_dir; i++)
		if (fnmatch(exclude_dir_globs[i], name, 0) == 0)
			return 0;
	return 1;
}
	...
	while ((nread = getdents64(dfd, dentbuf, 4096)) > 0) {
		...
		while (off < nread) {
			...
			if (strcmp(de->d_name, ".") != 0 &&
			    strcmp(de->d_name, "..") != 0) {
				snprintf(fullpath, 4096, "%s/%s", dirpath,
					 de->d_name);
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv,
				"EFGe:f:ivwxclLm:oqsbHhnTZA:B:C:aIrRV",
				long_options, NULL)) != -1) {
		switch (c) {
		case 'E':
			opt_mode = MODE_ERE;
		...
		case 'l':
			...
			break;
		case 'L':
			opt_files_no_match = 1;
			break;
		case 'm':
			opt_max_count = atoi(optarg);
			break;
		case 'o':
			opt_only_match = 1;
			break;
		...
		case 'R':
			...
			break;
		case 134:
			if (n_include < MAX_GLOBS)
				include_globs[n_include++] = strdup(optarg);
			break;
		case 135:
			if (n_exclude < MAX_GLOBS)
				exclude_globs[n_exclude++] = strdup(optarg);
			break;
		case 136:
			if (n_exclude_dir < MAX_GLOBS)
				exclude_dir_globs[n_exclude_dir++] =
					strdup(optarg);
			break;
		case 137:
			setlinebuf(stdout);
	...
	if (nfiles == 0) {
		const char *label =
			opt_label ? opt_label_str : "(standard input)";
		if (process_file(stdin, label, multi_file) == 0)
			ret = 0;
	} else {
		for (int i = optind; i < argc; i++) {
			if (strcmp(argv[i], "-") == 0) {
				const char *label = opt_label ?
							    opt_label_str :
							    "(standard input)";
				if (process_file(stdin, label, multi_file) == 0)
					ret = 0;
			} else {
				if (grep_file(argv[i], multi_file) == 0)
					ret = 0;
			}
		}
	}

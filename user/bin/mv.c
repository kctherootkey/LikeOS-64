...
static int do_move(const char *src, const char *dest)
{
	...
	if (dest_exists) {
		...
		if (opt_interactive == INTER_INTERACTIVE) {
			char buf[16];
			fprintf(stderr, PROGRAM_NAME ": overwrite '%s'? ",
				dest);
	...
	if (errno == EXDEV) {
		if (opt_no_copy) {
			fprintf(stderr,
				PROGRAM_NAME
				": cannot move '%s' to '%s': cross-device move "
		...
		if (ret < 0) {
			fprintf(stderr,
				PROGRAM_NAME ": failed to copy '%s' to '%s'\n",
				src, dest);
			return -1;
		}
		...
		if (ret < 0) {
			fprintf(stderr,
				PROGRAM_NAME ": warning: copied '%s' to '%s' "
...
int main(int argc, char **argv)
{
	...
	int c;
	while ((c = getopt_long(argc, argv, "bfinS:t:TuvZ", long_options,
				NULL)) != -1) {
		switch (c) {
		case 'b':

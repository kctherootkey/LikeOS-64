...
#define COL_DEFAULT \
	(COL_SOURCE | COL_SIZE | COL_USED | COL_AVAIL | COL_PCENT | COL_TARGET)
...
static void human_size(unsigned long bytes, int si, char *buf, size_t bufsz)
{
	...
	if (idx < 0) {
		...
		snprintf(buf, bufsz, "%lu%c", (unsigned long)(val + 0.5),
			 units[idx]);
	} else {
...
static unsigned long get_block_size(void)
{
	...
	if (opt_human || opt_si)
		return 1; /* bytes, formatted later */
	if (opt_block_1k)
		return 1024;
 ...
 * ================================================================ */
static void print_fs(const char *source, const char *fstype, const char *target,
		     const struct statfs *st, const char *file)
{
	...
	if (opt_inodes && !opt_columns)
		cols = COL_SOURCE | COL_ITOTAL | COL_IUSED | COL_IAVAIL |
		       COL_IPCENT | COL_TARGET;
...
static unsigned int parse_output_columns(const char *arg)
{
	...
	while (tok && *tok) {
		...
		if (strcmp(tok, "source") == 0)
			cols |= COL_SOURCE;
		else if (strcmp(tok, "fstype") == 0)
			cols |= COL_FSTYPE;
		else if (strcmp(tok, "size") == 0)
			cols |= COL_SIZE;
		else if (strcmp(tok, "used") == 0)
			cols |= COL_USED;
		else if (strcmp(tok, "avail") == 0)
			cols |= COL_AVAIL;
		else if (strcmp(tok, "pcent") == 0)
			cols |= COL_PCENT;
		else if (strcmp(tok, "target") == 0)
			cols |= COL_TARGET;
		else if (strcmp(tok, "itotal") == 0)
			cols |= COL_ITOTAL;
		else if (strcmp(tok, "iused") == 0)
			cols |= COL_IUSED;
		else if (strcmp(tok, "iavail") == 0)
			cols |= COL_IAVAIL;
		else if (strcmp(tok, "ipcent") == 0)
			cols |= COL_IPCENT;
...
int main(int argc, char **argv)
{
	...
	int c;
	while ((c = getopt_long(argc, argv, "aB:hHiklPTt:x:", longopts,
				NULL)) != -1) {
		switch (c) {
		case 'a':
			opt_all = 1;
			break;
		case 'B':
			opt_block_size = strtoul(optarg, NULL, 10);
			break;
		case 'h':
			opt_human = 1;
			opt_si = 0;
		...
		case 256:
			opt_total = 1;
			break;
		case 257: /* --no-sync, default */
			break;
		case 258: /* --sync */
			sync();
			break;
		case 259:
			opt_columns = parse_output_columns(optarg);
			break;
		case 260:
			usage();
			return 0;
		case 261:
			printf("df (LikeOS) 1.0\n");
			return 0;
		default:
			usage();
			return 1;
	...
	if (optind < argc) {
		...
		for (int i = optind; i < argc; i++) {
			...
			/* Filter by type */
			if (opt_type && strcmp(opt_type, fstype) != 0)
				continue;
			if (opt_exclude && strcmp(opt_exclude, fstype) == 0)
				continue;
		...
		if (opt_all) {
			...
			if (!opt_type || strcmp(opt_type, "devfs") == 0) {
				if (!opt_exclude ||
				    strcmp(opt_exclude, "devfs") != 0) {
					const char *dtype = "devfs";
					print_fs("devfs", dtype, "/dev", &devst,
						 NULL);
				}
			}
		}

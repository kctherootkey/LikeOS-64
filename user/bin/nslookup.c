...
static int decode_dns_name(const uint8_t *pkt, int pktlen, int off, char *out,
			   int outsz)
{
	...
	while (pos < pktlen && jumps < 16) {
		...
		pos++;
		if (olen > 0 && olen < outsz - 1)
			out[olen++] = '.';
		for (int i = 0; i < len && pos < pktlen && olen < outsz - 1;
		     i++)
...
static void print_state(void)
{
	...
	printf("  %sd2\t\t\t\t%s\n", d2_mode ? "" : "no",
	       d2_mode ? "d2" : "nod2");
	printf("  %ssearch\t\t\t%s\n", use_search ? "" : "no",
	       use_search ? "search" : "nosearch");
	printf("  %srecurse\t\t\t%s\n", use_recurse ? "" : "no",
	       use_recurse ? "recurse" : "norecurse");
	printf("  %sfail\t\t\t\t%s\n", use_fail ? "" : "no",
	       use_fail ? "fail" : "nofail");
	printf("  timeout = %d\n", timeout_secs);
	printf("  retry = %d\n", retry_count);
	printf("  port = %d\n", server_port);
	printf("  querytype = %s\n", query_type);
...
static void process_set(const char *arg)
{
	if (strcmp(arg, "all") == 0) {
		print_state();
		   ...
		   strncmp(arg, "querytype=", 10) == 0) {
		const char *val = strchr(arg, '=') + 1;
		strncpy(query_type, val, sizeof(query_type) - 1);
	} else if (strncmp(arg, "class=", 6) == 0) {
		strncpy(query_class, strchr(arg, '=') + 1,
			sizeof(query_class) - 1);
	} else if (strncmp(arg, "domain=", 7) == 0) {
		strncpy(search_domain, strchr(arg, '=') + 1,
			sizeof(search_domain) - 1);
		...
		d2_mode = 1;
		debug_mode = 1;
	} else if (strcmp(arg, "nod2") == 0) {
		d2_mode = 0;
	} else if (strcmp(arg, "search") == 0) {
		use_search = 1;
	} else if (strcmp(arg, "nosearch") == 0) {
		use_search = 0;
	} else if (strcmp(arg, "recurse") == 0) {
		use_recurse = 1;
	} else if (strcmp(arg, "norecurse") == 0) {
		use_recurse = 0;
...
static int do_lookup(const char *name)
{
	...
	if (is_reverse || strcmp(query_type, "PTR") == 0) {
		/* Reverse (PTR) lookup */
		if (is_reverse) {
			unsigned a = (check_ip >> 24) & 0xFF,
				 b = (check_ip >> 16) & 0xFF;
			unsigned c = (check_ip >> 8) & 0xFF,
				 d = check_ip & 0xFF;
		...
		if (strcmp(query_type, "A") == 0)
			qbuf.qtype = DNS_TYPE_A;
		else if (strcmp(query_type, "CNAME") == 0)
			qbuf.qtype = DNS_TYPE_CNAME;
		else if (strcmp(query_type, "NS") == 0)
			qbuf.qtype = 2;
		else if (strcmp(query_type, "MX") == 0)
			qbuf.qtype = 15;
		else if (strcmp(query_type, "SOA") == 0)
			qbuf.qtype = 6;
		else if (strcmp(query_type, "ANY") == 0)
			qbuf.qtype = 255;
		else
			qbuf.qtype = DNS_TYPE_A;
	}
	...
	for (int i = 0; i < ancount && pos < pktlen; i++) {
		...
		if (rtype == DNS_TYPE_A && rdlen == 4) {
			...
			char ptrname[256];
			decode_dns_name(pkt, pktlen, pos, ptrname,
					sizeof(ptrname));
			printf("%s\tname = %s.\n", qbuf.name, ptrname);
		} else if (rtype == 15 /* MX */ && rdlen >= 2) {
			uint16_t pref = (pkt[pos] << 8) | pkt[pos + 1];
			char mxname[256];
			decode_dns_name(pkt, pktlen, pos + 2, mxname,
					sizeof(mxname));
			printf("%s\tmail exchanger = %u %s.\n", name, pref,
			       mxname);
		} else if (rtype == 2 /* NS */) {
			char nsname[256];
			decode_dns_name(pkt, pktlen, pos, nsname,
					sizeof(nsname));
...
static void set_server(const char *server)
{
	...
	if (dns_resolve(server, &ip) == 0) {
		strncpy(default_server_addr, ip_to_str(ip),
			sizeof(default_server_addr) - 1);
	} else {
		strncpy(default_server_addr, server,
			sizeof(default_server_addr) - 1);
	}
}
...
static void parse_option(const char *opt)
{
	...
	if (strncmp(opt, "type=", 5) == 0 || strncmp(opt, "query=", 6) == 0 ||
		...
		strncpy(query_class, strchr(opt, '=') + 1,
			sizeof(query_class) - 1);
		...
		strncpy(search_domain, strchr(opt, '=') + 1,
			sizeof(search_domain) - 1);
	} else if (strcmp(opt, "debug") == 0) {
		debug_mode = 1;
	} else if (strcmp(opt, "nodebug") == 0) {
		debug_mode = 0;
	} else if (strcmp(opt, "d2") == 0) {
		d2_mode = 1;
		debug_mode = 1;
	} else if (strcmp(opt, "nod2") == 0) {
		d2_mode = 0;
	} else if (strcmp(opt, "search") == 0) {
		use_search = 1;
	} else if (strcmp(opt, "nosearch") == 0) {
		use_search = 0;
	} else if (strcmp(opt, "recurse") == 0) {
		use_recurse = 1;
	} else if (strcmp(opt, "norecurse") == 0) {
		use_recurse = 0;
	} else if (strcmp(opt, "vc") == 0) {
		use_vc = 1;
	} else if (strcmp(opt, "novc") == 0) {
		use_vc = 0;
	} else if (strcmp(opt, "fail") == 0) {
		use_fail = 1;
	} else if (strcmp(opt, "nofail") == 0) {
		use_fail = 0;
	}
}

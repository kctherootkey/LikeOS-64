...
static void usage(void) {
    printf(
"Usage: " PROGRAM_NAME " [OPTION]... MODE[,MODE]... FILE...\n"
...
static int parse_mode(const char *s, mode_spec_t *out) {
    ...
    if (*p) {
        const char *q = p;
        int all_octal = 1;
        for (; *q; q++) if (*q < '0' || *q > '7') { all_octal = 0; break; }
        if (all_octal && q != p) {
            out->is_octal = 1;
            out->octal_op = op;
...
static unsigned apply_symbolic(unsigned cur, int is_dir, const char *spec, int *err) {
    ...
    while (*p) {
        ...
        if (!who_given) who = 7;   /* default 'a' (umask applied below)   */

        if (*p != '+' && *p != '-' && *p != '=') { *err = 1; return cur; }

        /* one or more (op perms) groups */
        while (*p == '+' || *p == '-' || *p == '=') {
            char op = *p++;
            unsigned permbits = 0;    /* rwx pattern 0..7                 */
            unsigned special = 0;     /* setuid/setgid/sticky             */
            int copy_cat = -1;        /* u/g/o copy                       */

            if (*p == 'u' || *p == 'g' || *p == 'o') {
                copy_cat = (*p == 'u') ? 2 : (*p == 'g') ? 1 : 0;
                p++;
            } else {
                while (*p == 'r' || *p == 'w' || *p == 'x' || *p == 'X' ||
                       *p == 's' || *p == 't') {
                    switch (*p) {
                        case 'r': permbits |= 4; break;
                        case 'w': permbits |= 2; break;
                        case 'x': permbits |= 1; break;
                        case 'X':
                            if (is_dir || (mode & 0111)) permbits |= 1;
                            break;
                        case 's':
                            if (who & 4) special |= S_ISUID;
                            if (who & 2) special |= S_ISGID;
                            break;
                        case 't': special |= S_ISVTX; break;
                    }
                    p++;
                }
            }
            ...
            unsigned val = 0;
            if (who & 4) val |= permbits << 6;
            if (who & 2) val |= permbits << 3;
            if (who & 1) val |= permbits << 0;
            val |= special;
            ...
            unsigned whomask = 0;
            if (effective_who & 4) whomask |= 0700;
            if (effective_who & 2) whomask |= 0070;
            if (effective_who & 1) whomask |= 0007;
            if (effective_who & 4) whomask |= S_ISUID;
            if (effective_who & 2) whomask |= S_ISGID;
            whomask |= S_ISVTX;   /* 't' belongs to 'o'/all conceptually  */
        ...
        if (*p == ',') p++;
        else if (*p) { *err = 1; return cur; }
    }
    return mode & 07777u;
}

/* Compute the new 12-bit mode for a file given the parsed spec. */
static unsigned compute_mode(const mode_spec_t *spec, unsigned cur, int is_dir, int *err) {
    *err = 0;
    if (spec->is_octal) {
        if (spec->octal_op == '+')      return (cur | spec->octal_val) & 07777u;
        else if (spec->octal_op == '-') return (cur & ~spec->octal_val) & 07777u;
        else                            return spec->octal_val & 07777u; /* '=' or plain */
    }
...
static int do_path(const char *path, const mode_spec_t *spec) {
    struct stat st;
    if (lstat(path, &st) != 0) {
        if (!opt_silent) fprintf(stderr, PROGRAM_NAME ": cannot access '%s': %s\n", path, strerror(errno));
        exit_status = 1;
        return -1;
    }
    ...
    if (opt_recursive && S_ISDIR(st.st_mode)) {
        ...
        if (d) {
            ...
            while ((e = readdir(d)) != NULL) {
                ...
                if (pl + strlen(e->d_name) + 2 >= sizeof(child)) continue;
                snprintf(child, sizeof(child), "%s%s%s", path, need_slash ? "/" : "", e->d_name);
                struct stat cst;
            ...
            fprintf(stderr, PROGRAM_NAME ": cannot read directory '%s': %s\n", path, strerror(errno));
            exit_status = 1;
        }
    }
    ...
    if (newm != cur) {
        if (chmod(path, (mode_t)newm) != 0) {
            if (!opt_silent) fprintf(stderr, PROGRAM_NAME ": changing permissions of '%s': %s\n", path, strerror(errno));
            exit_status = 1;
            return -1;
        }
    }
...
int main(int argc, char **argv) {
    ...
    while ((c = getopt_long(argc, argv, "cfvR", long_opts, NULL)) != -1) {
        switch (c) {
            case 'c': opt_changes = 1; break;
            case 'f': opt_silent = 1; break;
            case 'v': opt_verbose = 1; break;
            case 'R': opt_recursive = 1; break;
            case 1: ref_file = optarg; break;
            case 2: opt_preserve_root = 0; break;
            case 3: opt_preserve_root = 1; break;
            case 4: usage(); return 0;
            case 5: version(); return 0;
    ...
    if (ref_file) {
        ...
        if (stat(ref_file, &rst) != 0) {
            fprintf(stderr, PROGRAM_NAME ": failed to get attributes of '%s': %s\n", ref_file, strerror(errno));
            return 1;
        }
        spec.is_octal = 1; spec.octal_op = '='; spec.octal_val = rst.st_mode & 07777u;
    } else {
        if (optind >= argc) {
    ...
    for (int i = first_file; i < argc; i++) {
        if (opt_recursive && opt_preserve_root && strcmp(argv[i], "/") == 0) {

...
{
	printf("Usage: " PROGRAM_NAME " [OPTION]... [-T] TARGET LINK_NAME\n"
...
static enum backup_type backup_from_string(const char *s)
{
	...
	if (!strcmp(s, "none") || !strcmp(s, "off"))
		return BK_NONE;
	if (!strcmp(s, "simple") || !strcmp(s, "never"))
		return BK_SIMPLE;
	if (!strcmp(s, "numbered") || !strcmp(s, "t"))
		return BK_NUMBERED;
	if (!strcmp(s, "existing") || !strcmp(s, "nil"))
		return BK_EXISTING;
	return BK_EXISTING;
}
...
static void make_abs(const char *path, char *out, size_t cap)
{
	...
	while (*s) {
		...
		char *seg = start;
		seg[len] =
			'\0'; /* safe: next char is '/' or end; we overwrite '/' */
		if (len == 1 && seg[0] == '.') {
			if (*s)
				s++;
			continue;
...
static void relative_target(const char *target, const char *linkname, char *out,
			    size_t cap)
{
	...
	int common = 0;
	while (common < tn && common < ln_ &&
	       strcmp(tp[common], lp[common]) == 0)
		common++;
	size_t o = 0;
	out[0] = '\0';
	for (int i = common; i < ln_; i++) {
		if (o + 3 < cap) {
			memcpy(out + o, "../", 3);
			o += 3;
		}
	...
	out[o] = '\0';
	if (o == 0)
		snprintf(out, cap, "%s", base_name(target));
}
...
{
	char bak[4096];
	if (opt_bktype == BK_NUMBERED || (opt_bktype == BK_EXISTING)) {
...
static int make_link(const char *target, const char *linkpath)
{
	...
	if (dst_exists) {
		...
		if (opt_backup) {
			...
			fprintf(stderr,
				PROGRAM_NAME
				": failed to create %s '%s': File exists\n",
	...
	if (opt_symbolic) {
		...
		char rel[4096];
		if (opt_relative) {
			relative_target(target, linkpath, rel, sizeof(rel));
			ltarget = rel;
		}
...
static int make_link_in_dir(const char *target, const char *dir)
{
	...
	snprintf(dest, sizeof(dest), "%s%s%s", dir, need_slash ? "/" : "",
		 base_name(target));
...
int main(int argc, char **argv)
{
	...
	if (envc)
		opt_bktype = backup_from_string(envc);

	int c;
	while ((c = getopt_long(argc, argv, "bdFfiLnPrsS:t:Tv", long_opts,
				NULL)) != -1) {
		switch (c) {
		case 1:
			opt_backup = 1;
			if (optarg)
				opt_bktype = backup_from_string(optarg);
			break;
		case 'b':
			opt_backup = 1;
			break;
	...
	if (no_target_dir) {
		if (nargs != 2) {
			fprintf(stderr, PROGRAM_NAME
				": with -T, exactly two operands are required\n");
			return 1;
		}
		return make_link(argv[optind], argv[optind + 1]) == 0 ?
			       exit_status :
			       exit_status;
	}
	...
	if (nargs == 1) {
		...
		return make_link_in_dir(argv[optind], ".") == 0 ? exit_status :
								  exit_status;
	}
	...
	if (!is_directory(dir)) {
		fprintf(stderr,
			PROGRAM_NAME ": target '%s' is not a directory\n", dir);
		return 1;
	}

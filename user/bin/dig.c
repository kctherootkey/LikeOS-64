...
static void parse_queryopt(const char *opt)
{
	...
	if (strcmp(p, "short") == 0) {
		...
		show_cmd = !negate;
	} else if (strcmp(p, "question") == 0) {
		show_question = !negate;
	} else if (strcmp(p, "answer") == 0) {
		show_answer = !negate;
	} else if (strcmp(p, "authority") == 0) {
		show_authority = !negate;
	} else if (strcmp(p, "additional") == 0) {
		show_additional = !negate;
	} else if (strcmp(p, "comments") == 0) {
		show_comments = !negate;
	} else if (strcmp(p, "stats") == 0) {
		show_stats = !negate;
	} else if (strcmp(p, "ttlid") == 0) {
		show_ttl = !negate;
		...
		show_comments = show_stats = !negate;
		   ...
		   strcmp(p, "vc") == 0) { /* accepted */
	} else if (strcmp(p, "ignore") == 0) { /* accepted */
	} else if (strcmp(p, "recurse") == 0) { /* accepted */
	} else if (strcmp(p, "aaflag") == 0 ||
		   strcmp(p, "aaonly") == 0) { /* accepted */
...
static void display_response(const uint8_t *pkt, int pktlen, const char *qname,
		...
		if (show_answer) {
			if (rtype == DNS_TYPE_A && rdlen == 4) {
				...
				if (short_mode) {
				   ...
				   rtype == 2 /* NS */) {
				char rdata_name[256];
				decode_dns_name(pkt, pktlen, pos, rdata_name,
						sizeof(rdata_name));
				if (short_mode) {
					printf("%s\n", rdata_name);
				} else {
...
static void do_reverse_lookup(const char *addr)
{
	...
	snprintf(qbuf.name, sizeof(qbuf.name), "%u.%u.%u.%u.in-addr.arpa", d, c,
		 b, a);
	qbuf.qtype = DNS_TYPE_PTR;

	int ret = dns_query(&qbuf);
	if (ret == 0 && qbuf.response_len > 0) {
		display_response(qbuf.response, qbuf.response_len, qbuf.name,
				 "PTR");
	} else {
		if (show_comments) {
			printf(";; Got answer:\n");
...
static void do_lookup(const char *name, const char *qtype)
{
	...
	if (strcmp(qtype, "PTR") == 0)
		...
		qt = DNS_TYPE_CNAME;
	else if (strcmp(qtype, "ANY") == 0)
		qt = 255;
	else if (strcmp(qtype, "NS") == 0)
		qt = 2;
	else if (strcmp(qtype, "MX") == 0)
		qt = 15;
	else if (strcmp(qtype, "SOA") == 0)
		qt = 6;
	else if (strcmp(qtype, "TXT") == 0)
		qt = 16;
	...
	for (i = 1; i < argc; i++) {
		...
		if (!name) {
			...
			if (strcmp(argv[i], "A") == 0 ||
			    strcmp(argv[i], "AAAA") == 0 ||
			    strcmp(argv[i], "MX") == 0 ||
			    strcmp(argv[i], "NS") == 0 ||
			    strcmp(argv[i], "SOA") == 0 ||
			    strcmp(argv[i], "TXT") == 0 ||
			    strcmp(argv[i], "CNAME") == 0 ||
			    strcmp(argv[i], "PTR") == 0 ||
			    strcmp(argv[i], "SRV") == 0 ||
			    strcmp(argv[i], "ANY") == 0 ||
			    strcmp(argv[i], "AXFR") == 0 ||
			    strcmp(argv[i], "IXFR") == 0) {
				qtype = argv[i];
			} else {
				name = argv[i];
			}

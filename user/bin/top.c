...
{
	for (const field_spec_t *f = field_specs; f->name; f++) {
		if (my_strcasecmp(name, f->name) == 0)
			return f;
	}
	return NULL;
}
...
static const char *mem_scale_label[] = { "KiB", "MiB", "GiB",
					 "TiB", "PiB", "EiB" };
...
static double mem_scale_bytes(uint64_t bytes, int scale)
{
	...
	switch (scale) {
	case 0:
		return val / 1024.0; /* KiB */
	case 1:
		return val / (1024.0 * 1024.0); /* MiB */
	case 2:
		return val / (1024.0 * 1024.0 * 1024.0); /* GiB */
	case 3:
		return val / (1024.0 * 1024.0 * 1024.0 * 1024.0); /* TiB */
	case 4:
		return val /
		       (1024.0 * 1024.0 * 1024.0 * 1024.0 * 1024.0); /* PiB */
	case 5:
		return val / (1024.0 * 1024.0 * 1024.0 * 1024.0 * 1024.0 *
			      1024.0); /* EiB */
	default:
		return val / 1024.0;
	}
...
static void format_elapsed(uint64_t uptime_ticks, uint64_t start_tick,
...
static void collect_processes(void)
{
	...
	for (int i = 0; i < n; i++) {
		...
		if (raw->is_kernel) {
			...
			pe->pmem =
				(double)rss_bytes * 100.0 / (double)g_total_mem;
		} else {
			pe->pmem = 0.0;
		}
...
{
	if (opt_n_filter_pids == 0)
		return 1;
	for (int i = 0; i < opt_n_filter_pids; i++) {
		if (pe->info.pid == opt_filter_pids[i])
			return 1;
	}
	return 0;
}
...
static int pass_other_filters(const proc_entry_t *pe)
{
	for (int f = 0; f < g_nfilters; f++) {
		...
		if (my_strcasecmp(flt->field_name, "PID") == 0)
			...
			snprintf(val_buf, sizeof(val_buf), "%c",
				 state_char(pe->info.state));
		else if (my_strcasecmp(flt->field_name, "RES") == 0)
			snprintf(val_buf, sizeof(val_buf), "%lu",
				 (unsigned long)(pe->info.rss * PAGE_SIZE /
						 1024));
		else if (my_strcasecmp(flt->field_name, "VIRT") == 0)
			snprintf(val_buf, sizeof(val_buf), "%lu",
				 (unsigned long)(pe->info.vsz / 1024));
...
{
	return pass_pid_filter(pe) && pass_user_filter(pe) &&
	       pass_idle_filter(pe) && pass_other_filters(pe);
}
...
static int compare_procs(const void *a, const void *b)
{
	...
	case FLD_PMEM:
		if (pa->pmem > pb->pmem)
			cmp = 1;
		else if (pa->pmem < pb->pmem)
			cmp = -1;
		else
			cmp = 0;
		break;
	case FLD_VIRT:
		if (pa->info.vsz > pb->info.vsz)
			cmp = 1;
		else if (pa->info.vsz < pb->info.vsz)
			cmp = -1;
		else
			cmp = 0;
		break;
	case FLD_RES:
		if (pa->info.rss > pb->info.rss)
			cmp = 1;
		else if (pa->info.rss < pb->info.rss)
			cmp = -1;
		else
			cmp = 0;
		break;
	...
	case FLD_TIME:
	case FLD_TIMEP:
		if (pa->total_ticks > pb->total_ticks)
			cmp = 1;
		else if (pa->total_ticks < pb->total_ticks)
			cmp = -1;
		else
			cmp = 0;
		break;
...
static void format_field_value(const proc_entry_t *pe, int field_id, char *buf,
			       size_t bufsz)
{
	...
	case FLD_PMEM:
		...
		break;
	case FLD_VIRT:
		format_mem_task(pe->info.is_kernel ? 0 : pe->info.vsz, buf,
				bufsz, opt_mem_task_scale);
		break;
	case FLD_RES:
		format_mem_task(pe->info.is_kernel ? 0 :
						     pe->info.rss * PAGE_SIZE,
				buf, bufsz, opt_mem_task_scale);
		break;
	case FLD_SHR:
	...
	case FLD_DATA:
		...
		break;
	case FLD_USED:
		format_mem_task(pe->info.rss * PAGE_SIZE, buf, bufsz,
				opt_mem_task_scale);
		break;
	case FLD_ELAPSED: {
...
static void print_summary_uptime(const struct sysinfo *si)
{
	...
	pos += snprintf(line + pos, sizeof(line) - pos,
			"top - %02d:%02d:%02d up ", tm->tm_hour, tm->tm_min,
			tm->tm_sec);
	...
	if (hrs > 0)
		pos += snprintf(line + pos, sizeof(line) - pos, "%d:%02d, ",
				hrs, mins);
	else
		pos += snprintf(line + pos, sizeof(line) - pos, "%d min, ",
				mins);
	...
	pos += snprintf(line + pos, sizeof(line) - pos,
			"load average: %.2f, %.2f, %.2f", la1, la5, la15);
...
static void print_summary_tasks(void)
{
	...
	snprintf(line, sizeof(line),
		 ...
		 opt_threads ? "Threads" : "Tasks", total, running, sleeping,
		 stopped, zombie);
...
static void print_summary_cpu(void)
{
	...
	if (g_prev_procs && g_iteration > 0) {
		...
		if (elapsed_ticks > 0.0) {
			for (int i = 0; i < g_nprocs; i++) {
				/* Find in previous */
				for (int j = 0; j < g_prev_nprocs; j++) {
					if (g_procs[i].info.pid ==
					    g_prev_procs[j].info.pid) {
						uint64_t du =
							(g_procs[i]
								 .info
								 .utime_ticks >
							 ...
							 g_prev_procs[j]
								...
								(g_procs[i]
								 ...
								 g_prev_procs[j]
									 ...
									 .stime_ticks) :
								0;
						total_us += (double)du * 100.0 /
							    elapsed_ticks;
						total_sy += (double)ds * 100.0 /
							    elapsed_ticks;
						break;
					}
				}
	...
	if (show_task_cpu == 1) {
		...
		char bar[128];
		int bpos = 0;
		for (int i = 0; i < bar_width && bpos < (int)sizeof(bar) - 1;
		     i++)
			bar[bpos++] = (i < filled) ? '|' : ' ';
		bar[bpos] = '\0';
		snprintf(line, sizeof(line), "%%Cpu(s) [%s] %5.1f%%", bar,
			 used);
		...
		char bar[128];
		int bpos = 0;
		for (int i = 0; i < bar_width && bpos < (int)sizeof(bar) - 4;
		     i++) {
			if (i < filled) {
...
static void print_summary_memory(const struct sysinfo *si)
{
	...
	if (show_memory == 1) {
		...
		snprintf(
			...
			lbl, mem_scale_bytes(swap_total, sc),
		...
		double mem_pct =
			(total > 0) ? ((double)used * 100.0 / (double)total) :
				      0.0;
		double swap_pct = (swap_total > 0) ?
					  ((double)swap_used * 100.0 /
					   (double)swap_total) :
					  0.0;
		...
		snprintf(line, sizeof(line), "%s Swp [%s] %5.1f%% of %.1f%s",
			 lbl, bar, swap_pct, mem_scale_bytes(swap_total, sc),
			 lbl);
		if (show_colors)
			printf("%s%s", ESC_BOLD, CLR_WHITE);
		print_limited(line, g_term_cols);
...
static void print_task_header(void)
{
	...
	for (int f = 0; f < g_nfields; f++) {
		...
		if (show_hlight_x && g_fields[f].id == opt_sort_field &&
		    show_colors) {
			printf("%s", CLR_YELLOW);
		}
...
static void print_task_row(const proc_entry_t *pe, int last_row)
{
	...
	for (int f = 0; f < g_nfields; f++) {
		...
		/* Zero suppress */
		if (!show_zero && g_fields[f].id != FLD_PID &&
		    g_fields[f].id != FLD_UID && g_fields[f].id != FLD_USER &&
		    g_fields[f].id != FLD_NI && g_fields[f].id != FLD_PR &&
		    g_fields[f].id != FLD_P && g_fields[f].id != FLD_S &&
		    g_fields[f].id != FLD_COMMAND) {
...
static void repaint_screen(void)
{
	...
	int avail_rows =
		g_term_rows - g_summary_lines - 1; /* -1 for header row */
	if (avail_rows < 0)
		avail_rows = 0;
...
static void show_help_screen(void)
{
	...
	printf("Window 1:Def: Cumulative mode %s.  System: Delay %.1f secs; Secure mode %s.\n\n",
	       opt_cumulative ? "On" : "Off", opt_delay,
	       opt_secure ? "On" : "Off");

	printf("  Z%s,%sB%s,%sE%s,%se%s   Global: 'Z' colors; 'B' bold; 'E'/'e' summary/task memory scale\n",
	       ESC_BOLD, ESC_RESET, ESC_BOLD, ESC_RESET, ESC_BOLD, ESC_RESET,
	       ESC_BOLD);
...
static void show_fields_screen(void)
{
	...
	while (!done) {
		...
		for (int i = 0; i < FLD__COUNT; i++) {
			...
			printf("  %c %c %-10s - %s", active ? '*' : ' ',
			       is_sort ? 's' : ' ', sp->name, sp->header);
		...
		if (ch == 'q' || ch == 27) {
			...
			if (idx >= 0) {
				...
				/* Add field */
				const field_spec_t *sp =
					find_field_spec(cursor);
				if (sp && g_nfields < MAX_FIELDS) {
					active_field_t *af =
						&g_fields[g_nfields++];
					af->id = sp->id;
					strncpy(af->header, sp->header,
						sizeof(af->header) - 1);
					af->header[sizeof(af->header) - 1] =
						'\0';
					af->width = sp->width;
					af->right = sp->right;
					af->visible = 1;
...
static void show_message(const char *msg)
{
	...
	printf("\033[%d;1H%s%s%s%s", g_summary_lines, ESC_BOLD, msg, ESC_RESET,
	       ESC_CLEAR_EOL);
	fflush(stdout);
}
...
{
	printf("\033[%d;1H%s%s%s %s", g_summary_lines, ESC_BOLD, prompt,
	       ESC_RESET, ESC_CLEAR_EOL);
	fflush(stdout);
...
static void process_interactive_command(int ch)
{
	...
	case 's':
		if (opt_secure)
			break;
		if (prompt_input("Change delay from %.1f to", buf,
				 sizeof(buf)) >= 0 &&
		    buf[0]) {
	...
	case 'H':
		...
		show_message(opt_threads ? "Threads mode On" :
					   "Threads mode Off");
		break;
	...
	case 'k':
		...
		if (opt_secure)
			break;
		if (prompt_input(
			    "PID to signal/kill [default pid = first displayed]",
			    buf, sizeof(buf)) >= 0) {
			int pid = 0;
			if (buf[0])
				pid = atoi(buf);
			...
			if (pid > 0) {
				char sig_buf[64];
				char sig_prompt[128];
				snprintf(sig_prompt, sizeof(sig_prompt),
					 "Send pid %d signal [15/sigterm]",
					 pid);
				if (prompt_input(sig_prompt, sig_buf,
						 sizeof(sig_buf)) >= 0) {
					int sig = SIGTERM;
					if (sig_buf[0]) {
						sig = atoi(sig_buf);
	...
	case 'r':
		...
		if (opt_secure)
			break;
		if (prompt_input(
			    "PID to renice [default pid = first displayed]",
			    buf, sizeof(buf)) >= 0) {
			int pid = 0;
			if (buf[0])
				pid = atoi(buf);
			...
			if (pid > 0) {
				char nice_buf[64];
				if (prompt_input("Renice PID to value",
						 nice_buf,
						 sizeof(nice_buf)) >= 0 &&
				    nice_buf[0]) {
	...
	case 't':
		...
		if (show_task_cpu == 0)
			show_task_cpu = 0; /* Off: handled in display */
		break;
	...
	case 'S':
		...
		show_message(opt_cumulative ? "Cumulative mode On" :
					      "Cumulative mode Off");
		break;
	...
	case 'U': {
		/* User filter */
		char user_buf[128];
		int r = prompt_input("Which user (blank for all)", user_buf,
				     sizeof(user_buf));
		if (r >= 0) {
			if (user_buf[0] == '\0') {
				opt_filter_user_active = 0;
				...
				strncpy(opt_filter_user, p,
					sizeof(opt_filter_user) - 1);
				opt_filter_user[sizeof(opt_filter_user) - 1] =
					'\0';
	...
	case '#': {
		/* Set maximum tasks */
		char n_buf[32];
		if (prompt_input("Maximum tasks = 0 is unlimited", n_buf,
				 sizeof(n_buf)) >= 0 &&
		    n_buf[0]) {
			int n = atoi(n_buf);
			if (n >= 0)
	...
	case 'o': {
		...
		char filter_buf[256];
		if (prompt_input("add filter #1 (ignstrcase) as: [!]FLD?VAL",
				 filter_buf, sizeof(filter_buf)) >= 0 &&
		    filter_buf[0]) {
			if (g_nfilters < MAX_FILTERS) {
				...
				if (*p) {
					...
					*p = '\0';
					p++;
					strncpy(flt->field_name, fn,
						sizeof(flt->field_name) - 1);
					strncpy(flt->value, p,
						sizeof(flt->value) - 1);
					g_nfilters++;
				}
	...
	case '>':
		...
		for (int i = 0; i < g_nfields; i++) {
			if (g_fields[i].id == opt_sort_field &&
			    i < g_nfields - 1) {
				opt_sort_field = g_fields[i + 1].id;
				break;
			}
	...
	case 'L': {
		/* Locate string */
		char search_buf[128];
		if (prompt_input("Locate string", search_buf,
				 sizeof(search_buf)) >= 0 &&
		    search_buf[0]) {
	...
	case 'g': {
		...
		char g_buf[8];
		if (prompt_input("Choose field group (1-4)", g_buf,
				 sizeof(g_buf)) >= 0) {
...
static int parse_options(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "bcd:E:e:Hhin:O:o:p:SsU:u:Vw::1")) !=
	       -1) {
		switch (opt) {
		case 'b':
			opt_batch = 1;
		...
		case 'E':
			...
			switch (optarg[0]) {
			case 'k':
				opt_mem_summary_scale = 0;
				break;
			case 'm':
				opt_mem_summary_scale = 1;
				break;
			case 'g':
				opt_mem_summary_scale = 2;
				break;
			case 't':
				opt_mem_summary_scale = 3;
				break;
			case 'p':
				opt_mem_summary_scale = 4;
				break;
			case 'e':
				opt_mem_summary_scale = 5;
				break;
			default:
				fprintf(stderr,
					"top: invalid memory scale '%c'\n",
					optarg[0]);
				return -1;
			}
			break;
		case 'e':
			...
			switch (optarg[0]) {
			case 'k':
				opt_mem_task_scale = 0;
				break;
			case 'm':
				opt_mem_task_scale = 1;
				break;
			case 'g':
				opt_mem_task_scale = 2;
				break;
			case 't':
				opt_mem_task_scale = 3;
				break;
			case 'p':
				opt_mem_task_scale = 4;
				break;
			default:
				fprintf(stderr,
					"top: invalid memory scale '%c'\n",
		...
		case 'p': {
			...
			while (tok && opt_n_filter_pids < MAX_PIDS) {
				opt_filter_pids[opt_n_filter_pids++] =
					atoi(tok);
				tok = strtok(NULL, ",");
			}
		...
		case 's':
			...
			break;
		case 'U':
			strncpy(opt_filter_user, optarg,
				sizeof(opt_filter_user) - 1);
			...
			if (opt_filter_user[0] == '!') {
				opt_filter_user_negate = 1;
				memmove(opt_filter_user, opt_filter_user + 1,
					strlen(opt_filter_user));
			}
			break;
		case 'u':
			strncpy(opt_filter_user, optarg,

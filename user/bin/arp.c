...
{
	unsigned int m[6];
	if (sscanf(mac_str, "%x:%x:%x:%x:%x:%x", &m[0], &m[1], &m[2], &m[3],
		   &m[4], &m[5]) != 6)
		return -1;
	for (int i = 0; i < 6; i++)
			     ...
			     const char *iface, int flags)
{
	...
	printf("%-24s ether   %02x:%02x:%02x:%02x:%02x:%02x   %-6s %s\n", ip,
	       mac[0], mac[1], mac[2], mac[3], mac[4], mac[5], fstr, iface);
}
			   ...
			   const char *filter_host)
{
	...
	for (int i = 0; i < n; i++) {
		...
		if (bsd_style) {
			show_entry_bsd(ipstr, entries[i].mac, iface, ATF_COM,
				       numeric);
		} else {
...
static int delete_entry(const char *hostname, const char *iface, int pub,
			int verbose)
{
...
static int set_entry(const char *hostname, const char *hw_addr,
		     const char *iface, int temp, int pub, int use_device,
		     int verbose)
{
	int fd = socket(AF_INET, SOCK_DGRAM, 0);
	if (fd < 0)
...
static int process_file(const char *filename, const char *iface, int use_device,
			int verbose)
{
	FILE *fp;
	...
	while (fgets(line, sizeof(line), fp)) {
		...
		int temp = 0, pub = 0;
		if (n >= 3) {
			if (strcmp(extra, "temp") == 0)
				temp = 1;
			else if (strcmp(extra, "pub") == 0)
				pub = 1;
		}

		if (set_entry(host, mac, iface, temp, pub, use_device,
			      verbose) < 0)
			errors++;
	}
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, "aed:s:f::i:H:t:nDvVh",
		...
		case 't':
			...
			if (strcmp(hw_type, "ether") != 0) {
				fprintf(stderr,
					"arp: hardware type '%s' not supported\n",
					hw_type);
				return 1;
			}
			break;
	...
	case 's': {
		...
		int temp = 0, pub = 0;
		while (optind < argc) {
			if (strcmp(argv[optind], "temp") == 0)
				temp = 1;
			else if (strcmp(argv[optind], "pub") == 0)
				pub = 1;
			else if (strcmp(argv[optind], "nopub") == 0)
				pub = 0;
			else if (strcmp(argv[optind], "trail") ==
				 0) { /* accepted, not used */
			}
			optind++;
		}

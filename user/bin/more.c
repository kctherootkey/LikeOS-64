	...
	for (;;) {
		...
		for (ssize_t i = 0; i < r; i++) {
			if (buf[i] == '\n' || line_pos >= LINE_BUF_SIZE - 1) {
				...
				prev_blank = is_blank;
				if (add_line_more(linebuf, line_pos) < 0)
					return -1;
				line_pos = 0;
			} else {
...
static void display_screen(int num_lines)
{
	...
	while (displayed < num_lines) {
		...
		if (current_line >= total_lines && pipe_fd >= 0) {
			...
			fcntl(pipe_fd, F_SETFL,
			      fl & ~O_NONBLOCK); /* blocking */
...
static int page_file(const char *filename)
{
	...
	if (current_line >= total_lines) {
		if (opt_exit_on_eof ||
		    (pipe_fd < 0 && tty_fd < 0 && !isatty(STDIN_FILENO)))
			return 0;
	}
	...
	for (;;) {
		...
		case '/': { /* Search */
			...
			if (plen > 0)
				strncpy(last_search, pattern,
					sizeof(last_search) - 1);
		...
		case '.': /* Repeat previous command */
			...
			if (prev_cmd == ' ' || prev_cmd == 'z')
				display_screen(prev_count ? prev_count :
							    screen_lines);
			else if (prev_cmd == '\n')
...
int main(int argc, char **argv)
{
	...
	int c;
	optind = 1;
	while ((c = getopt_long(real_argc, real_argv, "dlefpcsun:hV",
				long_options, NULL)) != -1) {
		switch (c) {
		case 'd':
			opt_silent = 1;

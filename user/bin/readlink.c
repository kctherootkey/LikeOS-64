...
static int canonicalize(const char *name, int mode, char *resolved, size_t cap) {
    ...
    if (name[0] == '/') {
        ...
        if (!getcwd(rpath, sizeof(rpath))) return -1;
        snprintf(rest, sizeof(rest), "%s", name);
    }

    while (rest[0]) {
        char comp[1024]; int ci = 0;
        char *s = rest;
        while (*s == '/') s++;
        while (*s && *s != '/') { if (ci < 1023) comp[ci++] = *s; s++; }
        comp[ci] = '\0';
        char remaining[8192];
        snprintf(remaining, sizeof(remaining), "%s", s);
        int is_last = (remaining[0] == '\0');

        if (ci == 0) { snprintf(rest, sizeof(rest), "%s", remaining); continue; }
        if (strcmp(comp, ".") == 0) { snprintf(rest, sizeof(rest), "%s", remaining); continue; }
        if (strcmp(comp, "..") == 0) {
            char *sl = strrchr(rpath, '/');
            if (sl && sl != rpath) *sl = '\0'; else { rpath[0] = '/'; rpath[1] = '\0'; }
            snprintf(rest, sizeof(rest), "%s", remaining); continue;
        }

        size_t rl = strlen(rpath);
        if (rl + 1 + (size_t)ci + 1 >= sizeof(rpath)) { errno = ENAMETOOLONG; return -1; }
        if (rl == 0 || rpath[rl - 1] != '/') rpath[rl++] = '/';
        memcpy(rpath + rl, comp, (size_t)ci + 1);

        struct stat st;
        if (lstat(rpath, &st) != 0) {
            if (errno == ENOENT && (mode == 'm' || (mode == 'f' && is_last))) {
                if (remaining[0]) {
                    size_t pl = strlen(rpath);
                    snprintf(rpath + pl, sizeof(rpath) - pl, "%s", remaining);
                }
                rest[0] = '\0';
                break;
            }
        ...
        if (S_ISLNK(st.st_mode)) {
            ...
            char target[4096];
            int tl = (int)readlink(rpath, target, sizeof(target) - 1);
            if (tl < 0) return -1;
            target[tl] = '\0';
            char *sl = strrchr(rpath, '/');
            if (sl && sl != rpath) *sl = '\0'; else { rpath[0] = '/'; rpath[1] = '\0'; }
            ...
            if (target[0] == '/') {
                rpath[0] = '/'; rpath[1] = '\0';
                snprintf(newrest, sizeof(newrest), "%s%s", target + 1, remaining);
            } else {
                snprintf(newrest, sizeof(newrest), "%s%s", target, remaining);
            }
...
static int handle(const char *file) {
    ...
    fputs(out, stdout);
    if (!opt_no_newline) fputc(opt_zero ? '\0' : '\n', stdout);
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    int c;
    while ((c = getopt_long(argc, argv, "femnqsvz", long_opts, NULL)) != -1) {
        switch (c) {
            case 'f': opt_canon = 'f'; break;
            case 'e': opt_canon = 'e'; break;
            case 'm': opt_canon = 'm'; break;
            case 'n': opt_no_newline = 1; break;
            case 'q': case 's': opt_verbose = 0; break;
            case 'v': opt_verbose = 1; break;
            case 'z': opt_zero = 1; break;
            case 1: usage(); return 0;

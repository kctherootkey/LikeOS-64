...
static int remove_file(const char *path)
{
	...
	if (unlink(path) < 0) {
		if (!opt_force) {
			fprintf(stderr,
				PROGRAM_NAME ": cannot remove '%s': %s\n", path,
				strerror(errno));
			return -1;
		}
		return 0;
...
int main(int argc, char **argv)
{
	...
	int c;
	while ((c = getopt_long(argc, argv, "fiIrRdv", long_options, NULL)) !=
	       -1) {
		switch (c) {
		case 'f':
			opt_force = 1;
		...
		case OPT_INTERACTIVE:
			if (!optarg || strcmp(optarg, "always") == 0)
				...
				fprintf(stderr,
					PROGRAM_NAME
					": invalid argument '%s' for '--interactive'\n",
					optarg);
				return 1;
			}
			break;

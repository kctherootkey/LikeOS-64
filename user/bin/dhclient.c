...
int main(int argc, char *argv[])
{
	...
	while (i < argc) {
		if (strcmp(argv[i], "-4") == 0) {
			...
			send_hostname = argv[i + 1];
			i += 2;
		} else if (strcmp(argv[i], "-F") == 0 && i + 1 < argc) {
			send_fqdn = argv[i + 1];
			i += 2;
		} else if (strcmp(argv[i], "-V") == 0 && i + 1 < argc) {
			vendor_class = argv[i + 1];
			i += 2;
		} else if (strcmp(argv[i], "-R") == 0 && i + 1 < argc) {
			request_list = argv[i + 1];
			i += 2;
	...
	for (int attempt = 0; attempt < 50; attempt++) {
		...
		for (int j = 0; j < n; j++) {
			if (ifaces[j].ip_addr != 0 &&
			    !(ifaces[j].flags & IFF_LOOPBACK)) {
				got_lease = 1;
				if (!quiet) {
					if (verbose) {
	...
	if (!got_lease) {
		if (!quiet)
			fprintf(stderr, "dhclient: DHCP request timed out\n");
		return 1;
	}

...
static int tty_fd =
	-1; /* fd for keyboard/terminal control (/dev/tty when piped) */
...
static int read_key(void)
{
	...
	if (c2 == '[') {
		...
		case '5':
			readch();
			return 1006; /* PGUP  (ESC[5~) */
		case '6':
			readch();
			return 1007; /* PGDN  (ESC[6~) */
		case '3':
			readch();
			return 1008; /* DEL   (ESC[3~) */
		default:
			return 27;
		}
	}
...
static int add_line(const char *text, int len)
{
	if (total_lines >= lines_capacity) {
		...
		if (new_cap > MAX_ALLOC_LINES)
			new_cap = MAX_ALLOC_LINES;
		if (total_lines >= new_cap)
			return -1;
		char **new_lines = (char **)realloc(
			lines, (size_t)new_cap * sizeof(char *));
		if (!new_lines)
			return -1;
		lines = new_lines;
...
static const char *ci_strstr(const char *haystack, const char *needle)
{
	...
	for (; *haystack; haystack++) {
		...
		for (size_t i = 0; i < nlen; i++) {
			if (!haystack[i] ||
			    !ci_char_eq(haystack[i], needle[i])) {
				match = 0;
				break;
			}
		}
...
{
	if (search_pattern[0] == '\0' || line_idx < 0 ||
...
static void render_line(int line_idx)
{
	...
	/* Status column */
	if (opt_status_col) {
		if (search_hits && line_idx < total_lines &&
		    search_hits[line_idx])
			write_char('*');
		else
			write_char(' ');
		scr_col++;
	...
	int do_highlight = opt_hilite_search && search_hits &&
			   line_idx < total_lines && search_hits[line_idx] &&
			   search_pattern[0];
...
static void show_default_prompt(void)
{
	...
	if (opt_very_long_prompt) {
		...
		if (total_lines == 0) {
			...
			int pct = total_lines > 0 ?
					  ((bot + 1) * 100 / total_lines) :
					  100;
		...
		if (bot >= total_lines)
			bot = total_lines - 1;
		int pct =
			total_lines > 0 ? ((bot + 1) * 100 / total_lines) : 100;
...
static void cmd_toggle_option(void)
{
	...
	for (int i = 0; buf[i]; i++) {
		...
		case 'i':
			...
			show_status(opt_ignore_case ?
					    "Ignore case in searches" :
					    "Case is significant");
			break;
		case 'N':
			opt_line_numbers = !opt_line_numbers;
			show_status(opt_line_numbers ? "Line numbers ON" :
						       "Line numbers OFF");
			break;
		case 'S':
			opt_chop_lines = !opt_chop_lines;
			show_status(opt_chop_lines ? "Chop long lines" :
						     "Fold long lines");
			break;
		case 's':
			opt_squeeze_blank = !opt_squeeze_blank;
			show_status(opt_squeeze_blank ?
					    "Squeeze blank lines" :
					    "Don't squeeze blank lines");
			break;
		case 'n':
			/* toggle line numbers */
			opt_line_numbers = !opt_line_numbers;
...
static void cmd_show_info(void)
{
	...
	snprintf(buf, sizeof(buf), "%s lines %d-%d/%d byte ? %d%%",
		 current_filename ? current_filename : "(stdin)", top_line + 1,
		 bot + 1, total_lines, pct);
	show_status(buf);
	readch();
}
...
int main(int argc, char **argv)
{
	...
	int c;
	optind = 1;
	while ((c = getopt_long(real_argc, real_argv,
				"ceEfgGiIJmMnNqQrRsSuUVwWx:Xz:~#:",
				long_options, NULL)) != -1) {
		switch (c) {
		case 'c':
			opt_clear_screen = 1;
		...
		case 'f':
			opt_force_open = 1;
			break;
		case 'g': /* highlight only last match */
			break;
		case 'G':
			opt_hilite_search = 0;
			break;
		...
		case 'm':
			...
			break;
		case 'M':
			opt_very_long_prompt = 1;
			break;
		case 'n': /* suppress line numbers (default) */
			break;
		case 'N':
			opt_line_numbers = 1;
			break;
		...
		case 'S':
			...
			break;
		case 'u':
		case 'U': /* underline handling - simplified */
			break;
		case 'V':
			...
			return 0;
		case 'w':
		case 'W': /* hilite unread - simplified */
			break;
		case 'x':
	...
	if (initial_cmd) {
		if (initial_cmd[0] == '/') {
			/* Search */
			strncpy(search_pattern, initial_cmd + 1,
				sizeof(search_pattern) - 1);
			search_direction = 1;
			update_search_hits();

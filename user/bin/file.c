...
#define M(off, bytes, desc, mime)                                           \
	{                                                                   \
		off, (const unsigned char *)bytes, sizeof(bytes) - 1, desc, \
			mime                                                \
	}
...
static const magic_entry_t magic_db[] = {
	...
	/* Scripts (shebang) */
	M(0, "#!/bin/sh", "POSIX shell script, ASCII text executable",
	  "text/x-shellscript"),
	M(0, "#!/bin/bash", "Bash shell script, ASCII text executable",
	  "text/x-shellscript"),
	M(0, "#!/usr/bin/env", "script, ASCII text executable",
	  "text/x-script"),
	...
	M(0, "RIFF", "RIFF data",
	  "application/octet-stream"), /* could be WAV, AVI, etc */
	...
	/* Mach-O */
	M(0, "\xfe\xed\xfa\xce", "Mach-O executable",
	  "application/x-mach-binary"),
	M(0, "\xfe\xed\xfa\xcf", "Mach-O 64-bit executable",
	  "application/x-mach-binary"),
	M(0, "\xce\xfa\xed\xfe", "Mach-O executable (reverse byte ordering)",
	  "application/x-mach-binary"),
	M(0, "\xcf\xfa\xed\xfe",
	  "Mach-O 64-bit executable (reverse byte ordering)",
	  "application/x-mach-binary"),
	...
	/* PSF font */
	M(0, "\x36\x04", "Linux/i386 PC Screen Font v1 data",
	  "application/x-font-psf"),
	M(0, "\x72\xb5\x4a\x86", "Linux/i386 PC Screen Font v2 data",
	  "application/x-font-psf"),
...
static unsigned long elf_read64(const unsigned char *buf, int off, int ei_data)
{
	if (ei_data == 1)
		return (unsigned long)buf[off] |
		       ...
		       ((unsigned long)buf[off + 4] << 32) |
		       ((unsigned long)buf[off + 5] << 40) |
		       ((unsigned long)buf[off + 6] << 48) |
		       ((unsigned long)buf[off + 7] << 56);
	return ((unsigned long)buf[off] << 56) |
	       ((unsigned long)buf[off + 1] << 48) |
	       ((unsigned long)buf[off + 2] << 40) |
	       ((unsigned long)buf[off + 3] << 32) |
	       ((unsigned long)buf[off + 4] << 24) |
...
static int elf_is_stripped(const unsigned char *buf, int len, const char *path)
{
	...
	if (sh_end <= (unsigned long)len) {
		for (unsigned int i = 0; i < e_shnum; i++) {
			unsigned long off =
				e_shoff + (unsigned long)i * e_shentsize;
			unsigned int sh_type =
				elf_read32(buf, (int)off + 4, ei_data);
	...
	for (unsigned int i = 0; i < e_shnum; i++) {
		...
		if (fseek(fp, (long)off, SEEK_SET) != 0)
			break;
		int toread = (int)(e_shentsize < sizeof(shdr) ? e_shentsize :
								sizeof(shdr));
		if ((int)fread(shdr, 1, toread, fp) < 8)
			break;
...
static void elf_scan_phdr(const unsigned char *buf, int len, const char *path,
			  int *has_interp, int *has_dynamic, char *interp_buf,
			  int interp_bufsz)
{
	*has_interp = 0;
	*has_dynamic = 0;
	...
	for (unsigned int i = 0; i < e_phnum; i++) {
		...
		if (!need_file && off + e_phentsize <= (unsigned long)len) {
			...
			if (fseek(fp, (long)off, SEEK_SET) != 0)
				break;
			int toread = (int)(e_phentsize < sizeof(phdr) ?
						   e_phentsize :
						   sizeof(phdr));
			if ((int)fread(phdr, 1, toread, fp) < 8)
				break;
			p = phdr;
		} else {
			break;
		...
		if (p_type == 3) { /* PT_INTERP */
			*has_interp = 1;
			...
			if (p_filesz > 0 &&
			    p_filesz < (unsigned long)(interp_bufsz - 1)) {
				if (p_offset + p_filesz <= (unsigned long)len) {
					memcpy(interp_buf, buf + p_offset,
					       p_filesz);
					interp_buf[p_filesz] = '\0';
				} else if (fp) {
					if (fseek(fp, (long)p_offset,
						  SEEK_SET) == 0) {
						int nr = (int)fread(
							interp_buf, 1,
							(int)p_filesz, fp);
			 ...
			 char *out, int outsz)
{
	...
	interp_path[0] = '\0';
	elf_scan_phdr(buf, len, path, &has_interp, &has_dynamic, interp_path,
		      (int)sizeof(interp_path));
	...
	case 2:
		...
		break;
	case 3:
		type = has_interp ? "pie executable" : "shared object";
		break;
	case 4:
		type = "core file";
		break;
	...
	if (stripped == 1)
		...
		strip_str = ", not stripped";

	if (has_interp && interp_path[0]) {
		snprintf(
			out, outsz,
			"ELF %s %s %s, %s, version %d (%s), %s, interpreter %s%s",
			bits, endian, type, machine, buf[6], osabi, link_str,
			interp_path, strip_str);
	} else {
...
static const char *detect_language(const unsigned char *buf, int len)
{
	...
	if (len > 20) {
		...
		if (strstr(s, "#include") || strstr(s, "#define") ||
		    strstr(s, "#ifndef") || strstr(s, "#ifdef") ||
		    strstr(s, "#pragma"))
			return "C source";
		if (strstr(s, "int main(") || strstr(s, "void main("))
			return "C source";
		if (strstr(s, "class ") &&
		    (strstr(s, "public:") || strstr(s, "private:") ||
		     strstr(s, "protected:")))
			return "C++ source";
		if (strstr(s, "namespace ") || strstr(s, "template<") ||
		    strstr(s, "std::"))
			return "C++ source";

		/* Python */
		if (strstr(s, "def ") && strstr(s, ":"))
			if (strstr(s, "import ") || strstr(s, "from ") ||
			    strstr(s, "class "))
		...
		/* Perl */
		if (strstr(s, "#!/usr/bin/perl") ||
		    (strstr(s, "use strict") && strstr(s, "my $")))
			return "Perl script";

		/* Ruby */
		if (strstr(s, "#!/usr/bin/ruby") ||
		    (strstr(s, "def ") && strstr(s, "end") &&
		     strstr(s, "puts ")))
			return "Ruby script";
		...
		/* Assembly */
		if (strstr(s, ".text") &&
		    (strstr(s, ".global") || strstr(s, ".globl")))
			return "assembler source";
		if (strstr(s, "section .") &&
		    (strstr(s, "global ") || strstr(s, "extern ")))
			return "assembler source";

		/* Troff */
		if (s[0] == '.' && (s[1] == 'T' || s[1] == 'S' || s[1] == 'b'))
			if (strstr(s, ".br") || strstr(s, ".TH") ||
			    strstr(s, ".SH"))
		...
		if (memcmp(buf + m->offset, m->magic, m->magic_len) == 0) {
			if (opt_mime || opt_mime_type) {
				...
				if (buf[0] == 0x7f && buf[1] == 'E' &&
				    buf[2] == 'L' && buf[3] == 'F') {
					char elf_desc[256];
					describe_elf(buf, nread, filename,
						     elf_desc,
						     sizeof(elf_desc));
					printf("%s", elf_desc);
				} else {
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv, "bEf:F:hikLNnprsv0", long_options,
				NULL)) != -1) {
		switch (c) {
		case 'b':
			opt_brief = 1;

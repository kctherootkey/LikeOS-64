...
static int opt_separate_dirs =
	0; /* -S: for dirs, do not include subdirectories */
...
/* Print a du entry */
static void print_entry(unsigned long bytes, const char *path,
			unsigned long mtime)
{
	/* Threshold filter */
	if (opt_threshold > 0 && (long)bytes < opt_threshold)
		return;
	if (opt_threshold < 0 && (long)bytes > -opt_threshold)
		return;
...
static unsigned long du_walk(const char *path, int depth, int is_cmdline_arg)
{
	...
	unsigned long display_total =
		opt_separate_dirs ? (dir_total - subdir_total) : dir_total;
...
int main(int argc, char **argv)
{
	...
	int c;
	while ((c = getopt_long(argc, argv, "0abB:cd:DhHklLmPSst:x", longopts,
				NULL)) != -1) {
		switch (c) {
		case '0':
			opt_null = 1;

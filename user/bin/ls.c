...
static void format_size(char *buf, size_t bufsz, uint64_t size)
{
	...
	if (whole >= 10 || tenths == 0)
		...
		snprintf(buf, bufsz, "%lu.%lu%c", (unsigned long)whole,
			 (unsigned long)tenths, units[u]);
}
...
static void format_time(char *buf, size_t bufsz, uint64_t t)
{
	...
	case TS_FULL_ISO:
		snprintf(buf, bufsz,
			 "%04d-%02d-%02d %02d:%02d:%02d.000000000 +0000",
	...
	default: {
		...
		if (tt > now - six_months && tt <= now) {
			snprintf(buf, bufsz, "%s %2d %02d:%02d", mon,
				 tm.tm_mday, tm.tm_hour, tm.tm_min);
		} else {
			snprintf(buf, bufsz, "%s %2d  %4d", mon, tm.tm_mday,
				 tm.tm_year + 1900);
		}
	} break;
	case TS_CUSTOM:
...
{
	if (!opt_classify && !opt_file_type && !opt_slash)
		return 0;
	if (opt_slash) {
...
static int version_compare(const char *a, const char *b)
{
	while (*a && *b) {
		if (isdigit((unsigned char)*a) && isdigit((unsigned char)*b)) {
			...
			while (isdigit((unsigned char)*ae))
				ae++;
			while (isdigit((unsigned char)*be))
				be++;
			int alen = (int)(ae - a);
			int blen = (int)(be - b);
			if (alen != blen)
...
static void sort_entries(entry_t *entries, int count)
{
	...
	case SORT_TIME:
		...
		break;
	case SORT_VERSION:
		base_cmp = cmp_version;
		break;
	case SORT_EXTENSION:
		base_cmp = cmp_extension;
		break;
	case SORT_WIDTH:
		base_cmp = cmp_width_fn;
		break;
	default:
		base_cmp = cmp_name;
		break;
 ...
 * Build path helper
 * ====================================================================== */
static void build_path(char *buf, size_t bufsz, const char *dir,
		       const char *name)
{
 ...
 * ====================================================================== */
static int collect_entries(const char *dirpath, entry_t *entries,
			   int max_entries)
{
	...
	while ((de = readdir(dir)) != NULL && count < max_entries) {
		...
		if (e->stat_valid && S_ISLNK(e->st.st_mode)) {
			int n = readlink(fullpath, e->link_target,
...
static void print_long(entry_t *entries, int count, const char *dirpath)
{
	...
	if (!opt_dired) {
		...
		for (int i = 0; i < count; i++) {
			if (entries[i].stat_valid) {
				uint64_t blk =
					(entries[i].st.st_size + 1023) / 1024;
				if (opt_kibibytes)
					total_blocks += blk;
				else
					total_blocks +=
						blk * 2; /* 512-byte blocks */
			}
		}
	...
	for (int i = 0; i < count; i++) {
		...
		if (opt_classify || opt_file_type) {
			...
			if (ic)
				putchar(ic);
		} else if (opt_slash && e->stat_valid &&
			   S_ISDIR(e->st.st_mode)) {
			putchar('/');
		}
...
static void print_columns(entry_t *entries, int count, int across)
{
	...
	int max_w = 0;
	for (int i = 0; i < count; i++) {
		int w = name_width(entries[i].name) +
			indicator_width(&entries[i]);
		if (opt_inode)
			w += 8;
		if (opt_size)
			w += 5;
	...
	for (int row = 0; row < nrows; row++) {
		for (int col = 0; col < ncols; col++) {
			...
			int nlen =
				print_name_to_buf(nbuf, sizeof(nbuf), e->name);
			printf("%s%s%s", color_for(e->st.st_mode), nbuf,
			       color_reset());
			printed += nlen;
...
static void print_commas(entry_t *entries, int count)
{
	...
	for (int i = 0; i < count; i++) {
		char nbuf[512];
		int nlen =
			print_name_to_buf(nbuf, sizeof(nbuf), entries[i].name);
		...
		printf("%s%s%s", color_for(entries[i].st.st_mode), nbuf,
		       color_reset());
		line_pos += nlen;
			...
			if (subpaths) {
				...
				for (int i = 0; i < count && si < nsub; i++) {
					...
					if (strcmp(entries[i].name, ".") == 0 ||
					    strcmp(entries[i].name, "..") == 0)
						continue;
					build_path(subpaths[si], 256, path,
						   entries[i].name);
					si++;
				}
 ...
 * ====================================================================== */
static void process_arg(const char *path, entry_t *dir_args, int *dir_count,
			entry_t *file_args, int *file_count)
{
	...
	if (S_ISDIR(st.st_mode) && !opt_directory) {
		...
		if (S_ISLNK(st.st_mode)) {
			int n = readlink(path, e->link_target,
					 sizeof(e->link_target) - 1);
			if (n > 0) {
				e->link_target[n] = '\0';
				e->link_valid = 1;
...
static void parse_time_style(const char *style)
{
	if (strcmp(style, "full-iso") == 0) {
		...
		strncpy(custom_time_fmt, style + 1,
			sizeof(custom_time_fmt) - 1);
		custom_time_fmt[sizeof(custom_time_fmt) - 1] = '\0';
	} else {
		fprintf(stderr,
			"ls: invalid argument '%s' for '--time-style'\n",
			style);
		exit_status = 2;
	}
}
...
static void parse_format(const char *word)
{
	...
	} else if (strcmp(word, "across") == 0 ||
		   strcmp(word, "horizontal") == 0) {
...
static void parse_indicator_style(const char *word)
{
	if (strcmp(word, "none") == 0) {
		...
		fprintf(stderr,
			"ls: invalid argument '%s' for '--indicator-style'\n",
			word);
		exit_status = 2;
	}
}
...
int main(int argc, char **argv)
{
	...
	if (nargs == 0) {
		...
		for (int i = optind; i < argc; i++) {
			process_arg(argv[i], dir_args, &dir_count, file_args,
				    &file_count);
		}
	}

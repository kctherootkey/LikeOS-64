...
static int lines_equal(const char *a, const char *b)
{
	...
	if (opt_check_chars > 0) {
		if (opt_ignore_case)
			return my_strncasecmp(pa, pb,
					      (size_t)opt_check_chars) == 0;
		else
...
int main(int argc, char *argv[])
{
	...
	int opt;
	while ((opt = getopt_long(argc, argv, "cdDf:is:uzw:", long_options,
				  NULL)) != -1) {
		switch (opt) {
		case 'c':
			opt_count = 1;
	...
	if (opt_group != GRP_OFF || opt_all_repeated) {
		...
		if (opt_group != GRP_OFF) {
			...
			for (int i = 0; i < nlines; i++) {
				int is_new_group =
					(i == 0 ||
					 !lines_equal(all_lines[i],
						      all_lines[i - 1]));
				if (is_new_group && !first_group) {
					if (opt_group == GRP_SEPARATE ||
					    opt_group == GRP_BOTH ||
					    opt_group == GRP_PREPEND)
						fputc(delim, out);
				}
				if (is_new_group && first_group) {
					if (opt_group == GRP_PREPEND ||
					    opt_group == GRP_BOTH)
						fputc(delim, out);
					first_group = 0;
				}
				...
				int is_last = (i == nlines - 1 ||
					       !lines_equal(all_lines[i],
							    all_lines[i + 1]));
				if (is_last && (opt_group == GRP_APPEND ||
						opt_group == GRP_BOTH))
					fputc(delim, out);
			}
		} else {

...
static int hist_real(int i)
{
	return (hist_start + i) % HIST_MAX;
}
 ...
 */
static int alias_expand_at(char *line, int maxlen, int pos, char **seen,
			   int *nseen)
{
	...
	if (word_len <= 0 || word_len >= ALIAS_NAME)
		return 0;
	...
	if (prefix_len + vlen + rest_len >= maxlen - 1)
		return 0;
...
static void alias_expand(char *line, int maxlen)
{
	...
	while (line[i]) {
		...
		if (sep_len > 0) {
			...
			for (int passes = 0; passes < 16; passes++) {
				if (!alias_expand_at(line, maxlen, cmd_start,
						     seen, &nseen))
					break;
			}
...
static void builtin_history(int argc, char **argv)
{
	...
	if (argv[1][0] != '-') {
		...
		if (*end == '\0' && n > 0) {
			int start = 0;
			if (n < hist_count)
				start = hist_count - (int)n;
			for (int i = start; i < hist_count; i++)
				printf(" %4d  %s\n", i + 1,
				       hist_buf[hist_real(i)]);
		} else {
			fprintf(stderr,
...
enum connector { CONN_NONE, CONN_SEMI, CONN_AND, CONN_OR };
			    ...
			    simple_cmd_t *cmd)
{
	...
	while (*pos < ntok && tokens[*pos].type != TOK_EOF) {
		...
		if (is_redirect_token(t->type) &&
		    t->type != TOK_REDIR_ERR_TO_OUT) {
			...
			if (*pos >= ntok || tokens[*pos].type != TOK_WORD) {
				fprintf(stderr,
					"sh: syntax error near redirect\n");
				return -1;
			}
			strncpy(r->filename, tokens[*pos].text,
				SHELL_MAX_LINE - 1);
 ...
 */
static int parse_pipeline(token_t *tokens, int ntok, int *pos, pipeline_t *pl)
{
	...
	while (*pos < ntok) {
		...
		if (t->type == TOK_PIPE || t->type == TOK_PIPE_BOTH) {
			...
			if (pl->ncmds >= MAX_PIPELINE) {
				fprintf(stderr,
					"sh: too many commands in pipeline\n");
				return -1;
			}
 ...
 */
static int parse_command_list(token_t *tokens, int ntok, cmd_entry_t *entries,
			      int max_entries)
{
	int nentries = 0;
	int pos = 0;
		...
		for (int c = 0; c < pl->ncmds; c++) {
			...
			for (int r = 0; r < cmd->nredirects; r++) {
				if (cmd->redirects[r].type == TOK_HEREDOC) {
					...
					strncpy(cmd->redirects[r].filename,
						content, SHELL_MAX_LINE - 1);
					cmd->redirects[r]
						.filename[SHELL_MAX_LINE - 1] =
						'\0';
				}
			}
		}
...
static int is_builtin(const char *name)
{
	return (strcmp(name, "cd") == 0 || strcmp(name, "exit") == 0 ||
		strcmp(name, "help") == 0 || strcmp(name, "history") == 0 ||
		strcmp(name, "export") == 0 || strcmp(name, "alias") == 0 ||
		strcmp(name, "unalias") == 0 || strcmp(name, "time") == 0);
}
	...
	for (int i = 0; i < pl->ncmds; i++) {
		...
		if (pid == 0) {
			...
			if (is_builtin(pl->cmds[i].argv[0])) {
				int rc = run_builtin(pl->cmds[i].argc,
						     pl->cmds[i].argv);
				fflush(stdout);
				fflush(stderr);
				_exit(rc);
			...
			for (int a = 0; a < pl->cmds[c].argc; a++) {
				if (a > 0)
					strncat(cmdstr, " ",
						sizeof(cmdstr) -
							strlen(cmdstr) - 1);
				strncat(cmdstr, pl->cmds[c].argv[a],
					sizeof(cmdstr) - strlen(cmdstr) - 1);
			}
		}
...
static void rl_redraw(const char *prompt, int plen, const char *buf, int len,
		      int pos)
{
...
static int shell_readline(const char *prompt, char *buf, int bufsz)
{
	...
	int hist_idx =
		hist_count; /* points one past the last entry (new line) */
	char saved_line[SHELL_MAX_LINE] =
...
int main(void)
{
	...
	while (1) {
		...
		if (hist_count == 0 ||
		    strcmp(hist_buf[hist_real(hist_count - 1)], hist_line) !=
			    0) {
			hist_add(hist_line);
		}
		...
		int nentries =
			parse_command_list(tokens, ntok, entries, MAX_COMMANDS);
		if (nentries <= 0)
			continue;

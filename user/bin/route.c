...
int main(int argc, char *argv[])
{
	...
	while (i < argc && argv[i][0] == '-') {
		if (strcmp(argv[i], "-n") == 0) {
			...
			if (strcmp(argv[i + 1], "inet") != 0 &&
			    strcmp(argv[i + 1], "inet4") != 0) {
				fprintf(stderr,
					"route: address family '%s' not supported\n",
					argv[i + 1]);
				return 1;
			}
			i += 2;
	...
	if (verbose) {
		printf("route: %s %s via %s dev %s metric %d\n",
		       adding ? "add" : "del", target, gw_str ? gw_str : "*",
		       dev ? dev : "(auto)", metric);
	}

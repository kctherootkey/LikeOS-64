...
static time_t simple_mktime(int year, int mon, int day, int hour, int min,
			    int sec)
{
...
static int parse_stamp(const char *stamp, struct timespec *ts)
{
	...
	/* Validate ranges */
	if (mon < 1 || mon > 12 || day < 1 || day > 31 || hour < 0 ||
...
static int parse_date_string(const char *str, struct timespec *ts)
{
	...
	if (mon < 1 || mon > 12 || day < 1 || day > 31 || hour > 23 ||
	    min > 59 || sec > 60)
		return -1;
...
static int do_touch(const char *path)
{
	...
	if (strcmp(path, "-") == 0) {
		...
		if (!(opt_change & CHANGE_ATIME))
			times[0].tv_nsec =
				((long)1 << 30) - 1; /* UTIME_OMIT equivalent */
...
int main(int argc, char **argv)
{
	...
	int c;
	while ((c = getopt_long(argc, argv, "acd:fhm r:t:", long_options,
				NULL)) != -1) {
		switch (c) {
		case 'a':

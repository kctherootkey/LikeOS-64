	...
	while (*s) {
		...
		while (*s && *s != '"') {
			...
			if (*s == '%') {
				...
				f->bytes_per_line +=
					u->iteration_count * u->byte_count;
			} else if (*s == '\\') {
			     ...
			     const unsigned char *rawbyte, int avail)
{
	...
	case FMT_CHAR: {
		...
		if (c == '\0') {
			...
			len = snprintf(buf, sizeof(buf), "%03o", c);
		}
		break;
	}
	case FMT_STRING: {
...
static void print_addr(const struct fmt_unit *u, unsigned long addr)
{
	...
	switch (u->addr_type) {
	case 'd':
		len = snprintf(buf, sizeof(buf), "%lu", addr);
		break;
	case 'o':
		len = snprintf(buf, sizeof(buf), "%lo", addr);
		break;
	case 'x':
	default:
		len = snprintf(buf, sizeof(buf), "%lx", addr);
		break;
	}
	if (u->width > len) {
		int pad = u->width - len;

...
static void format_bytes(uint64_t bytes, char *buf, size_t bufsz)
{
	if (bytes >= 1073741824ULL)
		snprintf(buf, bufsz, "%lu.%lu GiB",
			 (unsigned long)(bytes / 1073741824ULL),
			 (unsigned long)((bytes % 1073741824ULL) * 10 /
					 1073741824ULL));
	else if (bytes >= 1048576ULL)
		snprintf(buf, bufsz, "%lu.%lu MiB",
			 (unsigned long)(bytes / 1048576ULL),
			 (unsigned long)((bytes % 1048576ULL) * 10 /
					 1048576ULL));
	else if (bytes >= 1024ULL)
		snprintf(buf, bufsz, "%lu.%lu KiB",
...
static void show_interface_full(const net_iface_info_t *iface, int verbose)
{
	...
	if (iface->ip_addr != 0 || (iface->flags & IFF_LOOPBACK)) {
		...
		if ((iface->flags & IFF_BROADCAST) && iface->netmask != 0) {
			uint32_t bcast = (iface->ip_addr & iface->netmask) |
					 ~iface->netmask;
	...
	printf("        RX errors %lu  dropped %lu  overruns 0  frame 0\n",
	       ...
	       (unsigned long)iface->rx_dropped);
...
static void show_interface_short(const net_iface_info_t *iface)
{
	...
	if (iface->flags & IFF_LOOPBACK)
		flg[fi++] = 'L';
	if (iface->flags & IFF_MULTICAST)
		flg[fi++] = 'M';
	flg[fi] = '\0';

	printf("%-10s %-5u %-6lu %-6lu %-6lu %-6lu %s\n", iface->name,
	       iface->mtu, (unsigned long)iface->rx_packets, 0UL,
	       (unsigned long)iface->tx_packets, 0UL, flg);
}
...
static void show_one_interface(const char *name, int verbose)
{
	...
	fprintf(stderr,
		"%s: error fetching interface information: Device not found\n",
		name);
}
...
int main(int argc, char *argv[])
{
	...
	while (i < argc && argv[i][0] == '-') {
		if (strcmp(argv[i], "-a") == 0) {
			...
			break; /* not our option */
	}

	/* No interface specified: show all */
	if (i >= argc) {
		show_all_interfaces(show_all || (argc == 1), short_format,
				    verbose);
		return 0;
	}
	...
	while (i < argc) {
		...
		/* arp / -arp */
			...
			set_flags(fd, ifname, IFF_NOARP,
				  0); /* clear NOARP = enable ARP */
			i++;
		} else if (strcmp(argv[i], "-arp") == 0) {
			set_flags(fd, ifname, IFF_NOARP,
				  1); /* set NOARP = disable ARP */
			i++;
		}
		/* promisc / -promisc */
		...
		/* broadcast [addr] / -broadcast */
			...
			if (i + 1 < argc && argv[i + 1][0] != '-' &&
				...
				set_addr(fd, ifname, SIOCSIFBRDADDR,
					 argv[i + 1]);
		...
		/* pointopoint [addr] / -pointopoint */
			...
			if (i + 1 < argc && argv[i + 1][0] != '-' &&
			    strcmp(argv[i + 1], "up") != 0 &&
			    strcmp(argv[i + 1], "down") != 0) {
				set_addr(fd, ifname, SIOCSIFDSTADDR,
					 argv[i + 1]);
		...
		/* hw class address */
			...
			if (strcmp(argv[i + 1], "ether") != 0) {
				fprintf(stderr,
					"ifconfig: hardware class '%s' not supported. Use 'ether'.\n",
					argv[i + 1]);
				i += 3;
				continue;
			}
			...
			if (parse_hwaddr(argv[i + 2], mac) == 0) {
				...
				fprintf(stderr,
					"ifconfig: invalid hardware address '%s'\n",
					argv[i + 2]);
			}
			i += 3;
		}
		/* txqueuelen length */
			...
			ifr.ifr_metric =
				atoi(argv[i + 1]); /* txqueuelen stored there */
		...
		/* add addr/prefixlen (IPv6 - not supported) */
			...
			fprintf(stderr,
				"ifconfig: IPv6 address add not supported\n");
			i += 2;
		}
		/* del addr/prefixlen (IPv6 - not supported) */
		else if (strcmp(argv[i], "del") == 0 && i + 1 < argc) {
			fprintf(stderr,
				"ifconfig: IPv6 address del not supported\n");
			i += 2;
		}
		...
		/* name newname (not supported) */
			...
			fprintf(stderr,
				"ifconfig: interface rename not supported\n");
			i += 2;
		}

...
static void cs_clear(charset_t *cs)
{
	cs->len = 0;
}
...
static void parse_string(const char *spec, charset_t *cs, int target_len)
{
	...
	while (*p) {
		...
		/* Repeat [c*] or [c*N] */
		if (parse_repeat(&p, cs, target_len))
			continue;
...
int main(int argc, char *argv[])
{
	...
	while (argi < argc && argv[argi][0] == '-' && argv[argi][1]) {
		...
		if (strcmp(a, "--squeeze-repeats") == 0) {
			opt_squeeze = 1;
			argi++;
			continue;
		}
		if (strcmp(a, "--truncate-set1") == 0) {
			opt_truncate = 1;
			argi++;
			continue;
		}

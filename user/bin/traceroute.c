...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv,
				  "46dFf:g:ITi:m:N:np:t:l:w:q:rs:z:eAVUDP:M:Oh",
				  long_options, NULL)) != -1) {
		switch (opt) {
		case '4':
			break; /* IPv4 default */
		...
		case 'T':
			fprintf(stderr,
				"traceroute: TCP method not supported\n");
			return 1;
		case 'i':
			device = optarg;
			break;
		...
		case 'l':
			fprintf(stderr,
				"traceroute: IPv6 flow labels not supported\n");
			return 1;
		case 'w': {
		...
		case 'e':
			...
			break;
		case 'A':
			break; /* AS lookups - silently accepted */
		case 'V':
		...
		case 'D':
			...
			return 1;
		case 'P':
			break; /* raw protocol - accepted */
		case 'M':
			method = optarg;
			break;
		case 'O':
			break; /* method options */
		case 0x100: /* --sport */
			sport = atoi(optarg);
			break;
		case 0x101: /* --fwmark */
	...
	printf("traceroute to %s (%s), %d hops max, %d byte packets\n", host,
	       ip_to_str(dst_ip), max_ttl, packetlen);
	...
	for (int ttl = first_ttl; ttl <= max_ttl; ttl++) {
		...
		for (int q = 0; q < nqueries; q++) {
			...
			if (ret == 0) {
				...
				if (type ==
				    0) /* Echo reply - reached destination */
					reached = 1;
			} else {
				printf(" *");
			}

...
typedef struct ext4_super_block {
	...
	uint8_t s_pad_0198[0x270 -
			   0x198]; /* first/last-error records (unused) */
...
typedef struct ext4_inode {
	...
	packed)) ext4_inode; /* 160 bytes (covers 256-byte inode prefix) */
...
#define EXT4_XATTR_LEN(name_len)                \
	(((name_len) + EXT4_XATTR_ROUND +       \
	  (unsigned)sizeof(ext4_xattr_entry)) & \
	 ~EXT4_XATTR_ROUND)
...
_Static_assert(__builtin_offsetof(journal_superblock_t, s_maxlen) == 0x10,
	       "jsb s_maxlen");
_Static_assert(__builtin_offsetof(journal_superblock_t, s_sequence) == 0x18,
	       "jsb s_sequence");
_Static_assert(__builtin_offsetof(journal_superblock_t, s_start) == 0x1C,
	       "jsb s_start");
_Static_assert(__builtin_offsetof(journal_superblock_t, s_feature_incompat) ==
		       0x28,
	       "jsb incompat");

...
#define LPSS_PCI_PGCFG_ALL_GATE                            \
	(LPSS_PCI_PGCFG_PMCRE | LPSS_PCI_PGCFG_I3_ENABLE | \
	 LPSS_PCI_PGCFG_PGE | LPSS_PCI_PGCFG_SLEEP_EN | LPSS_PCI_PGCFG_HAE)
...
#define DW_IC_CON_BUS_CLEAR_CTRL \
	(1 << 11) // Preserve HW bus-clear when advertised
...
typedef struct __attribute__((packed)) {
	...
	uint16_t
		wMaxInputLength; // Max input report length (including 2-byte length prefix)
...
typedef struct {
	...
	volatile uint32_t
		last_abort_source; // Most recent TX_ABRT_SOURCE seen by transfer path
	...
	spinlock_t
		lock; // Protects I2C bus access (worker thread serialization)

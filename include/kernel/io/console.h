...
typedef struct {
	console_line_t *
		lines; // storage (pre-allocated, capacity CONSOLE_SCROLLBACK_LINES)
...
uint32_t console_get_sb_total(
	void); // Snapshot scrollback line count (for alt-screen save)
void console_restore_alt_screen(
	uint32_t saved_total); // Restore pre-alt-screen viewport
...
void console_insert_lines(int n, int top,
			  int bot); // IL:     scroll [cursor..bot] down by N
void console_delete_lines(int n, int top,
			  int bot); // DL:     scroll [cursor..bot] up by N
...
void console_scroll_region_up(int n, int top,
			      int bot); // SU:  scroll region [top..bot] up
void console_scroll_region_down(int n, int top,
				int bot); // SD:  scroll region [top..bot] down

...
struct percpu {
	...
	uint64_t
		syscall_kernel_rsp; // GS:8   — kernel RSP for current user task
	uint64_t
		syscall_user_rsp; // GS:16  — scratch: saves user RSP on syscall entry
	...
	uint64_t
		syscall_signal_pending; // GS:96  — signal number if pending, 0 otherwise
...
_Static_assert(__builtin_offsetof(percpu_t, self) == 0,
	       "percpu: self must be at offset 0");
_Static_assert(__builtin_offsetof(percpu_t, syscall_kernel_rsp) == 8,
	       "percpu: syscall_kernel_rsp must be at offset 8");
_Static_assert(__builtin_offsetof(percpu_t, syscall_user_rsp) == 16,
	       "percpu: syscall_user_rsp must be at offset 16");
_Static_assert(__builtin_offsetof(percpu_t, syscall_saved_user_rip) == 24,
	       "percpu: syscall_saved_user_rip must be at offset 24");
_Static_assert(__builtin_offsetof(percpu_t, syscall_saved_user_rflags) == 32,
	       "percpu: syscall_saved_user_rflags must be at offset 32");
_Static_assert(__builtin_offsetof(percpu_t, syscall_saved_user_rbp) == 40,
	       "percpu: syscall_saved_user_rbp must be at offset 40");
_Static_assert(__builtin_offsetof(percpu_t, syscall_saved_user_rbx) == 48,
	       "percpu: syscall_saved_user_rbx must be at offset 48");
_Static_assert(__builtin_offsetof(percpu_t, syscall_saved_user_r12) == 56,
	       "percpu: syscall_saved_user_r12 must be at offset 56");
_Static_assert(__builtin_offsetof(percpu_t, syscall_saved_user_r13) == 64,
	       "percpu: syscall_saved_user_r13 must be at offset 64");
_Static_assert(__builtin_offsetof(percpu_t, syscall_saved_user_r14) == 72,
	       "percpu: syscall_saved_user_r14 must be at offset 72");
_Static_assert(__builtin_offsetof(percpu_t, syscall_saved_user_r15) == 80,
	       "percpu: syscall_saved_user_r15 must be at offset 80");
_Static_assert(__builtin_offsetof(percpu_t, syscall_saved_user_rax) == 88,
	       "percpu: syscall_saved_user_rax must be at offset 88");
_Static_assert(__builtin_offsetof(percpu_t, syscall_signal_pending) == 96,
	       "percpu: syscall_signal_pending must be at offset 96");
_Static_assert(
...
static inline void write_kernel_gs_base(uint64_t base)
{
	...
	__asm__ volatile("wrmsr"
			 :
			 : "c"(MSR_KERNEL_GS_BASE), "a"(low), "d"(high));
}

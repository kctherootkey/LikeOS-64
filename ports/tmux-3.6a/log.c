...
log_open(const char *name)
{
	...
	xasprintf(&path, "/tmp/tmux-%s-%ld.log", name, (long)getpid());
	log_file = fopen(path, "a");
	free(path);
	if (log_file == NULL)
		return;

	/* LikeOS: fully-buffered I/O; per-line fflush would be too slow on FAT32. */
...
log_vwrite(const char *msg, va_list ap, const char *prefix)
{
	...
	(void)fprintf(log_file, "%lld.%06d %s%s\n", (long long)tv.tv_sec,
	    (int)tv.tv_usec, prefix, out);
	free(out);
}
...
fatal(const char *msg, ...)
{
	...
	/* LikeOS: always record fatals to a known file regardless of log_level. */
	ef = fopen("/tmp/tmux-fatal.log", "a");
	if (ef != NULL) {
		va_start(ap, msg);
		fprintf(ef, "[pid %ld] %s", (long)getpid(), tmp);
		vfprintf(ef, msg, ap);
		fputc('\n', ef);
		va_end(ap);
		fclose(ef);
...
fatalx(const char *msg, ...)
{
	...
	if (ef != NULL) {
		va_start(ap, msg);
		fprintf(ef, "[pid %ld] fatalx: ", (long)getpid());
		vfprintf(ef, msg, ap);
		fputc('\n', ef);
		va_end(ap);
		fclose(ef);

...
static void ic_evict_one(void)
{
	...
	if (victim == &ic_lru_sentinel) {
		WARN_ON_ONCE(
			1); /* eviction called on empty LRU - cache accounting bug */
	...
	BUG_ON(victim->refcount >
	       0); /* evicting inode still in use: LRU contains a referenced inode, data corruption will follow */
		       ...
		       uint16_t wrt_time, uint16_t wrt_date)
{
	...
	WARN_ON(start_cluster ==
		1); /* block-id 1 is reserved in FAT32; treated as invalid generically */
	if (!ic_initialized || start_cluster < 2)
		return 0;
	...
	while (n) {
		if (n->start_cluster == start_cluster) {
			...
			WARN_ON_ONCE(
				n->refcount <
				0); /* negative refcount before icache_get bump: use-after-free or icache_unref overcounted */
...
void icache_ref(ic_inode_t *inode)
{
	...
	WARN_ON(old <
		0); /* refcount was negative - use-after-free or corruption */
...
void icache_unref(ic_inode_t *inode)
{
	...
	if (new_rc < 0) {
		WARN(1, "icache_unref: refcount underflow on start_cluster=%lu",
		     inode->start_cluster);
		inode->refcount = 0;
		new_rc = 0;
	}
			   ...
			   vfs_superblock_t *sb)
{
	...
	if (inode->chain_len == 0) {
		if (!inode->chain) {
			...
			inode->chain = (unsigned long *)kalloc(
	...
	while (inode->chain_len <= target_idx) {
		...
		if (inode->chain_len >= inode->chain_cap) {
			...
			if (new_cap <= inode->chain_cap)
				return 0; // can't grow
			unsigned long *new_arr = (unsigned long *)kalloc(
				new_cap * sizeof(unsigned long));
			if (!new_arr)
				return 0;

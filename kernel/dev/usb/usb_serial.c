			      ...
			      uint16_t value, uint8_t *buf)
{
	return xhci_control_transfer(
		ctrl, dev, USB_RT_D2H | USB_RT_VENDOR | USB_RT_DEV,
		PL2303_VENDOR_READ_REQUEST, value, 0, 1, buf);
}
...
{
	return xhci_control_transfer(
		ctrl, dev, USB_RT_H2D | USB_RT_VENDOR | USB_RT_DEV,
		PL2303_VENDOR_WRITE_REQUEST, value, index, 0, NULL);
}
...
static int pl2303_init_device(xhci_controller_t *ctrl, usb_device_t *dev)
{
	...
	if (xhci_control_transfer(
		    ctrl, dev, USB_RT_H2D | USB_RT_CLASS | USB_RT_IFACE,
		    PL2303_SET_CONTROL_LINE_STATE,
		    PL2303_CTRL_DTR | PL2303_CTRL_RTS, 0, 0, NULL) != ST_OK) {
				   ...
				   usbserial_match_t *match)
{
	...
	while (ptr + 1 < end) {
		...
		if (type == USB_DESC_INTERFACE &&
			...
			if (iface->class_code == USB_CLASS_COMMUNICATIONS &&
			    first_comm == 0xFF) {
				first_comm = iface->interface_num;
			}
			if (iface->class_code == USB_CLASS_CDC_DATA &&
			    first_data == 0xFF) {
				first_data = iface->interface_num;
			}
		} else if (type == USB_CDC_DESC_CS_INTERFACE && len >= 3 &&
			   current_iface != 0xFF) {
			if (ptr[2] == USB_CDC_DESC_SUBTYPE_ACM &&
			    current_class == USB_CLASS_COMMUNICATIONS) {
				acm_iface = current_iface;
			} else if (ptr[2] == USB_CDC_DESC_SUBTYPE_UNION &&
				   len >= 5) {
				union_master = ptr[3];
				union_slave = ptr[4];
			}
	...
	while (ptr + 1 < end) {
		...
		if (type == USB_DESC_INTERFACE &&
			...
			if (current_iface == match->data_if &&
...
static int usbserial_send_control_setup(xhci_controller_t *ctrl,
					usb_device_t *dev, uint8_t comm_if)
{
	...
	if (xhci_control_transfer(ctrl, dev,
				  USB_RT_H2D | USB_RT_CLASS | USB_RT_IFACE,
				  USB_CDC_REQ_SET_LINE_CODING, 0, comm_if,
	...
	if (xhci_control_transfer(ctrl, dev,
				  USB_RT_H2D | USB_RT_CLASS | USB_RT_IFACE,
				  USB_CDC_REQ_SET_CONTROL_LINE_STATE, 0x0003,
				  comm_if, 0, NULL) != ST_OK) {
		return ST_ERR;
	}
			       ...
			       const char *data, uint32_t len)
{
	...
	while (offset < len) {
		...
		if (xhci_bulk_transfer_out(ctrl, dev, data + offset, chunk,
					   &transferred) != ST_OK) {
			return ST_IO;
		}
	...
	for (uint32_t index = 0; index < len; ++index) {
		...
		if (ch == '\n') {
			if (txlen == sizeof(txbuf)) {
				if (usbserial_write_raw(ctrl, dev, txbuf,
							txlen) != ST_OK) {
					return ST_IO;
				}
				txlen = 0;
		...
		if (txlen == sizeof(txbuf)) {
			if (usbserial_write_raw(ctrl, dev, txbuf, txlen) !=
			    ST_OK) {
				return ST_IO;
			}
			txlen = 0;
		...
		txbuf[txlen++] = ch;
		WARN_ON(txlen >
			sizeof(txbuf)); /* txbuf overflow: txlen exceeded txbuf capacity */
	}
...
static void usbserial_send_existing_klog(xhci_controller_t *ctrl,
					 usb_device_t *dev)
{
	int size = klog_size();
	if (size <= 0) {
		    ...
		    uint8_t *config_desc, uint16_t config_len)
{
	...
	if (!ctrl || !dev || !config_desc ||
	    config_len < sizeof(usb_config_desc_t)) {
		return ST_INVALID;
	}
	...
	// Try CDC ACM
	if (usbserial_parse_cdc_acm(config_desc, config_len, &match) != ST_OK ||
	    !match.valid) {
		return ST_UNSUPPORTED;
	}
...
void usbserial_log_write(const char *data, uint32_t len)
{
	...
	spin_lock_irqsave(&g_usbserial_lock, &flags);
	if (!g_usbserial.active || !g_usbserial.mirror_enabled ||
	    g_usbserial.write_busy) {
		spin_unlock_irqrestore(&g_usbserial_lock, flags);
		return;
	}

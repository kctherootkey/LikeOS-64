...
int ps2_init(void)
{
	...
	WARN_ON_ONCE(
		status &
		STR_OBF); /* PS/2 output buffer not empty at start of init: stale data from firmware */
	...
	if (ioapic_configure_legacy_irq(1, KBD_VECTOR, IOAPIC_POLARITY_HIGH,
	...
	if (ioapic_configure_legacy_irq(12, AUX_VECTOR, IOAPIC_POLARITY_HIGH,
					IOAPIC_TRIGGER_EDGE) == 0) {
		irq_disable(12);
	} else {
		irq_enable(12);

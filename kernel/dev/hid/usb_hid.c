...
{
	if (len < HID_BOOT_KBD_REPORT_SIZE)
		return;
	...
	uint8_t shift_pressed =
		(modifiers & (HID_KBD_MOD_LSHIFT | HID_KBD_MOD_RSHIFT)) ? 1 : 0;
	uint8_t prev_shift =
		(prev_modifiers & (HID_KBD_MOD_LSHIFT | HID_KBD_MOD_RSHIFT)) ?
			1 :
			0;
	...
	uint8_t ctrl_pressed =
		(modifiers & (HID_KBD_MOD_LCTRL | HID_KBD_MOD_RCTRL)) ? 1 : 0;
	uint8_t prev_ctrl =
		(prev_modifiers & (HID_KBD_MOD_LCTRL | HID_KBD_MOD_RCTRL)) ? 1 :
									     0;
	...
	uint8_t alt_pressed =
		(modifiers & (HID_KBD_MOD_LALT | HID_KBD_MOD_RALT)) ? 1 : 0;
	uint8_t prev_alt =
		(prev_modifiers & (HID_KBD_MOD_LALT | HID_KBD_MOD_RALT)) ? 1 :
									   0;
	...
	for (int i = 0; i < HID_BOOT_KBD_MAX_KEYS; i++) {
		...
		if (prev_key == 0 || prev_key >= 0x04) {
			...
			if (!still_pressed &&
			    prev_key < sizeof(hid_to_scancode)) {
	...
	for (int i = 0; i < HID_BOOT_KBD_MAX_KEYS; i++) {
		uint8_t key = keys[i];
		if (key == 0 || key <= HID_KEY_ERR_UNDEFINED)
			continue;
		...
		if (!was_pressed) {
			...
			if (key < sizeof(hid_to_scancode)) {
				...
				if (sc != 0) {
					...
					char ch = scan_code_to_ascii(
						sc, shift_pressed);
					if (ch) {
						if (alt_pressed) {
							tty_input_char(
								tty, 27,
								0); // ESC prefix for Alt
						}
						tty_input_char(tty, ch,
							       ctrl_pressed);
					}
				}
			}
...
{
	if (!hdev || !hdev->int_in_ring || !hdev->report_buf)
		return ST_INVALID;
	...
	WARN_ON(slot == 0 ||
		slot > XHCI_MAX_SLOTS); /* slot 0 reserved; valid range is 1..XHCI_MAX_SLOTS */
	WARN_ON(hdev->int_in_ep ==
		0); /* EP 0 is the control endpoint — interrupt IN cannot use EP 0 */
	WARN_ON(dci >=
		XHCI_MAX_ENDPOINTS); /* DCI out of range for pending_xfer array */
	WARN_ON(max_pkt ==
		0); /* zero max_pkt: TRB_LEN(0) queues a 0-byte transfer, device will NAK forever */
	...
	uint32_t control =
		(TRB_TYPE_NORMAL << 10) | TRB_FLAG_IOC | TRB_FLAG_ISP;
...
static void hid_resubmit_irq(usbhid_device_t *hdev)
{
	...
	uint16_t max_pkt = hdev->int_in_max_pkt;
	WARN_ON(slot == 0 ||
		slot > XHCI_MAX_SLOTS); /* invalid slot in IRQ resubmit path */
	WARN_ON(hdev->int_in_ep ==
		0); /* EP 0 is control — must not resubmit interrupt IN on EP 0 */
			  ...
			  uint8_t cc, uint32_t residue)
{
	...
	for (int i = 0; i < g_hid_device_count; i++) {
		...
		if (!h->active || !h->configured)
			continue;
		if (h->ctrl != ctrl)
			continue;
		if (h->usb_dev->slot_id != slot)
			continue;
	...
	if (cc == TRB_CC_SUCCESS || cc == TRB_CC_SHORT_PACKET) {
		...
		WARN_ON(received >
			hdev->int_in_max_pkt); /* residue > max_pkt: xHCI reported more bytes than the endpoint maximum */
		if (received > USBHID_REPORT_BUF_SIZE)
			received = USBHID_REPORT_BUF_SIZE;
		...
		if (hdev->type == USBHID_TYPE_KEYBOARD &&
		    received >= HID_BOOT_KBD_REPORT_SIZE) {
			hid_process_keyboard_report(hdev, hdev->report_buf,
						    (uint8_t)received);
		} else if (hdev->type == USBHID_TYPE_MOUSE &&
			   received >= HID_BOOT_MOUSE_MIN_SIZE) {
			hid_process_mouse_report(hdev, hdev->report_buf,
						 (uint8_t)received);
		}
...
static int hid_set_idle(xhci_controller_t *ctrl, usb_device_t *dev,
			uint8_t interface_num, uint8_t duration,
			uint8_t report_id)
{
...
static int hid_setup_interrupt_endpoint(usbhid_device_t *hdev)
{
	...
	WARN_ON(dci >=
		32); /* DCI >= 32 makes (1 << dci) shift undefined behaviour and corrupts the input context add-flags bitmap */
	...
	uint8_t *ep_ptr =
		ctrl->input_ctx_raw + (2 + dci - 1) * ctrl->context_size;
	...
	if (xhci_send_command(ctrl, ctrl->input_ctx_phys, 0, cmd_control) !=
	    ST_OK) {
...
{
	if (vendor_id != INTEL_ISH_PCI_VENDOR)
		return 0;
		 ...
		 uint8_t *config_desc, uint16_t config_len)
{
	...
	if (!ctrl || !dev || !config_desc || config_len < 9)
		return ST_INVALID;
	if (!g_hid_initialized)
		usbhid_init();
	...
	if (end > config_desc + config_len)
		end = config_desc + config_len;
		...
		if (desc_type == USB_DESC_INTERFACE) {
			...
			if (cur_iface->class_code == USB_CLASS_HID) {
				hid_dbg("Found HID interface %d: subclass=%d protocol=%d eps=%d\n",
					...
					cur_iface->subclass,
					cur_iface->protocol,
					cur_iface->num_endpoints);

				// We support Boot Protocol keyboards and mice
				if (cur_iface->subclass ==
					...
					usbhid_device_t *hdev =
						&g_hid_devices
							[g_hid_device_count];
					hid_memset(hdev, 0,
						   sizeof(usbhid_device_t));
					...
					if (cur_iface->protocol ==
					    USB_HID_PROTOCOL_KEYBOARD) {
						hdev->type =
							USBHID_TYPE_KEYBOARD;
						hdev->report_size =
							HID_BOOT_KBD_REPORT_SIZE;
					} else {
					...
					if (hid_alloc_report_buffer(hdev) !=
					    ST_OK) {
						hid_memset(
							hdev, 0,
							sizeof(usbhid_device_t));
...
void usbhid_poll(void)
{
	...
	for (int i = 0; i < g_hid_device_count; i++) {
		...
		if (!hdev->active || !hdev->configured)
			continue;
		...
		if (hdev->xfer_pending && hdev->xfer.completed) {
			...
			if (cc == TRB_CC_SUCCESS || cc == TRB_CC_SHORT_PACKET) {
				...
				if (hdev->type == USBHID_TYPE_KEYBOARD &&
				    received >= HID_BOOT_KBD_REPORT_SIZE) {
					hid_process_keyboard_report(
						hdev, hdev->report_buf,
						(uint8_t)received);
				} else if (hdev->type == USBHID_TYPE_MOUSE &&
					   received >=
						   HID_BOOT_MOUSE_MIN_SIZE) {
					hid_process_mouse_report(
						hdev, hdev->report_buf,
						(uint8_t)received);
				}
			} else if (cc == TRB_CC_STALL) {
				...
				xhci_control_transfer(
					hdev->ctrl, hdev->usb_dev,
					USB_RT_H2D | USB_RT_STD | USB_RT_EP,
					USB_REQ_CLEAR_FEATURE, 0,
					(hdev->int_in_ep | USB_EP_DIR_IN), 0,
					NULL);
			} else {
...
{
	for (int i = 0; i < g_hid_device_count; i++) {
		if (g_hid_devices[i].active &&
		    g_hid_devices[i].type == USBHID_TYPE_KEYBOARD) {
			return 1;
		}
	}
	return 0;
...
int usbhid_has_mouse(void)
{
	for (int i = 0; i < g_hid_device_count; i++) {
		if (g_hid_devices[i].active &&
		    g_hid_devices[i].type == USBHID_TYPE_MOUSE) {
			return 1;
		}
	}
	return 0;

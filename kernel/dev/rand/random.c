...
static void pool_rekey(void)
{
	...
	for (int i = 0; i < 8; i++)
		g_pool.key[i] = output[i];
	for (int i = 0; i < 3; i++)
		g_pool.nonce[i] = output[8 + i];
	g_pool.counter++;
	g_pool.reseed_count++;
}
...
void random_add_entropy(const void *data, size_t len)
{
	...
	if (g_pool.entropy_bits > 4096)
		g_pool.entropy_bits = 4096;
...
int random_get_bytes(void *buf, size_t len, int blocking)
{
	...
	if (blocking) {
		...
		while (g_pool.entropy_bits < (int)(len * 8) &&
		       g_pool.entropy_bits < 256) {
			spin_unlock_irqrestore(&g_pool.lock, flags);
...
__no_stack_protector uint64_t generate_stack_canary(void)
{
	uint64_t canary = 0;
...
apply_mask:
	...
	WARN_ON(canary ==
		0); /* generate_stack_canary returned zero - entropy source failed */
	WARN_ON(canary &

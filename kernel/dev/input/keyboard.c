...
{
	BUILD_BUG_ON(
		sizeof(scan_code_to_ascii_table) !=
		sizeof(scan_code_to_ascii_shifted)); /* both scan tables must be same size */
...
void keyboard_buffer_add(uint8_t scan_code)
{
	...
	if (kb_state.buffer_count < KEYBOARD_BUFFER_SIZE) {
		...
		kb_state.buffer_end =
			(kb_state.buffer_end + 1) % KEYBOARD_BUFFER_SIZE;
...
uint8_t keyboard_buffer_get(void)
{
	...
	if (kb_state.buffer_count > 0) {
		...
		kb_state.buffer_start =
			(kb_state.buffer_start + 1) % KEYBOARD_BUFFER_SIZE;
		kb_state.buffer_count--;
	}
	spin_unlock_irqrestore(&kb_lock, flags);
	return scan_code;
...
void keyboard_irq_handler(void)
{
	...
	// F1-F4 use SS3 format (\033OP..\033OS), F5-F12 use CSI format (\033[N~).
	{
		...
		if (fnum) {
			WARN_ON(fnum < 1 ||
				fnum > 12); /* fnum out of F1-F12 range */
			tty_t *tty = tty_get_console();
			...
			if (fnum <= 4) {
				...
				static const char *fseq[] = { "15", "17", "18",
							      "19", "20", "21",
							      "23", "24" };

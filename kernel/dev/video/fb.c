// LikeOS Framebuffer Optimization System - Implementation
...
static uint32_t g_static_back_buffer[MAX_STATIC_FB_SIZE / sizeof(uint32_t)]
...
int fb_optimize_init(framebuffer_info_t *fb_info)
{
	...
	size_t buffer_size = g_double_buffer.height * g_double_buffer.pitch *
			     sizeof(uint32_t);
#if BOOT_DEBUG
	...
	if (buffer_size <= MAX_STATIC_FB_SIZE) {
		...
		kprintf("  Using static back buffer %p (size fits: %zu <= %zu)\n",
			g_static_back_buffer, buffer_size,
			(size_t)MAX_STATIC_FB_SIZE);
#endif
	} else {
	...
	fast_memcpy(g_double_buffer.back_buffer, g_double_buffer.front_buffer,
		    buffer_size);
	g_initialized = 1;
#if BOOT_DEBUG
...
static int merge_dirty_regions(void)
{
	...
	for (uint32_t i = 0; i < g_double_buffer.num_dirty_regions; i++) {
		...
		for (uint32_t j = i + 1; j < g_double_buffer.num_dirty_regions;
		     j++) {
	...
	if (merges > 0) {
		...
		for (uint32_t read_idx = 0;
		     read_idx < g_double_buffer.num_dirty_regions; read_idx++) {
			if (g_double_buffer.dirty_regions[read_idx].dirty) {
				if (write_idx != read_idx) {
					g_double_buffer
						.dirty_regions[write_idx] =
						g_double_buffer.dirty_regions
							[read_idx];
				}
				write_idx++;
			}
...
static void copy_region_to_front(uint32_t x1, uint32_t y1, uint32_t x2,
				 uint32_t y2)
{
	WARN_ON(x2 >=
		g_double_buffer
			.width); /* dirty region right edge past framebuffer width */
	WARN_ON(y2 >=
		g_double_buffer
			.height); /* dirty region bottom edge past framebuffer height */
...
void fb_mark_dirty_unlocked(uint32_t x1, uint32_t y1, uint32_t x2, uint32_t y2)
{
	...
	if (g_double_buffer.num_dirty_regions >=
	    g_double_buffer.max_dirty_regions) {
		g_double_buffer.full_screen_dirty = 1;
		return;
	}
	...
	dirty_rect_t *region =
		&g_double_buffer
			 .dirty_regions[g_double_buffer.num_dirty_regions];
	region->x1 = x1;
	region->y1 = y1;
	region->x2 = x2;
	region->y2 = y2;
...
void fb_flush_dirty_regions_unlocked(void)
{
	...
	for (uint32_t i = 0; i < g_double_buffer.num_dirty_regions; i++) {
		...
		if (region->dirty) {
			...
			if (x1 >= g_double_buffer.width) {
				x1 = g_double_buffer.width ?
					     g_double_buffer.width - 1 :
					     0;
			}
			if (y1 >= g_double_buffer.height) {
				y1 = g_double_buffer.height ?
					     g_double_buffer.height - 1 :
					     0;
			}
			if (x2 >= g_double_buffer.width) {
				x2 = g_double_buffer.width ?
					     g_double_buffer.width - 1 :
					     0;
			}
			if (y2 >= g_double_buffer.height) {
				y2 = g_double_buffer.height ?
					     g_double_buffer.height - 1 :
					     0;
			}
			if (x1 > x2 || y1 > y2) {
			...
			uint64_t max_index =
				(uint64_t)(g_double_buffer.height - 1) *
					g_double_buffer.pitch +
				(g_double_buffer.width - 1);
			uint64_t start_index =
				(uint64_t)y1 * g_double_buffer.pitch + x1;
			uint64_t end_index =
				(uint64_t)y2 * g_double_buffer.pitch + x2;
		  ...
		  uint32_t src_y, uint32_t width, uint32_t height)
{
	...
	if (dst_y > src_y || (dst_y == src_y && dst_x > src_x)) {
		...
		for (int32_t y = height - 1; y >= 0; y--) {
			uint32_t src_offset =
				(src_y + y) * g_double_buffer.pitch + src_x;
			uint32_t dst_offset =
				(dst_y + y) * g_double_buffer.pitch + dst_x;
			// Use memmove-like behavior for overlapping copies
			for (int32_t x = width - 1; x >= 0; x--) {
				g_double_buffer.back_buffer[dst_offset + x] =
					g_double_buffer
						.back_buffer[src_offset + x];
			}
		}
	} else {
...
void fb_print_performance_stats(void)
{
	...
	if (g_double_buffer.total_updates > 0) {
		uint64_t avg_pixels = g_double_buffer.pixels_copied /
				      g_double_buffer.total_updates;
		uint64_t total_pixels =
			g_double_buffer.width * g_double_buffer.height;
		uint32_t efficiency =
			(uint32_t)((avg_pixels * 100) / total_pixels);
...
int configure_write_combining_mtrr(uint64_t fb_base, uint64_t fb_size)
{
	...
	for (uint32_t i = 0; i < num_var_mtrrs; i++) {
		...
		if (!(mask &
		      (1ULL
		       << 11))) { // Check if MTRR is available (valid bit clear)
			mtrr_index = i;
			break;
		}
	}
	...
	__asm__ volatile("cpuid"
			 : "=a"(eax_mp), "=b"(ebx_mp), "=c"(ecx_mp),
			   "=d"(edx_mp)
			 : "a"(0x80000008));
	uint32_t maxphyaddr = eax_mp & 0xFF;
	if (maxphyaddr < 36)
		maxphyaddr = 36; // Minimum sane value
	uint64_t phys_base = mtrr_base | MTRR_TYPE_WC;
	uint64_t phys_mask =
		(~(mtrr_size - 1) & ((1ULL << maxphyaddr) - 1)) | (1ULL << 11);
...
void fb_pat_program_wc_this_cpu(void)
{
	...
	__asm__ volatile(
		"mov %0, %%cr0" ::"r"(cr0 | (1ULL << 30))); // Set CD bit
...
int configure_pat_write_combining(uint64_t fb_phys_base, uint64_t fb_size)
{
	...
	uint64_t end_2mb =
		(fb_phys_base + fb_size + 0x1FFFFFULL) & ~0x1FFFFFULL;
	...
	for (uint64_t addr = start_2mb; addr < end_2mb; addr += 0x200000ULL) {
		...
		pd[pd_idx] = (pd[pd_idx] & ~((1ULL << 4) | (1ULL << 12))) |
			     (1ULL << 3);
		pages_modified++;
	}
		       ...
		       uint32_t fg_color, uint32_t bg_color)
{
	...
	// Bounds check
	if (x + font_w > scr_w || y + font_h > scr_h)
		return;
	...
	if (bytes_per_row == 1 && font_w == 8) {
		...
		for (uint32_t row = 0; row < font_h; row++) {
			...
			for (uint32_t col = 0; col < font_w; col++) {
				uint32_t byte_idx =
					row * bytes_per_row + (col / 8);
				uint8_t bit_mask = 0x80 >> (col % 8);
				dst[col] = (glyph[byte_idx] & bit_mask) ?
						   fg_color :
						   bg_color;
			}
		}

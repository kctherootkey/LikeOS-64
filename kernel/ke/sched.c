...
static inline void switch_address_space(task_t *prev, task_t *next)
{
	BUG_ON(next == NULL);
	WARN_ON_ONCE(
		next->privilege == TASK_USER &&
		next->kernel_stack_top ==
			0); /* user task has no kernel stack: syscall/interrupt would overwrite user stack */
...
static void task_list_remove(task_t *t)
{
	...
	for (task_t *prev = g_task_list_head; prev; prev = prev->next) {
		if (prev->next == t) {
			prev->next = t->next;
			t->next = NULL;
			__atomic_fetch_sub(&g_task_list_count, 1,
					   __ATOMIC_RELAXED);
			return;
		}
	}
...
static void rq_enqueue_locked(percpu_t *cpu, task_t *task)
{
	...
	if (task->state == TASK_RUNNING) {
		WARN_ON(task->state ==
			TASK_RUNNING); /* enqueuing RUNNING task: CPU accounting bug */
...
static task_t *rq_dequeue_locked(percpu_t *cpu)
{
	...
	if (task) {
		...
		if (cpu->runqueue_head == task) {
			WARN_ON_ONCE(
				cpu->runqueue_head ==
				task); /* dequeued task still at runqueue_head: circular rq_next self-link */
...
__attribute__((no_stack_protector)) void sched_schedule(void)
{
	...
	__asm__ volatile(
		"" ::
			: "memory"); // Compiler barrier — same-CPU store ordering is guaranteed on x86
...
{
	static const char *state_names[] = { "READY", "RUN", "BLOCK", "STOP",
					     "ZOMBIE" };
	tty_printf(tty, "\n=== Debug Dump (Ctrl+D) ===\n");
	tty_printf(tty,
		   "Ticks: %llu  IRQ0: %llu  TotalIRQ: %llu  Schedules: %llu\n",
		   timer_ticks(), g_irq0_count, g_total_irq_count,
		   g_total_schedules);
	...
	int snap_cap =
		__atomic_load_n(&g_task_list_count, __ATOMIC_RELAXED) + 16;
	struct {
	...
	spin_lock_irqsave(&g_task_list_lock, &flags);
	task_t *cur = sched_current();
	for (task_t *t = g_task_list_head; t && nsnaps < snap_cap;
	     t = t->next) {
		...
		snaps[nsnaps].last_rip =
			t->preempt_frame ? t->preempt_frame->rip : 0;
	...
	for (int i = 0; i < nsnaps; i++) {
		const char *sn = (snaps[i].state <= TASK_ZOMBIE) ?
					 state_names[snaps[i].state] :
					 "???";
		tty_printf(
			tty,
     ...
     * (e.g., a task killed while holding the mutex). */
	{
		...
		tty_printf(tty, "ext4 io_lock: held=%d depth=%d owner=%lld\n",
			   ext4_io_locked, ext4_io_depth,
			   (long long)ext4_io_owner);
	}
     ...
     * msd->io_locked; if a task dies mid-write both can orphan. */
	{
		...
		for (int mi = 0; mi < g_msd_count && mi < 8; mi++) {
			...
			if (m) {
				tty_printf(
					tty,
					"MSD[%d] io_lock: held=%d owner=%lld\n",
					mi, m->io_locked,
					(long long)m->io_owner);
			}
		}
	}
	slab_free(snaps);
...
{
	static const char *snames[] = { "READY", "RUN", "BLOCK", "STOP",
					"ZOMBIE" };
	...
	while (t && total < 256) {
		...
		kprintf("  [%d] tgid=%d %-12s %-8s cpu=%u rip=%016llx\n", t->id,
			t->tgid, t->comm[0] ? t->comm : "(anon)", s, t->on_cpu,
			t->preempt_frame ? t->preempt_frame->rip :
					   t->syscall_rip);
		t = t->next;
		total++;
	}
...
void sched_enable_smp(void)
{
	...
	for (task_t *t = g_task_list_head; t; t = t->next) {
		if (t->state == TASK_READY && !is_idle_task(t) &&
		    !is_bootstrap_task(t)) {
			t->on_cpu = 0;
			uint64_t flags;
...
{
	if (cpu_id == 0 || cpu_id >= MAX_AP_IDLE)
		return;
	...
	if (!idle) {
		mm_free_guarded_kstack(g_ap_idle_stacks[cpu_id],
				       KERNEL_STACK_SIZE);
...
void sched_load_balance(void)
{
	...
	if (busiest_cpu == my_cpu || busiest_len < my_len + 1)
		return;
...
void thread_group_remove(task_t *thread)
{
	...
	thread->thread_group_prev->thread_group_next =
		thread->thread_group_next;
	thread->thread_group_next->thread_group_prev =
		thread->thread_group_prev;
...
mm_struct_t *mm_struct_clone(mm_struct_t *src)
{
	...
	if (src->mmap_regions && src->mmap_count > 0) {
		mm->mmap_regions = (mmap_region_t *)kalloc(
			src->mmap_capacity * sizeof(mmap_region_t));
		if (mm->mmap_regions) {
...
files_struct_t *files_struct_create(void)
{
	files_struct_t *files =
		(files_struct_t *)kalloc(sizeof(files_struct_t));
	if (!files)
		return NULL;
...
sighand_struct_t *sighand_struct_create(void)
{
	sighand_struct_t *sighand =
		(sighand_struct_t *)kalloc(sizeof(sighand_struct_t));
	if (!sighand)
		return NULL;
			       ...
			       unsigned long active)
{
	...
	newload =
		load * exp + active * LOADAVG_FIXED_1 * (LOADAVG_FIXED_1 - exp);
...
void sched_calc_load(void)
{
	...
	g_loadavg[0] = calc_load(g_loadavg[0], LOADAVG_EXP_1,
				 (unsigned long)nr_active);
	g_loadavg[1] = calc_load(g_loadavg[1], LOADAVG_EXP_5,
				 (unsigned long)nr_active);

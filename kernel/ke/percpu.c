...
__no_stack_protector void percpu_init(void)
{
...
__no_stack_protector void percpu_init_cpu(uint32_t cpu_id, uint32_t apic_id)
{
	...
	percpu->self = percpu;
	BUG_ON((uint64_t)percpu &
	       (PAGE_SIZE - 1)); /* percpu struct must be page-aligned */

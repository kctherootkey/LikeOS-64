...
static inline uint16_t acl_le16(const uint8_t *p) { return (uint16_t)(p[0] | (p[1] << 8)); }
                    ...
                    uint32_t fuid, uint32_t fgid, int want, int use_real)
{
    ...
    if (cuid == 0) return 1;                  /* root: not restricted by the ACL */
    if (!acl || len < 4) return 1;
    const uint8_t *p = (const uint8_t *)acl;
    if (acl_le32(p) != ACL_EA_VERSION) return 1;
    p += 4; len -= 4;
    ...
    for (const uint8_t *q = p; (q + 4) <= (p + len); ) {
        ...
        if (q + esz > p + len) break;
        if (tag == ACL_MASK) mask = acl_le16(q + 2) & 7;
        q += esz;
    }
    ...
    while (len >= 4) {
        ...
        case ACL_USER:
            if (cuid == id)
                return ((perm & mask & want) == want) ? 0 : -EACCES;
            break;
        case ACL_GROUP_OBJ:
            if (acl_in_group(c, fgid, cgid)) {
                found_group = 1;
                if ((perm & mask & want) == want) group_ok = 1;
            }
            break;
        case ACL_GROUP:
            ...
            break;
        case ACL_OTHER:
            if (found_group) return group_ok ? 0 : -EACCES;
            return ((perm & want) == want) ? 0 : -EACCES;
        default: break;                       /* ACL_MASK / unknown: skip */
        }
        p += esz; len -= esz;
    }

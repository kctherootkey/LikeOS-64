...
static uint64_t dhcp_lease_start_ticks = 0; // ticks; divided by hz on use
			     ...
			     uint8_t *buf, int buflen)
{
	BUG_ON(dev == NULL);
	BUG_ON(buf == NULL);
	if (buflen < DHCP_FIXED_HDR + 4)
		return -1; // need at least header + minimal options
			       ...
			       const uint8_t **classless, int *classless_len)
{
	...
	while (i < optlen) {
		...
		case DHCP_OPT_SERVER_ID:
			if (len >= 4)
				*server_id = ((uint32_t)options[i] << 24) |
					     ((uint32_t)options[i + 1] << 16) |
					     ((uint32_t)options[i + 2] << 8) |
					     options[i + 3];
			break;
...
void dhcp_rx(net_device_t *dev, const uint8_t *data, uint16_t len)
{
	...
	if (dhcp->op != DHCP_BOOTREPLY)
		return;
	if (net_ntohl(dhcp->xid) != dhcp_xid)
		return;
	if (net_ntohl(dhcp->magic_cookie) != DHCP_MAGIC_COOKIE)
		return;
	...
	BUILD_BUG_ON(DHCP_MAGIC_COOKIE !=
		     0x63825363u); /* RFC 2131 magic cookie constant */
	uint8_t msg_type = 0;
	...
	dhcp_parse_options(dhcp->options, optlen, &msg_type, &subnet, &router,
			   &dns, &server_id, &lease, &t1, &t2, &classless,
			   &classless_len);
	...
	case DHCP_STATE_DISCOVERING:
		if (msg_type == DHCP_OFFER && offered != 0) {
			...
			uint8_t pkt[576];
			int pktlen = dhcp_build_packet(
				dev, DHCP_REQUEST, dhcp_offered_ip,
	...
	case DHCP_STATE_REBINDING:
		if (msg_type == DHCP_ACK && offered != 0) {
			...
			kprintf(" mask %d.%d.%d.%d", (subnet >> 24) & 0xFF,
				(subnet >> 16) & 0xFF, (subnet >> 8) & 0xFF,
				subnet & 0xFF);
			...
			if (classless && classless_len > 0) {
				...
				while (i < classless_len) {
					...
					if (i + obytes + 4 > classless_len)
						break;
					uint32_t dst = 0;
					for (uint8_t k = 0; k < obytes; k++)
						dst |= ((uint32_t)classless[i +
									    k])
						       << (24 - 8 * k);
					i += obytes;
					uint32_t gw =
						((uint32_t)classless[i] << 24) |
						((uint32_t)classless[i + 1]
						 << 16) |
						((uint32_t)classless[i + 2]
						 << 8) |
						classless[i + 3];
					i += 4;
					uint32_t mask =
					...
					route_add(dst, mask, gw, dev, 0,
						  RTF_UP | (gw ? RTF_GATEWAY :
								 0));
				}
			} else if (router != 0) {
				route_add(0, 0, router, dev, 0,
					  RTF_UP | RTF_GATEWAY);
			}
...
int dhcp_discover(net_device_t *dev)
{
	...
	uint8_t pkt[576];
	int pktlen =
		dhcp_build_packet(dev, DHCP_DISCOVER, 0, 0, pkt, sizeof(pkt));
	if (pktlen > 0) {
...
int dhcp_release(net_device_t *dev)
{
	...
	if (pktlen > 0) {
		...
		udp_send(dev, dhcp_server_ip, DHCP_CLIENT_PORT,
			 DHCP_SERVER_PORT, pkt, (uint16_t)pktlen);
	}
...
int dhcp_renew(net_device_t *dev)
{
	...
	dhcp_state = DHCP_STATE_REQUESTING;
	uint8_t pkt[576];
	int pktlen = dhcp_build_packet(dev, DHCP_REQUEST, dhcp_offered_ip,
				       dhcp_server_ip, pkt, sizeof(pkt));
	if (pktlen > 0) {
...
void dhcp_tick(void)
{
	if (dhcp_state != DHCP_STATE_BOUND &&
		...
		return;
	if (!dhcp_bound_dev || dhcp_lease_seconds == 0)
		return;
	...
	if (elapsed >= dhcp_t2_seconds && dhcp_state != DHCP_STATE_REBINDING) {
		...
		if (n > 0)
			dhcp_send(dhcp_bound_dev, pkt, n);
		return;
	}

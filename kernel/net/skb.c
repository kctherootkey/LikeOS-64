...
static inline void skb_init_slot(sk_buff_t *skb, uint8_t *data,
				 uint32_t end_off, uint32_t sig)
{
	skb->next = NULL;
	skb->refcount = 0;
	skb->pool_sig = sig;
...
void skb_pool_init(void)
{
	...
	for (int i = SKB_SMALL_COUNT - 1; i >= 0; i--) {
		...
		skb_init_slot(skb, small_pool[i].data, SKB_SMALL_DATA,
			      SKB_SIG_SMALL);
	...
	for (int i = SKB_JUMBO_COUNT - 1; i >= 0; i--) {
		...
		skb_init_slot(skb, jumbo_pool[i].data, SKB_JUMBO_DATA,
			      SKB_SIG_JUMBO);
...
sk_buff_t *skb_alloc(uint32_t min_payload)
{
	...
	if (need > SKB_JUMBO_DATA) {
		WARN_RATELIMIT(1,
			       "skb_alloc: oversized request %u (jumbo max=%u)",
			       need, SKB_JUMBO_DATA);
	...
	WARN_ON(skb->data < skb->head ||
		skb->data >
			skb->end); /* skb->data out of [head,end] bounds after skb_reset */
	return skb;
}
...
void skb_put(sk_buff_t *skb)
{
	...
	if (old != 1)
		return; // still referenced by someone else
	...
	if (skb->pool_sig == SKB_SIG_SMALL) {
		...
		WARN_ON_ONCE(
			1); /* bad pool_sig: SKB corruption - not returning to pool */
	}
}
...
void skb_queue_tail(skb_queue_t *q, sk_buff_t *skb)
{
	...
	q->len++;
	WARN_ON((q->head == NULL) !=
		(q->tail == NULL)); /* head/tail asymmetry: queue corruption */
...
sk_buff_t *skb_queue_head(skb_queue_t *q)
{
	...
	if (skb) {
		...
		q->len--;
		WARN_ON((int)q->len <
			0); /* skb queue len underflow - more dequeues than enqueues */
		skb->next = NULL;
	}
	spin_unlock_irqrestore(&q->lock, f);
	return skb;

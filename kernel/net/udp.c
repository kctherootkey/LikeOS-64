// LikeOS UDP (User Datagram Protocol)
//
	     ...
	     uint16_t dst_port, const uint8_t *data, uint16_t len)
{
	...
	WARN_ON(src_port ==
		0); /* UDP source port is 0 - ephemeral ports must be assigned */
	WARN_ON(dst_port == 0); /* UDP destination port is 0 - invalid */
	WARN_ON(len >
		65507); /* UDP payload exceeds max (65535 - IP header - UDP header) */
	if (!dev)
		return -1;

	uint32_t udp_len = sizeof(udp_header_t) + (uint32_t)len;
	if (udp_len > 65535U - sizeof(ipv4_header_t))
		return -1;
	    ...
	    const uint8_t *data, uint16_t len, uint8_t ttl, uint8_t tos)
{
	...
	if (!sock_find_udp(dst_port, dst_ip)) {
		...
		uint8_t quoted[sizeof(ipv4_header_t) + sizeof(udp_header_t) +
			       8];
		ipv4_header_t *ip = (ipv4_header_t *)quoted;
		ip->version_ihl = 0x45;
		ip->tos = 0;
		ip->total_length =
			net_htons((uint16_t)(sizeof(ipv4_header_t) + udp_len));
		ip->identification = 0;
		ip->flags_fragment = 0;
		ip->ttl = 64;
		...
		if (copy_len > sizeof(quoted) - sizeof(ipv4_header_t))
			copy_len = (uint16_t)(sizeof(quoted) -
					      sizeof(ipv4_header_t));
		for (uint16_t i = 0; i < copy_len; i++)
			quoted[sizeof(ipv4_header_t) + i] = data[i];

		icmp_send_error_packet(
			dev, src_ip, ICMP_DEST_UNREACH, ICMP_PORT_UNREACH,
			quoted, (uint16_t)(sizeof(ipv4_header_t) + copy_len),
			0);
		return;
	}

	udp_deliver_to_socket(src_ip, src_port, dst_ip, dst_port, payload,
			      payload_len, ttl, tos, dev);
}
			   ...
			   uint8_t ttl, uint8_t tos, net_device_t *dev)
{
	...
	if (copy_len > NET_RX_BUF_SIZE)
		copy_len = NET_RX_BUF_SIZE;
	for (uint16_t i = 0; i < copy_len; i++)
	...
	sk->udp_rx_queue[sk->udp_rx_tail].from.sin_addr.s_addr =
		net_htonl(src_ip);
	...
	if (dev) {
		for (int idx = 0; idx < 16; idx++) {
			if (net_get_device(idx) == dev) {
				sk->udp_rx_queue[sk->udp_rx_tail].ifindex =
					(uint32_t)idx;
				break;
			}
		}

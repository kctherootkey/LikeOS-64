...
int net_rl_allow(net_rl_t *rl)
{
	...
	WARN_ON(rl->tokens >
		rl->burst); /* token bucket exceeded burst limit - replenishment clamping failed */
	if (rl->tokens == 0)
		return 0;
	rl->tokens--;
	return 1;
		     ...
		     uint32_t burst)
{
	BUG_ON(t == NULL);
	WARN_ON(burst ==
		0); /* src ratelimiter with burst==0 will drop every packet from every source */
	WARN_ON(rate_per_tick ==
		0); /* src ratelimiter with rate==0: once bucket drains it never replenishes */
	...
	if (hit < 0) {
		...
		if (slot < 0) {
			...
			for (int i = 1; i < NET_RL_SRC_TABLE_SIZE; i++) {
				if ((int32_t)(t->entries[i].last_use_tick -
					      oldest) < 0) {
		...
		WARN_ON(slot <
			0); /* LRU eviction found no slot despite non-empty table - NET_RL_SRC_TABLE_SIZE must be > 0 */

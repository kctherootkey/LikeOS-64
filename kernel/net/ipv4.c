...
{
	if (mtu < 68)
		mtu = 68; // RFC 791 minimum
	uint64_t f;
	spin_lock_irqsave(&pmtu_lock, &f);
	...
	for (int i = 0; i < PMTU_CACHE_SIZE; i++) {
		...
		if (pmtu_cache[i].dst_ip == 0 && free_slot < 0)
			free_slot = i;
		if (pmtu_cache[i].last_use < pmtu_cache[oldest].last_use)
			oldest = i;
	}
...
static void ipv4_dispatch_payload(net_device_t *dev, uint32_t src_ip,
				  ...
				  uint8_t ttl, uint8_t tos)
{
	...
	case IP_PROTO_TCP:
		...
		break;
	default:
		WARN_RATELIMIT(1, "ipv4_dispatch: unknown protocol %u",
			       protocol);
		break;
	}
}

static ipv4_reassembly_slot_t *ipv4_get_reassembly_slot(uint32_t src_ip,
							uint32_t dst_ip,
							uint16_t identification,
							uint8_t protocol)
{
...
static void ipv4_mark_fragment(ipv4_reassembly_slot_t *slot, uint16_t offset,
			       uint16_t len)
{
				    ...
				    uint16_t payload_len, uint8_t ttl)
{
	...
	WARN_RATELIMIT(offset > 65528,
		       "ipv4: fragment offset %u > 65528 (malformed)", offset);
	uint16_t identification = net_ntohs(ip->identification);
	ipv4_reassembly_slot_t *slot = ipv4_get_reassembly_slot(
		src_ip, dst_ip, identification, ip->protocol);
	...
	ipv4_dispatch_payload(dev, src_ip, dst_ip, ip->protocol, slot->data,
			      slot->total_len, ttl, ip->tos);
	slot->active = 0;
	return 1;
}

static int ipv4_send_common(net_device_t *dev, uint32_t dst_ip,
			    uint8_t protocol, const uint8_t *payload,
			    uint16_t len, uint8_t ttl, uint8_t tos)
{
	BUILD_BUG_ON(sizeof(ipv4_header_t) != 20);
	...
	if (!out_dev) {
		...
		if (out_dev->gateway != 0 &&
		    (dst_ip & out_dev->netmask) !=
			    (out_dev->ip_addr & out_dev->netmask)) {
	...
	uint16_t mtu =
		out_dev == net_get_loopback() ? NET_MTU_DEFAULT : out_dev->mtu;
	if (mtu < sizeof(ipv4_header_t) + 8)
		return -1;
	...
	while (offset < len) {
		...
		if (chunk)
			mm_memcpy(buf + sizeof(ipv4_header_t), payload + offset,
				  chunk);
		...
		if (out_dev == net_get_loopback()) {
			...
			ret = eth_send(out_dev, dst_mac, ETH_P_IP, skb->data,
				       skb->len);
			skb_put(skb);
		} else {
		  ...
		  const uint8_t *full_ip_packet, uint16_t total_len)
{
	...
	if (!dev || !full_ip_packet || total_len < sizeof(ipv4_header_t))
		return -1;
	uint32_t next_hop = dst_ip;
	net_device_t *out_dev = route_lookup(dst_ip, &next_hop);
	if (!out_dev)
...
void ipv4_rx(net_device_t *dev, const uint8_t *data, uint16_t len)
{
	...
	if (dev != net_get_loopback()) {
		// Drop: 0.0.0.0/8
		if ((src_ip & 0xFF000000u) == 0x00000000u)
			return;
		// Drop: 127.0.0.0/8
		if ((src_ip & 0xFF000000u) == 0x7F000000u)
			return;
		// Drop: 169.254.0.0/16 (link-local — should not appear routed)
		if ((src_ip & 0xFFFF0000u) == 0xA9FE0000u)
			return;
		// Drop: 240.0.0.0/4 (reserved / class E)
		if ((src_ip & 0xF0000000u) == 0xF0000000u)
			return;
		// Drop: 255.x.x.x
		if ((src_ip & 0xFF000000u) == 0xFF000000u)
			return;
	}
	...
	if (ihl > 20) {
		...
		for (uint8_t oi = 0; oi < opt_end;) {
			...
			if (opt_len < 2 || oi + opt_len > opt_end)
				break;
	...
	if (dev != net_get_loopback() && dev->netmask != 0) {
		...
		if (dst_ip == bcast && ip->protocol == IP_PROTO_ICMP)
			return;
	}
	...
	int is_mcast = (dst_ip >= 0xE0000000U && dst_ip < 0xF0000000U);
	if (dev != net_get_loopback() && dst_ip != dev->ip_addr &&
	    dst_ip != 0xFFFFFFFF && dst_ip != (dev->ip_addr | ~dev->netmask) &&
	    (dst_ip & 0xFF000000) != 0x7F000000 && !is_mcast &&
	    dev->ip_addr != 0) {
		return;
	}
	...
	if ((frag & IPV4_FLAG_MF) || (frag & IPV4_FRAG_OFFSET_MASK)) {
		...
		if (!frag_ok)
			return;
		ipv4_reassemble_fragment(dev, ip, src_ip, dst_ip, payload,
					 payload_len, ip->ttl);
		return;
	}
	...
	ipv4_dispatch_payload(dev, src_ip, dst_ip, ip->protocol, payload,
			      payload_len, ip->ttl, ip->tos);
}

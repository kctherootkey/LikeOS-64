...
static uint8_t xterm256_to_vga(int n)
{
	...
	if (n > 255)
		n = 255;
	if (n < 8)
		return ansi_to_vga_fg[n];
	if (n < 16)
		return ansi_to_vga_fg[n - 8] + 8;
	if (n >= 232) {
		int g = n - 232;
		if (g < 4)
			return 0;
	...
	if (r == g && g == b) {
		...
		base = ansi_to_vga_fg[1];
	else if (g >= r && g >= b)
		base = ansi_to_vga_fg[2];
	else
		base = ansi_to_vga_fg[4];
	if (r >= 3 && g >= 3 && b < 3)
		base = ansi_to_vga_fg[3];
	else if (r >= 3 && b >= 3 && g < 3)
		base = ansi_to_vga_fg[5];
	else if (g >= 3 && b >= 3 && r < 3)
		base = ansi_to_vga_fg[6];
...
static void vt_set_cursor(struct vt_state *vt, int row, int col)
{
	...
	WARN_ON(vt->rows == 0 ||
		vt->cols ==
			0); /* vt_set_cursor called with zero terminal dimensions */
	if (row < 0)
		row = 0;
	if (col < 0)
		col = 0;
...
static void vt_apply_sgr(struct vt_state *vt)
{
	...
	for (int i = 0; i < vt->nparam; i++) {
		...
		if (p == 0) {
			...
			vt->bold = 1;
			if (!vt->fg_is_rgb && vt->cur_vga_fg < 8)
				vt->cur_vga_fg += 8;
		} else if (p == 22) {
			int was_bold = vt->bold;
			vt->bold = 0;
			if (was_bold && !vt->fg_is_rgb && vt->cur_vga_fg >= 9 &&
			    vt->cur_vga_fg <= 14)
				vt->cur_vga_fg -= 8;
		} else if (p >= 30 && p <= 37) {
...
static void vt_cells_fill_row_range(struct vt_state *vt, int row, int col_start,
				    int col_end)
{
	if (row < 0 || row >= VT_MAX_ROWS)
		return;
	if (col_start < 0)
		col_start = 0;
	if (col_end >= VT_MAX_COLS)
...
static void vt_cells_insert_lines(struct vt_state *vt, int n,
				  int top __attribute__((unused)), int bot)
{
...
static void vt_cells_delete_lines(struct vt_state *vt, int n,
				  int top __attribute__((unused)), int bot)
{
...
{
	vt_cells_fill_row_range(vt, vt->cur_row, vt->cur_col,
				vt->cur_col + n - 1);
}
...
vt_redraw_all_from_cells(struct vt_state *vt)
{
	int rows = vt->rows > 0 ? vt->rows : 25;
	int cols = vt->cols > 0 ? vt->cols : 80;
	if (rows > VT_MAX_ROWS)
...
#define P1_OR(def) \
	((vt->nparam >= 1 && vt->params[0] > 0) ? vt->params[0] : (def))

		switch (ch) {
		case 'H':
		case 'f': {
			/* CUP — Cursor Position (1-based). */
			int row = (vt->nparam >= 1 && vt->params[0] > 0) ?
					  vt->params[0] - 1 :
					  0;
			int col = (vt->nparam >= 2 && vt->params[1] > 0) ?
					  vt->params[1] - 1 :
					  0;
		...
		case 'r': {
			...
			int top = (vt->nparam >= 1 && vt->params[0] > 0) ?
					  vt->params[0] - 1 :
					  0;
			int bot = (vt->nparam >= 2 && vt->params[1] > 0) ?
					  vt->params[1] - 1 :
					  rows2 - 1;
			if (top < 0)
				top = 0;
		...
		case 't': {
			...
			if (op == 18) {
				...
				tty_enqueue_read_locked(vt->tty, ';');
				ln = itoa_simple((int)vt->tty->winsz.ws_row,
						 num);
				for (int i = 0; i < ln; i++)
					tty_enqueue_read_locked(vt->tty,
								num[i]);
				tty_enqueue_read_locked(vt->tty, ';');
				ln = itoa_simple((int)vt->tty->winsz.ws_col,
						 num);
				for (int i = 0; i < ln; i++)
					tty_enqueue_read_locked(vt->tty,
								num[i]);
		...
		case 'l': {
			...
			if (vt->private_mode) {
				...
				if (mode == 25) {
					...
					vt->tty->mouse_btn_event =
						(uint8_t)enable;
					if (!enable)
						vt->tty->mouse_sgr_mode = 0;
				} else if (mode == 1006) {
					vt->tty->mouse_sgr_mode =
						(uint8_t)enable;
					...
					if (enable) {
						if (mode == 1049) {
							vt_save_state(vt);
							vt->alt_screen_sb_total =
								console_get_sb_total();
						}

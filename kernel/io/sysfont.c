...
static sysfont_t g_sysfont = { .glyphs = 0,
			       .numglyphs = 0,
			       .width = 0,
			       .height = 0,
			       .bytesperglyph = 0,
...
int sysfont_load(const char *path)
{
	...
	if (magic[0] == PSF2_MAGIC0 && magic[1] == PSF2_MAGIC1 &&
		...
		// Read all glyph data
		long glyph_data_size =
			(long)(header.numglyph * header.bytesperglyph);
		BUG_ON(glyph_data_size >
		       (long)sizeof(
			       g_glyph_buffer)); /* PSF2 glyph data exceeds static buffer (already validated above) */

...
int scrollbar_init(scrollbar_t *scrollbar, uint32_t x, uint32_t y,
		   uint32_t height)
{
	if (!scrollbar)
		return -1;
...
void scrollbar_calculate_positions(scrollbar_t *scrollbar)
{
	...
	scrollbar->track_height =
		scrollbar->height - (2 * scrollbar->button_size);

	// Default thumb until content sync sets real values
	scrollbar->thumb_height = (scrollbar->track_height > 0) ?
					  (scrollbar->track_height / 4) :
					  0;
	if (scrollbar->thumb_height < 8)
		scrollbar->thumb_height = 8; // Minimum size
...
void scrollbar_render_thumb(scrollbar_t *scrollbar)
{
	...
	uint32_t width = (scrollbar->width > 2) ?
				 (scrollbar->width - 2) :
				 scrollbar->width; // 2px inset guard
	...
	draw_rounded_rect(x, y, width, height, bg_color,
			  SCROLLBAR_BORDER_COLOR - 0x222222);
...
void draw_triangle_up(uint32_t center_x, uint32_t center_y, uint32_t size,
		      uint32_t color)
{
...
void draw_triangle_down(uint32_t center_x, uint32_t center_y, uint32_t size,
			uint32_t color)
{
			...
			uint32_t color_top, uint32_t color_bottom)
{
	...
	for (uint32_t row = 0; row < height; row++) {
		...
		uint8_t r =
			((r_top * (255 - factor)) + (r_bottom * factor)) / 255;
		uint8_t g =
			((g_top * (255 - factor)) + (g_bottom * factor)) / 255;
		uint8_t b =
			((b_top * (255 - factor)) + (b_bottom * factor)) / 255;
...
{
	switch (state) {
	case SCROLLBAR_STATE_HOVER:
		return SCROLLBAR_BUTTON_HOVER;
	case SCROLLBAR_STATE_PRESSED:
		return SCROLLBAR_BUTTON_PRESSED;
	default:
		return SCROLLBAR_BUTTON_NORMAL;
	}
}
...
uint32_t get_thumb_color(uint8_t state)
{
	switch (state) {
	case SCROLLBAR_STATE_HOVER:
		return SCROLLBAR_THUMB_HOVER;
	case SCROLLBAR_STATE_PRESSED:
		return SCROLLBAR_THUMB_PRESSED;
	default:
		return SCROLLBAR_THUMB_NORMAL;
	}
}
...
void scrollbar_set_button_state(scrollbar_t *scrollbar, uint8_t button,
				uint8_t state)
{
	if (!scrollbar)
		return;
...
int scrollbar_init_system_default(scrollbar_t *scrollbar)
{
	...
	if (scrollbar_init(scrollbar, scrollbar_x, scrollbar_y,
			   scrollbar_height) != 0) {
		return -1;
	}
...
void scrollbar_compute_geometry(scrollbar_t *sb)
{
	...
	sb->track_height = (sb->height > 2 * sb->button_size) ?
				   (sb->height - 2 * sb->button_size) :
				   0;
	...
	if (sb->total_content == 0 || sb->visible_content == 0 ||
	    sb->total_content <= sb->visible_content) {
		sb->thumb_height = sb->track_height; // all content fits
		sb->thumb_y = sb->track_y;
		return;
	}
	uint32_t min_thumb = 8;
	uint32_t h = (uint32_t)((uint64_t)sb->track_height *
				(uint64_t)sb->visible_content /
				(uint64_t)sb->total_content);
	if (h < min_thumb)
		h = min_thumb;
	if (h > sb->track_height)
		h = sb->track_height;
	sb->thumb_height = h;
	...
	sb->thumb_y = ty;
	WARN_ON(sb->thumb_y <
		sb->track_y); /* thumb_y below track_y: geometry overflow */
	WARN_ON(sb->thumb_height > 0 &&
		sb->thumb_y + sb->thumb_height >
			sb->track_y +
				sb->track_height); /* thumb extends past track bottom */
}
...
int scrollbar_hit_up(const scrollbar_t *sb, uint32_t x, uint32_t y)
{
	if (!sb || !sb->visible)
		return 0;
	return (x >= sb->x && x < sb->x + sb->width && y >= sb->y &&
		y < sb->y + sb->button_size);
}
...
int scrollbar_hit_down(const scrollbar_t *sb, uint32_t x, uint32_t y)
{
	...
	return (x >= sb->x && x < sb->x + sb->width && y >= by &&
		y < by + sb->button_size);
}
...
int scrollbar_hit_thumb(const scrollbar_t *sb, uint32_t x, uint32_t y)
{
	...
	return (x >= tx && x < tx + tw && y >= sb->thumb_y &&
		y < sb->thumb_y + sb->thumb_height);
}

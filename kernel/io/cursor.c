...
static cursor_data_t g_cursor = { .pixels = 0,
				  .width = 0,
				  .height = 0,
				  .xhot = 0,
				  .yhot = 0,
...
int cursor_load(const char *path)
{
	...
	// Validate dimensions
	if (img_header.width > CURSOR_MAX_WIDTH ||
	    img_header.height > CURSOR_MAX_HEIGHT) {
		kprintf("cursor: image too large (%ux%u, max %ux%u)\n",
			img_header.width, img_header.height, CURSOR_MAX_WIDTH,
			CURSOR_MAX_HEIGHT);
		vfs_close(file);
		return -1;
	}
	...
	g_cursor.loaded = 1;
	WARN_ON_ONCE(
		img_header.xhot >= img_header.width ||
		img_header.yhot >=
			img_header
				.height); /* cursor hotspot outside image bounds */

	kprintf("cursor: loaded %s (%ux%u, hotspot %u,%u)\n", path,
		img_header.width, img_header.height, img_header.xhot,
		img_header.yhot);

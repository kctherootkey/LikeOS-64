...
static const uint32_t size_classes[SLAB_NUM_CLASSES] = { 32,  64,   128, 256,
							 512, 1024, 2048 };
...
static slab_page_t *slab_alloc_page(slab_cache_t *cache)
{
	...
	WARN_ON(phys_page &
		(PAGE_SIZE - 1)); /* slab physical page is not page-aligned */
	...
	if (!mm_map_page(virt_addr, phys_page,
	...
	WARN_ON(slab->free_count ==
		0); /* newly-allocated slab has zero total_objects: calc_objects_per_slab returned 0 */
	slab->cache = cache;
	slab->next = NULL;
	slab->prev = NULL;
	slab->phys_addr =
		phys_page; // Store physical address for later unmapping
...
void slab_init(void)
{
	...
	for (int i = 0; i < SLAB_NUM_CLASSES; i++) {
		...
		cache->objects_per_slab =
			calc_objects_per_slab(size_classes[i]);
...
void *slab_alloc(size_t size)
{
	...
	if (size > SLAB_MAX_SIZE) {
		...
		for (size_t i = 0; i < page_count; i++) {
			...
			bool ok = mm_map_page(vaddr, paddr,
			...
			if (!ok) {
				...
				for (size_t j = 0; j < i; j++) {
					mm_unmap_page_no_shootdown(
						virt_base + (j * PAGE_SIZE));
				}
				if (i > 0 && sched_is_smp()) {
		...
		large_alloc_header_t *header =
			(large_alloc_header_t *)virt_base;
		header->magic = SLAB_LARGE_MAGIC;
		header->page_count = page_count;
		header->size = size;
		header->phys_addr = phys_pages;
	...
	BUG_ON(class_idx >=
	       SLAB_NUM_CLASSES); /* class_idx out of range after successful slab_get_size_class: slab descriptor corruption */
	...
	if (cache->partial_slabs) {
		...
		WARN_ON(slab->free_count ==
			0); /* partial slab has zero free objects: slab list classification is wrong */
		slab_global_stats.cache_hits++;
	}
	// Try empty slabs (cached for reuse)
	else if (cache->empty_slabs) {
		slab = cache->empty_slabs;
		slab_move_to_list(slab, &cache->empty_slabs,
				  &cache->partial_slabs);
	...
	if (slab->free_count == 0) {
		slab_move_to_list(slab, &cache->partial_slabs,
				  &cache->full_slabs);
	}
...
void slab_free(void *ptr)
{
	...
	large_alloc_header_t *large_header =
		(large_alloc_header_t *)((uint8_t *)ptr -
					 sizeof(large_alloc_header_t));
	...
	if (addr < SLAB_VIRT_BASE || addr >= slab_next_virt_addr) {
		...
		kprintf("SLAB: Invalid free - ptr %p not in SLAB range [%p-%p] (caller=%p)\n",
			ptr, (void *)SLAB_VIRT_BASE,
			(void *)slab_next_virt_addr, ra);
		return;
	}
...
#pragma GCC diagnostic pop
	...
	VM_BUG_ON(
		slab->free_count >
		slab->total_objects); /* free_count exceeds total - double-free or corruption */

	// Move slab between lists as needed
	if (was_full) {
		// Move from full to partial
		slab_move_to_list(slab, &cache->full_slabs,
				  &cache->partial_slabs);
	...
	for (int i = 0; i < SLAB_NUM_CLASSES; i++) {
		...
		if (cache->total_allocs > 0 || cache->slab_count > 0) {
			kprintf("  %4u bytes: allocs=%lu frees=%lu slabs=%u empty=%u\n",
				cache->object_size, cache->total_allocs,
				cache->total_frees, cache->slab_count,
				cache->empty_slab_count);
		}
	}
	kprintf("=================================\n");
...
int slab_validate(const char *caller)
{
	for (int i = 0; i < SLAB_NUM_CLASSES; i++) {
		...
		for (slab_page_t *slab = cache->partial_slabs; slab;
		     slab = slab->next) {
			if (slab->magic != SLAB_MAGIC) {
		...
		// Validate full slabs
		for (slab_page_t *slab = cache->full_slabs; slab;
		     slab = slab->next) {
			if (slab->magic != SLAB_MAGIC) {
		...
		for (slab_page_t *slab = cache->empty_slabs; slab;
		     slab = slab->next) {
			if (slab->magic != SLAB_MAGIC) {

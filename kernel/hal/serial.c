...
static inline void uart_out(unsigned short off, unsigned char v) { outb(COM1_PORT + off, v); }
static inline unsigned char uart_in(unsigned short off) { return inb(COM1_PORT + off); }
...
void serial_write_char(char c) {
    if (!g_serial_initialized) serial_init();
    if (!g_serial_available) return;
    if (c == '\n') {
...
void serial_write(const char* s, uint32_t len) {
    if (!g_serial_initialized) serial_init();
    if (!g_serial_available || !s) return;

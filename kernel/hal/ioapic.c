...
#include <kernel/hal/lapic.h> // for lapic_get_id_cpuid
#include <kernel/mm/memory.h> // for phys_to_virt
#include <kernel/ke/sched.h> // for sched_is_smp()
...
{
	if (!ioapic_base) {
		/* IOAPIC uses indirect registers (IOREGSEL/IOWIN) that require UC access.
           mm_map_device_mmio remaps the direct-map PTE with PWT+PCD (UC-). */
		void *va = mm_map_device_mmio(IOAPIC_DEFAULT_BASE, 1);
		if (va) {
			ioapic_base = (volatile uint32_t *)va;
			kprintf("IOAPIC: UC-remapped 0x%lx -> VA %p\n",
				IOAPIC_DEFAULT_BASE, va);
		} else {
			ioapic_base = (volatile uint32_t *)phys_to_virt(
				IOAPIC_DEFAULT_BASE);
			kprintf("IOAPIC: WARNING — UC remap failed, using WB direct-map\n");
		}
	}
	return ioapic_base;
}
...
int ioapic_configure_legacy_irq(uint8_t gsi, uint8_t vector, uint8_t polarity,
				uint8_t trigger_mode)
{
	...
	low |= vector; // bits 0-7
	WARN_ON(vector <
		32); /* IOAPIC vector must be >= 32 (vectors 0-31 are CPU exceptions) */
	...
	if (sched_is_smp()) {
		...
		// Single CPU: fixed delivery to BSP
		// Delivery mode = 000 (fixed)
		low |= (IOAPIC_DELMODE_FIXED << 8);
		// Destination mode = 0 (physical)
		// Use CPUID-based APIC ID (works before lapic_init)
		high = (lapic_get_id_cpuid() & 0xFF) << 24;
	}
	...
	kprintf("IOAPIC: GSI %u -> vector 0x%02x (polarity=%s, trigger=%s, %s)\n",
		gsi, vector, (polarity == IOAPIC_POLARITY_LOW) ? "low" : "high",
		(trigger_mode == IOAPIC_TRIGGER_LEVEL) ? "level" : "edge",
		sched_is_smp() ? "lowest-priority" : "fixed");
	return 0;
}
...
{
	if (low)
		*low = ioapic_read(IOAPIC_REG_REDIR(gsi));
	if (high)
		*high = ioapic_read(IOAPIC_REG_REDIR(gsi) + 1);
}

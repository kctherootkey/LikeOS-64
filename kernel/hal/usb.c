...
usb_device_t* usb_alloc_device(uint8_t port){
    if(g_usb_table.count >= USB_MAX_DEVICES) return 0;
    usb_device_t* d = &g_usb_table.devices[g_usb_table.count++];
    d->address = 0; // will be assigned after SET_ADDRESS
    d->port_number = port;
    d->speed = USB_SPEED_UNKNOWN; // TODO: derive from port status bits later
    d->slot_id = 0;
    d->vid = 0; d->pid = 0;
    d->class_code = 0; d->subclass = 0; d->protocol = 0;
    d->configured = 0;
    d->input_ctx = 0;
    d->device_ctx = 0;
    d->have_desc8 = 0;

...
const char* cpu_features_to_string(uint32_t features) {
    ...
    if (features & CPU_FEATURE_SSE2) kstrncat(feature_str, "SSE2 ", sizeof(feature_str));
    if (features & CPU_FEATURE_SSE3) kstrncat(feature_str, "SSE3 ", sizeof(feature_str));
...
static int merge_dirty_regions(void) {
    if (g_double_buffer.num_dirty_regions < 2) return 0;
    
    int merges = 0;
    
    for (uint32_t i = 0; i < g_double_buffer.num_dirty_regions; i++) {
        if (!g_double_buffer.dirty_regions[i].dirty) continue;
        
        for (uint32_t j = i + 1; j < g_double_buffer.num_dirty_regions; j++) {
            if (!g_double_buffer.dirty_regions[j].dirty) continue;
...
void fb_mark_dirty(uint32_t x1, uint32_t y1, uint32_t x2, uint32_t y2) {
    ...
    if (x1 >= g_double_buffer.width) x1 = g_double_buffer.width - 1;
    if (y1 >= g_double_buffer.height) y1 = g_double_buffer.height - 1;
    if (x2 >= g_double_buffer.width) x2 = g_double_buffer.width - 1;
    if (y2 >= g_double_buffer.height) y2 = g_double_buffer.height - 1;
...
void fb_set_pixel(uint32_t x, uint32_t y, uint32_t color) {
    if (!g_initialized) return;
    if (x >= g_double_buffer.width || y >= g_double_buffer.height) return;
...
uint32_t fb_get_pixel(uint32_t x, uint32_t y) {
    ...
    if (x >= g_double_buffer.width || y >= g_double_buffer.height) return 0;
...
void fb_copy_rect(uint32_t dst_x, uint32_t dst_y, uint32_t src_x, uint32_t src_y, 
    ...
    // Bounds checking
    if (dst_x + width > g_double_buffer.width) width = g_double_buffer.width - dst_x;
    if (dst_y + height > g_double_buffer.height) height = g_double_buffer.height - dst_y;
    if (src_x + width > g_double_buffer.width) width = g_double_buffer.width - src_x;
    if (src_y + height > g_double_buffer.height) height = g_double_buffer.height - src_y;
...
void fb_fill_rect(uint32_t x, uint32_t y, uint32_t width, uint32_t height, uint32_t color) {
    ...
    // Bounds checking
    if (x >= g_double_buffer.width || y >= g_double_buffer.height) return;
    if (x + width > g_double_buffer.width) width = g_double_buffer.width - x;
    if (y + height > g_double_buffer.height) height = g_double_buffer.height - y;
